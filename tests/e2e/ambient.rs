...
fn test_ambient_state_lifecycle() {
    use weavecoder::ambient::{AmbientCycleResult, AmbientState, AmbientStatus, CycleStatus};
...
fn test_ambient_scheduled_queue() {
    use weavecoder::ambient::{Priority, ScheduledItem, ScheduledQueue};

    let tmp = std::env::temp_dir().join("wvc-test-queue.json");
    ...
    queue.push(ScheduledItem {
        ...
        target: weavecoder::ambient::ScheduleTarget::Ambient,
...
fn test_adaptive_scheduler_intervals() {
    use weavecoder::ambient_scheduler::{AdaptiveScheduler, AmbientSchedulerConfig};
...
async fn test_ambient_end_cycle_tool() -> Result<()> {
    ...
    let provider: Arc<dyn weavecoder::provider::Provider> = Arc::new(provider);
    ...
    let result = weavecoder::tool::ambient::take_cycle_result();
...
async fn test_ambient_request_permission_tool() -> Result<()> {
    ...
    weavecoder::tool::ambient::register_ambient_session(ambient_session_id.clone());

    let response = agent.run_once_capture("Request permission").await?;
    weavecoder::tool::ambient::unregister_ambient_session(&ambient_session_id);
...
async fn test_ambient_runner_status() {
    use weavecoder::ambient_runner::AmbientRunnerHandle;
    use weavecoder::safety::SafetySystem;
...
async fn test_ambient_runner_trigger_and_stop() {
    use weavecoder::ambient::AmbientStatus;
    use weavecoder::ambient_runner::AmbientRunnerHandle;
    use weavecoder::safety::SafetySystem;
...
fn test_memory_reinforcement_provenance() {
    use weavecoder::memory::{MemoryCategory, MemoryEntry};
...
fn test_ambient_config_defaults() {
    use weavecoder::config::AmbientConfig;
