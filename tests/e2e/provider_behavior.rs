...
async fn test_resume_restores_model_and_tool_history() -> Result<()> {
    ...
    session.add_message(
        weavecoder::message::Role::User,
        vec![weavecoder::message::ContentBlock::Text {
            text: "Run a tool".to_string(),
            cache_control: None,
        }],
    );
    session.add_message(
        weavecoder::message::Role::Assistant,
        vec![
            weavecoder::message::ContentBlock::Text {
                text: "Running...".to_string(),
                cache_control: None,
            },
            weavecoder::message::ContentBlock::ToolUse {
                id: "tool-1".to_string(),
                name: "bash".to_string(),
    ...
    session.add_message(
        weavecoder::message::Role::User,
        vec![weavecoder::message::ContentBlock::ToolResult {
...
async fn test_resume_session_with_local_history_uses_metadata_only_history() -> Result<()> {
    ...
    let provider_dyn: Arc<dyn weavecoder::provider::Provider> = provider.clone();
...
/// The agent should identify as "wvc" or just a generic "coding assistant powered by Claude"
#[tokio::test]
async fn test_system_prompt_no_claude_code_identity() -> Result<()> {
    ...
    let provider_dyn: Arc<dyn weavecoder::provider::Provider> = provider;
