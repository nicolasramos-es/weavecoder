...
WVC_DIR = os.path.expanduser("~/.wvc")
...
def test_selfdev_status():
    ...
    try:
        ...
        result = send_cmd(sock, "create_session:selfdev:/home/jeremy/wvc")
...
def test_reload_context():
    ...
    context_candidates = sorted(
        glob.glob(os.path.join(WVC_DIR, "reload-context-*.json"))
    )
    legacy_context_path = os.path.join(WVC_DIR, "reload-context.json")
