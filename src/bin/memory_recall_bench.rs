...
//! REAL wvc retrieval primitives:
//!   - `weavecoder::memory_graph::MemoryGraph` deserialization (real on-disk graphs)
//!   - `weavecoder::embedding::embed` (real all-MiniLM-L6-v2 ONNX model)
//!   - `weavecoder::memory::format_context_for_relevance` (real live query window)
//!   - a faithful re-implementation of `score_and_filter` (cosine + gap filter)
//!
//! Privacy: all data lives OUTSIDE the repo (default `~/wvc-memory-bench`).
...
fn bench_root() -> PathBuf {
    std::env::var("MEMORY_BENCH_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| dirs_home().join("wvc-memory-bench"))
}
...
fn cmd_queries(args: &[String]) -> Result<()> {
    ...
    let sessions_dir = opts
        .get("sessions")
        .cloned()
        .unwrap_or_else(|| format!("{}/.wvc/sessions", dirs_home().display()));
    ...
    for (path, _) in sessions {
        ...
        let user_turns: Vec<usize> = messages
            .iter()
            .enumerate()
            .filter(|(_, m)| matches!(m.role, weavecoder::message::Role::User))
            .map(|(i, _)| i)
...
// ---------------- LLM judge (direct Anthropic via wvc Sidecar) ----------------
...
//
// The prompt/parse/rerank logic lives in the shared `weavecoder::memory_rerank`
...
use weavecoder::memory_rerank::{LLM_RERANK_SYSTEM, build_rerank_prompt, parse_rerank_response};
            ...
            .map(|input| {
                ...
                async move {
                    let sidecar = if backend == "openai" {
                        ...
                        weavecoder::sidecar::Sidecar::with_openai_model(&model, eff)
                    } else {
                        weavecoder::sidecar::Sidecar::with_claude_model(&model)
                    };
...
fn cmd_metrics(args: &[String]) -> Result<()> {
    ...
    // For `prod_hybrid`: seed a temp WVC_HOME project graph with the corpus and
    ...
    let prod_mgr = if config == "prod_hybrid" {
        ...
        unsafe { std::env::set_var("WVC_HOME", &tmp) };
        let project_dir = "/bench/prod-validate";
        let mgr = weavecoder::memory::MemoryManager::new().with_project_dir(project_dir);
    ...
    let alt_embedder = opts.get("embedder").map(|dir| {
        ...
        weavecoder::embedding::Embedder::load_from_dir(Path::new(dir)).expect("load alt embedder")
    });
    ...
    let openai_backend = if config == "openai_dense" || config == "openai_hybrid" {
        ...
        let key = opts
            ...
            .or_else(|| {
                weavecoder::provider_catalog::load_api_key_from_env_or_config(
                    "OPENAI_API_KEY",
                    "openai.env",
                )
            })
    ...
    let openai_corpus_emb: Vec<(String, Vec<f32>)> = match openai_backend.as_ref() {
        Some(b) => {
            use weavecoder::embedding_backend::EmbeddingBackend;
    ...
    let ce_reranker = opts.get("reranker").map(|dir| {
        ...
        weavecoder::embedding::CrossEncoder::load_from_dir(Path::new(dir)).expect("load reranker")
    });
        ...
        let raw: Vec<RecallResultRow> = rt.block_on(async {
            ...
            stream::iter(jobs)
                .map(|(qid, query, cands)| {
                    ...
                    async move {
                        let sidecar = if backend == "openai" {
                            weavecoder::sidecar::Sidecar::with_openai_model(&model, None)
                        } else {
                            weavecoder::sidecar::Sidecar::with_claude_model(&model)
                        };
                        let n = cands.len();
            ...
            stream::iter(jobs)
                .map(|(qid, query, cands)| {
                    ...
                    async move {
                        ...
                        let mut out_chars = 0usize;
                        for _ in 0..n_ens {
                            let sidecar = weavecoder::sidecar::Sidecar::with_claude_model(&model);
                            for attempt in 0..2 {
                                match sidecar.complete(JUDGE_SYSTEM, &prompt).await {
                                    Ok(resp) => {
                                        out_chars += resp.len();
...
fn cmd_probe(args: &[String]) -> Result<()> {
    ...
    let e = weavecoder::embedding::Embedder::load_from_dir(Path::new(&dir))?;
