//! `wvc provider-doctor` command: a user-facing strict provider/model diagnostic.
        ...
        crate::provider_catalog::openai_compatible_profile_by_id(provider).with_context(|| {
            format!(
                "`{provider}` is not a known OpenAI-compatible provider. \
                 Run `wvc provider-test-coverage` to see provider ids, or check your spelling."
            )
        })?;
        ...
        .with_context(|| {
            format!(
                "no API key found for `{provider}` (looked in env `{}` and `{}`). \
                 Run `wvc login --provider {provider}`, or use `--tier offline` to check wiring only.",
...
fn format_report(report: &DoctorReport, colorize: bool) -> String {
    ...
    out.push_str(match report.tier {
        DoctorTier::Offline => "(no API key, no spend: validates wvc wiring only)\n",
...
fn next_step_hint(checkpoint: &str) -> String {
    ...
    let hint = match checkpoint {
        cp::AUTH_CREDENTIAL_LOADED => {
            "  Next: run `wvc login --provider <provider>` to store a working credential."
        }
        ...
        | cp::MODEL_SWITCH_ROUTE => {
            "  Next: this is a wvc-side routing/picker bug for this provider. \
