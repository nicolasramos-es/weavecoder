...
pub async fn run_tui_client(
    ...
    if let Some(ref session_id) = resume_session {
        ...
        let _ = crossterm::execute!(std::io::stdout(), crossterm::terminal::SetTitle("wvc"));
    }
        ...
        if let Some(output) = video_output {
            let output_path = if output == "auto" {
                ...
                let safe_name = session_id_or_path
                    ...
                    .collect::<String>();
                std::path::PathBuf::from(format!("wvc_swarm_replay_{}_{}.mp4", safe_name, date))
            } else {
    ...
    if let Some(output) = video_output {
        let output_path = if output == "auto" {
            ...
            let safe_name = session_name
                ...
                .collect::<String>();
            std::path::PathBuf::from(format!("wvc_replay_{}_{}.mp4", safe_name, date))
        } else {
            std::path::PathBuf::from(output)
        };
        eprintln!(
...
pub fn list_sessions() -> Result<()> {
    fn build_resume_target_command(
        ...
        match target {
            ...
            wvc_tui_session_picker::ResumeTarget::CodexSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            wvc_tui_session_picker::ResumeTarget::PiSession { session_path } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            wvc_tui_session_picker::ResumeTarget::OpenCodeSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
            ...
            wvc_tui_session_picker::ResumeTarget::CursorSession { session_id, .. } => (
                exe.to_path_buf(),
                vec![
                    "--resume".to_string(),
