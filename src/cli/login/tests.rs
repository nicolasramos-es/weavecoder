...
fn scriptable_resume_command_matches_input_kind() {
    assert_eq!(
        ...
        "wvc login --provider openai --callback-url '<url-or-query>'"
    );
    assert_eq!(
        scriptable_resume_command("gemini", "auth_code"),
        "wvc login --provider gemini --auth-code '<code>'"
    );
    assert_eq!(
        scriptable_resume_command("copilot", "complete"),
        "wvc login --provider copilot --complete"
    );
}
...
fn load_pending_login_removes_expired_record() {
    ...
    set_or_clear_env("WVC_HOME", prev_home);
}
