...
pub(super) fn local_endpoint_hint(provider_id: &str) -> String {
    match provider_id {
        "ollama" => "Next step: install a model with `ollama pull llama3.2`, then run \
            ...
            .to_string(),
        "lmstudio" => "Next step: load a chat model in LM Studio's Local Server, then run wvc \
             with that exact model id, for example \
             `wvc --provider lmstudio --model <model-id> run 'hello'`."
            .to_string(),
        other => format!(
            "Next step: run wvc with a model available on this endpoint, for example \
             `wvc --provider {} --model <model-id> run 'hello'`.",
            other
        ),
    }
}
...
mod tests {
    ...
    fn extracted_hints_match_the_strings_login_printed_before_extraction() {
        assert_eq!(
            ...
            "Next step: install a model with `ollama pull llama3.2`, then run `wvc --provider ollama --model llama3.2 run 'hello'`."
        );
        assert_eq!(
            local_endpoint_hint("lmstudio"),
            "Next step: load a chat model in LM Studio's Local Server, then run wvc with that exact model id, for example `wvc --provider lmstudio --model <model-id> run 'hello'`."
        );
        assert_eq!(
            local_endpoint_hint("ollama-turbo"),
            "Next step: run wvc with a model available on this endpoint, for example `wvc --provider ollama-turbo --model <model-id> run 'hello'`."
        );
    }
