...
pub(crate) fn configure_provider_profile(
    ...
    Ok(ProviderSetupReport {
        ...
        run_command: format!(
            "wvc --provider-profile {} --model {} run 'hello'",
            shell_quote(&name),
            shell_quote(&model)
        ),
        auth_test_command: format!(
            "wvc --provider-profile {} auth-test --prompt {}",
            shell_quote(&name),
            shell_quote("Reply exactly WVC_PROVIDER_SETUP_OK")
        ),
    })
}
...
mod tests {
    ...
    fn provider_add_writes_named_profile_env_file_and_default() {
        ...
        let _key = EnvVarGuard::remove("WVC_PROVIDER_MY_API_API_KEY");
        ...
        assert_eq!(
            ...
            Some("WVC_PROVIDER_MY_API_API_KEY")
        );
        ...
        assert!(env_content.contains("WVC_PROVIDER_MY_API_API_KEY=secret-test-key"));
    }
