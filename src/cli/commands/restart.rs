...
pub async fn run_restart_save_command(auto_restore: bool) -> Result<()> {
    ...
    if snapshot.sessions.is_empty() {
        ...
        if auto_restore {
            println!("Automatic restore is armed for the next plain `wvc` launch.");
        }
        println!("\nNo active wvc windows were detected.");
        return Ok(());
    }
    ...
    if auto_restore {
        println!("\nAutomatic restore is armed for the next plain `wvc` launch.");
    }
...
pub async fn maybe_run_pending_restart_restore_on_startup() -> Result<bool> {
    ...
    if snapshot.auto_restore_on_next_start {
        ...
        println!(
            "Found a reboot snapshot with auto-restore enabled. Restoring {} wvc window(s)...\n",
            snapshot.sessions.len()
        );
...
pub fn run_restart_restore_command() -> Result<()> {
    ...
    if launched > 0 {
        println!("Restored {} wvc window(s).", launched);
    }
    ...
    if fallback > 0 {
        ...
        println!(
            "\nThe reboot snapshot was kept so you can try `wvc restart restore` again later."
        );
        return Ok(());
    }
...
fn current_restart_restore_exe() -> Result<PathBuf> {
    crate::build::client_update_candidate(false)
        ...
        .ok_or_else(|| anyhow::anyhow!("Could not determine wvc executable for restore"))
}
