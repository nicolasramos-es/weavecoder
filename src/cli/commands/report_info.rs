...
async fn run_auth_doctor_validation(
    ...
    .await
    {
        ...
        Err(_) => format!(
            "validation timed out after {}s; run `wvc auth-test --provider {}` for detailed output",
...
pub(super) fn run_version_command(emit_json: bool) -> Result<()> {
    let report = VersionReport {
        version: wvc_build_meta::version().to_string(),
        semver: wvc_build_meta::semver().to_string(),
        base_semver: wvc_build_meta::base_semver().to_string(),
        update_semver: wvc_build_meta::update_semver().to_string(),
        git_hash: wvc_build_meta::git_hash().to_string(),
        git_tag: wvc_build_meta::git_tag().to_string(),
        build_time: crate::build::current_binary_build_time_string()
            .unwrap_or_else(|| "unknown".to_string()),
        git_date: wvc_build_meta::git_date().to_string(),
        release_build: wvc_build_meta::is_release_build(),
    };
...
pub(super) async fn run_usage_command(emit_json: bool) -> Result<()> {
    ...
    if report.providers.is_empty() {
        ...
        println!();
        println!("Next steps:");
        println!("- Use `wvc login --provider claude` to connect Claude OAuth.");
        println!("- Use `wvc login --provider openai` to connect ChatGPT / Codex OAuth.");
        return Ok(());
    }
    ...
    if let Some(provider_arg) = provider_arg {
        let provider =
            crate::provider_catalog::resolve_login_provider(provider_arg).ok_or_else(|| {
                anyhow::anyhow!(
                    "Unknown provider '{}'. Use `wvc provider list` to see valid provider ids.",
                    provider_arg
                )
            })?;
...
mod tests {
    ...
    async fn cli_auth_status_doctor_and_login_lifecycle_uses_fresh_sandbox() {
        ...
        assert!(before_doctor_provider.diagnostics.iter().any(|line| {
            line == &format!("{} is not configured for wvc yet.", provider.display_name)
        }));
        assert!(
            before_doctor_provider
                ...
                .iter()
                .any(|line| {
                    line == &format!("Connect it: wvc login --provider {}", provider.id)
                })
        );
        ...
        assert!(
            after_doctor_provider
                .recommended_actions
                .iter()
                .any(|line| { line == "Review current state: wvc auth status --json" })
        );
    }
}
