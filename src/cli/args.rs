...
pub(crate) struct Args {
    ...
    /// Disable auto-detection of wvc repository and self-dev mode
    #[arg(long, global = true)]
...
pub(crate) enum Command {
    ...
    /// Run as an Agent Client Protocol (ACP) adapter backed by the Weavecoder daemon
    Acp,

    /// Manage the background server daemon (e.g. `wvc server stop`).
    Server {
        #[command(subcommand)]
    ...
    Login {
        /// Provider to log in to. Equivalent to --provider for this command, e.g. `wvc login google`.
        // Distinct clap id: the global `--provider` flag also has id "provider";
        // sharing the id makes clap drop the flag inside `login` (so
        // `wvc login --provider x` errors) and propagate the global default
        ...
        /// OpenAI-compatible API key. If omitted, wvc prompts securely when needed.
        #[arg(long)]
    ...
    /// Debug socket CLI - interact with running wvc server
    Debug {
    ...
    Transcript {
        ...
        /// How to apply the transcript inside Weavecoder
    ...
    Dictate {
        /// Type the transcript into the focused app instead of sending to wvc
        #[arg(long)]
        r#type: bool,
    },
    ...
    CodeSearch {
        /// Search query (symbol name, meaning, or substring).
        query: String,

        /// SQLite database path with an initialized code graph.
        #[arg(long, value_name = "PATH")]
        db: Option<String>,
    ...
    ///
    /// This is the endpoint the TypeScript SDK (`@nicolasramos-es/weavecoder-sdk`) connects to. It
    ...
    ApiBridge {
        /// Path of the API socket to listen on (default: $XDG_RUNTIME_DIR/wvc-api.sock)
        ///
...
pub(crate) enum CloudSessionsCommand {
    ...
    Upload {
        /// Path to a local Weavecoder session JSON file
        session_file: String,
    ...
    /// Upload the newest local Weavecoder session to Jade cloud storage
    UploadLatest {
        /// Directory containing local Weavecoder session JSON files
        #[arg(long, default_value = "~/.wvc/sessions")]
        sessions_dir: String,
    ...
    Sync {
        /// Directory containing local Weavecoder session JSON files (default: ~/.wvc/sessions)
        #[arg(long)]
...
pub(crate) struct JadeCloudOptions {
    ...
    /// Path to the private Jade session helper. Defaults to $WVC_JADE_SESSIONS_HELPER or ~/jade/scripts/jade_sessions.py.
    #[arg(long)]
...
pub(crate) enum RestartCommand {
    ...
    Save {
        /// Restore this reboot snapshot automatically the next time plain `wvc` starts
        #[arg(long)]
        auto_restore: bool,
    },
...
pub(crate) enum ProviderCommand {
    ...
    Add {
        ...
        /// API key value to store in wvc's private provider env file. Prefer --api-key-stdin for shell history safety.
        #[arg(long, conflicts_with_all = ["api_key_stdin", "no_api_key"])]
        api_key: Option<String>,

        /// Read the API key from stdin and store it in wvc's private provider env file
        #[arg(long, conflicts_with = "no_api_key")]
        api_key_stdin: bool,
        ...
        /// Private env file name under wvc's app config directory for stored API keys
        #[arg(long)]
