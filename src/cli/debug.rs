...
pub async fn run_debug_command(
    ...
    if !crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("\nOr use 'wvc debug start' to start a server.");
        eprintln!("Use 'wvc debug list' to see running servers.");
...
async fn debug_list_servers() -> Result<()> {
    ...
    if servers.is_empty() {
        println!("No running wvc servers found.");
        println!("\nStart one with: wvc debug start");
        return Ok(());
    }

    println!("Running wvc servers:\n");
    ...
    println!("  wvc debug -s /path/to/socket.sock sessions");
...
async fn debug_start_server(arg: &str, socket_path: Option<String>) -> Result<()> {
            ...
            .is_ok()
    {
        ...
        eprintln!("Use 'wvc debug list' to see all servers.");
        return Ok(());
    }
    ...
    if crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("⚠ Debug socket not enabled. Add to ~/.wvc/config.toml:");
        eprintln!("  [display]");
        eprintln!("  debug_socket = true");
    }
