...
pub fn hot_reload(session_id: &str) -> Result<()> {
    ...
    if let Ok(migrate_binary) = std::env::var("WVC_MIGRATE_BINARY") {
...
/// Minimum interval between `git fetch` update probes across all wvc
...
pub fn run_auto_update() -> Result<()> {
    ...
    let repo_dir =
        get_repo_dir().ok_or_else(|| anyhow::anyhow!("Could not find wvc repository"))?;
...
pub fn run_update() -> Result<()> {
    if update::is_release_build() {
        ...
        match update::check_for_update_blocking() {
            Ok(Some(release)) => {
                ...
                reload_server_after_update("installed update");
                update::print_centered("Restart wvc to use the new version.");
            }
            Ok(None) => {
                if repair_stale_shared_server_after_update_check() {
                    reload_server_after_update("repaired stale server target");
    ...
    update::print_centered(&format!("Updating wvc from {}...", repo_dir.display()));
...
fn reload_server_after_update(reason: &str) {
    ...
    let Some(exe) = exe else {
        crate::logging::warn("update: could not find wvc binary to reload stale server");
        return;
    };
