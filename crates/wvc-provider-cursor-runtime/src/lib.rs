...
//! `wvc_base::provider::cursor` because base's model-routing logic needs it
//! without a runtime.
...
fn runtime_cursor_api_key() -> Option<String> {
    wvc_base::auth::cursor::load_api_key().ok()
}
...
impl CursorCliProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(wvc_base::storage::app_config_dir()?.join("cursor_models_cache.json"))
    }
