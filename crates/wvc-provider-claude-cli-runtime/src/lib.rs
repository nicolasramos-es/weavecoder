...
impl Provider for ClaudeProvider {
        ...
        tokio::spawn(async move {
            ...
            for attempt in 0..MAX_RETRIES {
                if attempt > 0 {
                    ...
                    let base_delay = wvc_provider_core::attempt_tracker::retry_backoff_delay(
                        attempt,
                        RETRY_BASE_DELAY_MS,
                    );
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        match wvc_base::provider::fetch_anthropic_model_catalog_oauth(&access_token).await {
            Ok(catalog) => {
...
async fn run_claude_cli(
    ...
    tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = reader.next_line().await {
            wvc_base::logging::debug(&format!("[claude-cli] {}", line));
        }
        drop(tx_stderr);
    });
