...
//   cargo run --profile selfdev --example bench_anim -p wvc-tui-anim
// The wvc-tui-anim lib is pinned to opt-level=3 in every profile, so this
...
fn main() {
    ...
    let no = time(
        "new (angle table)",
        wvc_tui_anim::sample_orbit_rings,
        frames,
        sw,
        sh,
    );
