...
//!
//! wvc's palette (`theme.rs` and the many ad hoc `rgb(...)` call sites) is
...
//! detection (OSC 11 query / `WVC_THEME` / `display.theme` config) and
...
pub enum ThemeMode {
    /// Dark background, light text (wvc's native palette).
    #[default]
    Dark,
