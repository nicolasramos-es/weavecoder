...
I ran the same code-navigation task twice in the `weavecoder` codebase:
...
Benchmarks below were run on the `weavecoder` repo using the release build on:
...
For a representative `weavecoder` query, rough human-readable output sizes were:
