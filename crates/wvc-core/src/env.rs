...
/// Mutate the process environment for wvc runtime configuration.
///
/// Rust 2024 makes environment mutation unsafe because it can race with
/// concurrent environment access in foreign code. wvc intentionally mutates
...
where
    ...
    V: AsRef<OsStr>,
{
    // SAFETY: wvc treats these mutations as process-global configuration.
...
/// Remove a process environment variable used by wvc runtime configuration.
