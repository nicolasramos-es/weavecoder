...
/// API spells the latency-optimized tier `auto`; wvc also accepts `priority`
...
/// tier since wvc cannot see the per-request tier split.
...
pub fn copilot_pricing(model: &str, zero_premium_mode: bool) -> RouteCheapnessEstimate {
    ...
    RouteCheapnessEstimate::included_quota(
        ...
        Some(if zero_premium_mode {
            "Copilot zero-premium mode: wvc will send requests as agent/non-premium when possible"
                .to_string()
