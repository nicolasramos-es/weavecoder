...
//!
//! Historically wvc encoded that decision as free-form strings spread across
//! several overlapping vocabularies:
//!
//! | concept              | runtime env (`WVC_RUNTIME_PROVIDER`) | route / stable-id    | CLI `--provider` | model prefix     |
//! |----------------------|----------------------------------------|----------------------|------------------|------------------|
...
impl AuthRoute {
    ...
    /// Parse a dual-auth token from *any* of wvc's overlapping vocabularies
    ...
    /// Canonical `WVC_RUNTIME_PROVIDER` value that pins this route.
...
/// Read `WVC_RUNTIME_PROVIDER` and return the dual-auth route it pins, if any.
pub fn runtime_env_auth_route() -> Option<AuthRoute> {
    let value = std::env::var("WVC_RUNTIME_PROVIDER").ok()?;
    AuthRoute::parse(&value)
}

/// Read `WVC_RUNTIME_PROVIDER` and resolve the dual-auth mode it pins for a
