...
pub fn can_prompt_for_external_auth() -> bool {
    std::io::stdin().is_terminal()
        ...
        && std::env::var("WVC_NON_INTERACTIVE").is_err()
}
...
pub fn external_auth_blocked_message(
    ...
    format!(
        "Found existing {} credentials from {} at {} but wvc will not read them without confirmation. Re-run in an interactive terminal to approve this auth source for future wvc sessions, or run `{}`.",
        provider_name,
        source_name,
        path.display(),
        login_hint
...
pub fn prompt_to_trust_external_auth(
    ...
    eprintln!("wvc will only read that source in place after you approve it.");
    eprintln!("It will not move, delete, or rewrite the original auth there.");
    eprint!("Trust this auth source for future wvc sessions? [y/N]: ");
    io::stdout().flush()?;
...
impl ExternalAuthAutoImportOutcome {
    ...
    /// wvc's global provider preference: Anthropic first, then OpenAI, followed
...
fn prompt_to_review_external_auth_sources(
    ...
    eprintln!();
    eprintln!("Found existing logins that wvc can reuse.");
    eprintln!("Nothing has been imported yet.");
    eprintln!(
        "Approve the sources you want wvc to read in place; rejected sources stay untouched."
    );
    eprintln!();
...
fn approve_external_auth_review_candidate(candidate: &ExternalAuthReviewCandidate) -> Result<()> {
    match candidate.action {
        ...
        ExternalAuthReviewAction::ClaudeCodeNative => {
            // Trust, then snapshot the Keychain/env credentials into wvc's own
            ...
            if let Err(err) = auth::claude::import_native_credentials_into_account() {
                crate::logging::warn(&format!(
                    "Trusted Claude Code native credentials but could not snapshot them into wvc: {err}"
                ));
            }
        }
...
fn token_freshness_note(expires_at_ms: i64) -> String {
    ...
    if expires_at_ms <= now_ms {
        " The access token is expired; wvc will refresh it on first use, or run /login if that fails.".to_string()
    } else {
        String::new()
    }
}
...
// then discard the rotated result, breaking both wvc and the original tool.
