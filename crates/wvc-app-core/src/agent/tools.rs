...
pub(super) fn cap_tool_output_for_history(tool_name: &str, mut output: ToolOutput) -> ToolOutput {
    ...
    output.output = format!(
        "{}\n\n[Tool output truncated by wvc: tool `{}` produced {} chars; kept first {} chars to protect the remote protocol, session history, and prompt cache. Redirect large logs to a file and read targeted sections.]",
...
pub(super) fn tool_output_side_pane_images(
    ...
    output: &ToolOutput,
) -> Vec<wvc_session_types::RenderedImage> {
    ...
    output
        .images
        .iter()
        .map(|img| wvc_session_types::RenderedImage {
            ...
            anchor: Some(wvc_session_types::RenderedImageAnchor::ToolCall {
...
mod tests {
    ...
    fn cap_tool_output_adds_visible_truncation_notice() {
        ...
        assert!(capped.output.contains("Tool output truncated by wvc"));
    ...
    fn cap_sdk_tool_content_adds_same_notice() {
        ...
        assert!(capped.contains("Tool output truncated by wvc"));
