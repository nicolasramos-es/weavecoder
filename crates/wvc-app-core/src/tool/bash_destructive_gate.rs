...
pub(super) fn destructive_command_refusal(
    ...
    let risk_ctx = wvc_command_risk::RiskContext::from_env(working_dir);
    let assessment = wvc_command_risk::assess(command, &risk_ctx);
    if assessment.level.runs_immediately() {
        return None;
    }

    let justification = wvc_command_risk::Justification {
        text: justification.map(str::to_string),
    };
    match wvc_command_risk::gate(&assessment, &justification) {
        wvc_command_risk::GateOutcome::Allow => None,
        wvc_command_risk::GateOutcome::Deny { reason } => {
            crate::logging::warn(&format!("[bash] denied destructive command: {command}"));
            Some(reason)
        }
        wvc_command_risk::GateOutcome::Reflect { prompt } => {
...
pub(super) fn bash_parameters_schema() -> serde_json::Value {
    let cmd_desc = if cfg!(windows) {
        ...
        "The bash command to execute. Put large temp files under `$WVC_SCRATCH_DIR`, not `/tmp`."
    };
    serde_json::json!({
        "type": "object",
        "required": ["command"],
        "properties": {
            ...
            "run_in_background": {
                "type": "boolean",
                "description": "Run in background. Emit `WVC_PROGRESS {json}` lines for progress reporting."
            },
            "notify": {
                "type": "boolean",
