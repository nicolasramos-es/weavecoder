...
/// The `wvc login` invocation most likely to fix a credential wave for
...
fn plan_status_budget_line(summary: &PlanGraphStatus, deep_cap: usize) -> Option<String> {
    ...
    let budget_label = if budget == usize::MAX {
        format!("{} (member cap)", wvc_swarm_core::MAX_SWARM_MEMBERS)
    } else {
        budget.to_string()
    };
    ...
    let effective_budget = if budget == usize::MAX {
        wvc_swarm_core::MAX_SWARM_MEMBERS
    } else {
        budget
    };
...
fn format_swarm_model_list(
    ...
    model_routes: &[wvc_provider_core::ModelRoute],
) -> String {
