...
impl SelfDevTool {
    ...
    /// building wvc from source: the rust toolchain, git, and a local repo
    ...
    pub(super) async fn do_setup(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        // Rust toolchain: cargo + rustc are required to build wvc.
        ...
        if repo_dir.is_none() {
            ...
            if SelfDevTool::is_test_session() {
                clone_note = Some("Test mode: skipped cloning the wvc source.".to_string());
            } else if git_available {
                match Self::clone_selfdev_source() {
                    Ok(path) => {
                        clone_note = Some(format!("Cloned wvc source into {}.", path.display()));
                        repo_dir = Some(path);
                    }
                    Err(err) => {
                        clone_note =
                            Some(format!("Could not clone wvc source automatically: {err}",));
                    }
                }
            }
        }
        ...
        match &repo_dir {
            ...
            None => {
                let target = Self::selfdev_clone_dir()
                    ...
                    .unwrap_or_else(|_| "~/.wvc/source/wvc".to_string());
                checks.push(SetupCheck::missing(
                    "repository",
                    "no local wvc checkout found",
                    format!(
        ...
        if let Some(repo) = repo_dir.as_deref() {
            match build::find_dev_binary(repo) {
                ...
                None => checks.push(SetupCheck::missing(
                    ...
                    "Build it once with `wvc self-dev --build`, or inside a \
        ...
        if !failing.is_empty() {
            ...
            output.push_str(
                "\nAll prerequisites satisfied. Use `selfdev enter` to start working on wvc.\n",
            );
        }
    ...
    /// `selfdev find-config`: report the key wvc paths (config file, home,
    ...
    pub(super) async fn do_find_config(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        let wvc_home = storage::wvc_dir().ok();
        let config_path = wvc_home.as_ref().map(|home| home.join("config.toml"));
        ...
        let mut output = String::from("## wvc config & paths\n\n");
        output.push_str(&format!(
            "**Config file:** {}\n",
        ...
        let metadata = json!({
            ...
            "wvc_home": wvc_home.as_ref().map(|p| p.display().to_string()),
    ...
    pub(super) async fn do_reload_to_newer_build(&self, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        if !server::server_has_newer_binary() {
            return Ok(ToolOutput::new(
                "Already running the newest installed wvc build; no reload needed.",
            ));
        }

        let hash = wvc_build_meta::git_hash().to_string();
    ...
    fn selfdev_clone_dir() -> Result<std::path::PathBuf> {
        Ok(storage::wvc_dir()?.join("source").join("wvc"))
    }

    /// Clone the wvc source into the default self-dev source directory.
    fn clone_selfdev_source() -> Result<std::path::PathBuf> {
        ...
        if repo_dir.exists() {
            ...
            anyhow::bail!(
                "{} exists but is not a wvc repository; move it aside and retry",
                repo_dir.display()
            );
        }
        ...
        if !build::is_wvc_repo(&repo_dir) {
            anyhow::bail!(
                "cloned source at {} is not a valid wvc repository",
                repo_dir.display()
            );
        }
        Ok(repo_dir)
