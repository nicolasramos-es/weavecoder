...
pub const WVC_REPO_URL: &str = "https://github.com/nicolasramos-es/weavecoder.git";
...
impl BuildRequest {
    ...
    fn requests_dir() -> Result<PathBuf> {
        let dir = storage::wvc_dir()?.join("selfdev-build-requests");
...
impl SelfDevTool {
    ...
    fn build_lock_path(worktree_scope: &str) -> Result<PathBuf> {
        let dir = storage::wvc_dir()?.join("selfdev-build-locks");
        storage::ensure_dir(&dir)?;
