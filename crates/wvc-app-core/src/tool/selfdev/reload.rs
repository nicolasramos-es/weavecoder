use super::*;
pub use wvc_selfdev_types::ReloadRecoveryDirective;
...
impl ReloadContext {
    ...
    pub fn path_for_session(session_id: &str) -> Result<std::path::PathBuf> {
        ...
        Ok(storage::wvc_dir()?.join(format!("reload-context-{}.json", sanitized)))
    }

    fn legacy_path() -> Result<std::path::PathBuf> {
        Ok(storage::wvc_dir()?.join("reload-context.json"))
    }
...
impl SelfDevTool {
    pub(super) async fn do_reload(
        ...
        // a local debug/selfdev checkout may not have target/release/wvc).
        ...
        if !SelfDevTool::is_test_session() && !target_binary.exists() {
            return Ok(ToolOutput::new(
                format!(
                    "No binary found at {}.\n\
                     Run 'wvc self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p wvc --bin wvc' and then try reload again.",
        ...
        let version_before = wvc_build_meta::version().to_string();
