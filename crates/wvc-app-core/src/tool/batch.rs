        ...
        for call in calls.iter_mut() {
            if let Some(obj) = call.as_object_mut() {
                ...
                let top_level_accept = obj
                    .get(wvc_tool_core::ACCEPT_LARGE_OUTPUT_KEY)
                    .filter(|value| !value.is_null())
                    .cloned();
                if let Some(accept) = top_level_accept
                    && let Some(params) = obj.get_mut("parameters").and_then(Value::as_object_mut)
                    && !params.contains_key(wvc_tool_core::ACCEPT_LARGE_OUTPUT_KEY)
                {
                    params.insert(wvc_tool_core::ACCEPT_LARGE_OUTPUT_KEY.to_string(), accept);
                }
