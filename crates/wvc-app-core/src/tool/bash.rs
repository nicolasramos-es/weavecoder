...
const PROGRESS_MARKER_PREFIX: &str = "WVC_PROGRESS ";
const CHECKPOINT_MARKER_PREFIX: &str = "WVC_CHECKPOINT ";
const BACKGROUND_PROGRESS_GUIDANCE: &str = "For long-running background commands, prefer scripts or commands that periodically print progress updates. Best format: print lines starting with `WVC_PROGRESS ` followed by JSON like {\"percent\":42,\"message\":\"Running\"} or {\"current\":120,\"total\":1000,\"unit\":\"batches\",\"message\":\"Epoch 2/5\",\"eta_seconds\":30}. Supported JSON fields are `percent`, `message`, `current`, `total`, `unit`, `eta_seconds`, and optional `kind`=`indeterminate` or `kind`=`checkpoint`. For milestone-style wakeups, print `WVC_CHECKPOINT {\"message\":\"Unit tests passed\"}`. Generic fallback output that can be parsed includes `42%`, `3/10 tests`, `3 of 10 steps`, `1.5/3.0 GiB`, or phase lines like `Compiling ...`, `Downloading ...`, `Running ...`, and `Building ...`. If you are writing the script yourself, add these progress/checkpoint lines explicitly. Put large temporary files, worktrees, and virtual environments under `$WVC_SCRATCH_DIR`, not `/tmp`, because `/tmp` may be RAM-backed.";
...
async fn handle_background_output_line(
    ...
    match parse_heuristic_progress(raw_line) {
        ...
        Ok(None) => {}
        Err(err) => {
            let warning = format!("[wvc warning] failed to parse background progress: {err}\n");
...
fn tool_scratch_dir() -> Option<std::path::PathBuf> {
    let dir = std::env::var_os("WVC_SCRATCH_DIR")
...
fn configure_tool_scratch(command: &mut TokioCommand) {
    if let Some(dir) = tool_scratch_dir() {
        command.env("TMPDIR", &dir).env("WVC_SCRATCH_DIR", dir);
    }
}
...
fn build_detached_shell_wrapper(command: &str) -> StdCommand {
    ...
    cmd.arg("-lc")
        .arg(
            r#"eval "$WVC_RELOAD_DETACH_COMMAND"; status=$?; printf '\n--- Command finished with exit code: %s ---\n' "$status"; exit "$status""#,
        )
        .env("WVC_RELOAD_DETACH_COMMAND", command);
    if let Some(dir) = tool_scratch_dir() {
        cmd.env("TMPDIR", &dir).env("WVC_SCRATCH_DIR", dir);
    }
    cmd
}
...
mod utf8_truncation_tests {
    ...
    async fn build_shell_command_uses_disk_backed_scratch_directory() {
        let expected = super::tool_scratch_dir().expect("wvc scratch directory");
        let output = build_shell_command("printf '%s\\n%s\\n' \"$TMPDIR\" \"$WVC_SCRATCH_DIR\"")
            .output()
            .await
...
impl Tool for BashTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        if crate::browser::is_browser_command(&params.command) {
            ...
            // Start/attach a browser session for this wvc session.
