...
pub fn spawn_recent_index_warmup() {
    tokio::task::spawn_blocking(|| {
        ...
        remove_legacy_index();
        let empty_query = QueryProfile::new("__wvc_index_warmup__");

        let wvc_count = (|| -> Result<usize> {
            let sessions_dir = storage::wvc_dir()?.join("sessions");
            let collection = collect_session_files(&sessions_dir, DEFAULT_MAX_SCAN_SESSIONS)?;
            if collection.files.is_empty() {
                return Ok(0);
            }
            let _ = wvc_index_candidates(&collection.files, &empty_query)?;
            Ok(collection.files.len())
        })()
        .unwrap_or_else(|err| {
            crate::logging::info(&format!("wvc session index warmup skipped: {err}"));
            0
        });
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "source": {
                    "type": "string",
                    "enum": ["all", "wvc", "claude", "codex", "pi", "opencode", "cursor"],
...
fn normalize_source_filter(raw: Option<&str>) -> std::result::Result<Option<String>, String> {
    ...
    match normalized.as_str() {
        "all" => Ok(None),
        "wvc" | "claude" | "claude-code" | "codex" | "pi" | "opencode" | "cursor" => {
...
fn search_sessions_blocking(
    ...
    if source_matches_filter("wvc", options) {
        ...
        if !files.is_empty() {
            ...
            report.scanned_wvc_sessions = files.len();
            ...
            if !files.is_empty() {
                ...
                let mut candidates = if options.exhaustive {
                    ...
                    match wvc_index_candidates(&files, query) {
                ...
                report.candidate_wvc_sessions = candidates.len();
                if using_index {
...
fn wvc_index_candidates(
    ...
    let index_path = index_dir()?.join("session_search_wvc_index_v2.bin");
...
fn claude_index_candidates(
    sessions: &[wvc_import_core::ClaudeCodeSessionInfo],
    query: &QueryProfile,
) -> Vec<wvc_import_core::ClaudeCodeSessionInfo> {
    let index = index_dir()
                ...
                for session in chunk {
                    ...
                    records.push(ExternalSessionRecord {
                        ...
                        short_name: Some(format!(
                            "claude {}",
                            wvc_core::util::truncate_str(&session.session_id, 8)
                        )),
                        title: Some(title),
...
fn external_text_matches_query(text: &str, query: &QueryProfile) -> bool {
    wvc_session_types::normalized_session_search_text_matches(&text.to_lowercase(), query)
}
...
fn append_session_results(
    ...
    if !wvc_session_matches_filters(session, options) {
        return;
    }
    ...
    for (message_index, msg) in session.messages.iter().enumerate() {
        ...
        results.push(SearchResult {
            ...
            matched_terms: match_score.matched_terms,
            exact_match: match_score.exact_match,
            context: build_wvc_context(&session.messages, message_index, options),
        });
    }
}
...
fn wvc_session_matches_filters(session: &Session, options: &SearchOptions) -> bool {
    if !source_matches_filter("wvc", options) {
        return false;
    }
    if !provider_matches(session.provider_key.as_deref(), "wvc", options) {
        return false;
    }
