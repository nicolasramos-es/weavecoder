//! Launching wvc sessions in new terminal windows.
//!
//! These helpers spawn a fresh `wvc` process (resume or self-dev) inside a
...
/// spawn hooks and spawned terminals as `WVC_SPAWN_*` env vars so external
...
pub struct SessionSpawnContext {
    ...
    /// Extra `WVC_SPAWN_*` env entries (e.g. swarm/coordinator ids).
...
pub fn resumed_window_title(session_id: &str) -> String {
    ...
    {
        format!("wvc/{} {}", server_info.name, session_label)
    } else {
        format!("wvc {}", session_label)
    };
...
/// `WVC_FOCUS_SESSION_ID` and `WVC_FOCUS_TITLE` env vars.
...
/// var is exported natively and under a `WVC_CLIENT_<NAME>` alias.
pub fn focus_session_via_hook_with_env(
    ...
    cmd.args(args)
        .env("WVC_FOCUS_SESSION_ID", session_id)
        .env("WVC_FOCUS_TITLE", title)
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    for (key, value) in client_terminal_env {
        cmd.env(key, value);
        cmd.env(format!("WVC_CLIENT_{key}"), value);
    }
...
fn focus_title_best_effort(title: &str) {
    ...
    cmd.arg("-c")
        .arg(
            "sleep 0.4; \
             if command -v wmctrl >/dev/null 2>&1; then wmctrl -a \"$WVC_WINDOW_TITLE\" >/dev/null 2>&1 && exit 0; fi; \
             if command -v xdotool >/dev/null 2>&1; then xdotool search --name \"$WVC_WINDOW_TITLE\" windowactivate >/dev/null 2>&1 && exit 0; fi; \
             exit 0",
        )
        .env("WVC_WINDOW_TITLE", title)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn resume_terminal_candidates_windows() -> Vec<String> {
    std::env::var("WVC_RESUME_TERMINAL")
        .ok()
        .map(|value| {
            value
                .split(',')
...
pub fn spawn_resume_in_new_terminal_with_context(
    ...
    let mut wvc_args: Vec<String> = Vec::new();
    if let Some(provider_arg) = resume_provider_arg(provider_key) {
        wvc_args.push("--provider".to_string());
        wvc_args.push(provider_arg.to_string());
    }
    wvc_args.push("--resume".to_string());
    wvc_args.push(session_id.to_string());

    let hook_command = crate::terminal_launch::TerminalCommand::new(exe, wvc_args.clone())
