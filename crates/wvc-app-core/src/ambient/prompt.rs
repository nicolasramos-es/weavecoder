...
pub fn gather_feedback_memories(memory_manager: &crate::memory::MemoryManager) -> Vec<String> {
    ...
    let transcripts_dir = match crate::storage::wvc_dir() {
...
pub fn gather_recent_sessions(since: Option<DateTime<Utc>>) -> Vec<RecentSessionInfo> {
    let sessions_dir = match crate::storage::wvc_dir() {
...
pub fn build_ambient_system_prompt(
    ...
    prompt.push_str(
        "You are the ambient agent for wvc. You operate autonomously without \
    ...
    prompt.push_str(
        "## Instructions\n\n\
         ...
         without opening wvc. You can optionally target a specific channel \
