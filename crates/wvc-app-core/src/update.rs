...
const GITHUB_REPO: &str = "nicolasramos-es/weavecoder";
...
/// who opens wvc a few dozen times an hour exhausted the bucket and then saw
...
pub fn is_release_build() -> bool {
    wvc_build_meta::is_release_build()
}

fn current_update_semver() -> &'static str {
    wvc_build_meta::update_semver()
}

fn source_build_root() -> Result<PathBuf> {
    Ok(storage::wvc_dir()?.join("builds").join("source"))
}

fn source_build_repo_dir() -> Result<PathBuf> {
    Ok(source_build_root()?.join("wvc"))
}

pub fn should_auto_update() -> bool {
    if std::env::var("WVC_NO_AUTO_UPDATE").is_ok() {
        return false;
    }
...
fn github_api_request(
    ...
    if let Some(token) = wvc_base::github::github_public_api_token() {
...
fn prepare_stable_update_blocking() -> Result<PreparedUpdate> {
    let current_version = wvc_build_meta::version();
...
/// Build wvc from source by cloning/pulling the repo and running cargo build
fn build_from_source() -> Result<PathBuf> {
    let started = Instant::now();
    let build_dir = source_build_root()?;
    fs::create_dir_all(&build_dir)?;

    let repo_dir = build_dir.join("wvc");
    ...
    if repo_dir.join(".git").exists() {
        ...
        let output = std::process::Command::new("git")
            .args([
                "clone", "--depth", "1", "--branch", "main", &clone_url, "wvc",
            ])
...
pub fn download_and_install_blocking_with_progress(
    ...
    let temp_path = temp_dir.join(format!("wvc-update-{}", std::process::id()));
    ...
    if asset.name.ends_with(".tar.gz") {
        ...
        for entry in archive.entries()? {
            ...
            entry.unpack(&dest)?;
            if file_name.starts_with("wvc") && !file_name.ends_with(".bin") {
                extracted_binary = Some(dest);
            }
        }
        let Some(extracted_binary) = extracted_binary else {
            anyhow::bail!("Could not find wvc binary inside tar.gz archive");
        };
...
pub fn check_and_maybe_update(auto_install: bool) -> UpdateCheckResult {
    ...
    match check_for_update_blocking() {
        Ok(Some(release)) => {
            let current = wvc_build_meta::version().to_string();
...
mod tests {
    ...
    fn test_parse_sha256sums_accepts_standard_and_binary_lines() {
        ...
        let contents = format!(
            "# generated by release workflow\n{}  wvc-linux-x86_64.tar.gz\r\n{} *wvc-windows-x86_64.exe\n",
            digest_a, digest_b
        );
        let parsed = parse_sha256sums(&contents).unwrap();
        assert_eq!(
            parsed.get("wvc-linux-x86_64.tar.gz").map(String::as_str),
            Some(digest_a.as_str())
        );
        assert_eq!(
            parsed.get("wvc-windows-x86_64.exe").map(String::as_str),
    ...
    fn test_verify_asset_checksum_text_accepts_matching_digest() {
        ...
        let contents = format!("{}  wvc-linux-x86_64.tar.gz\n", digest);
        verify_asset_checksum_text(&contents, "wvc-linux-x86_64.tar.gz", bytes).unwrap();
    }
    ...
    fn test_verify_asset_checksum_text_rejects_mismatch() {
        ...
        let contents = format!("{}  wvc-linux-x86_64.tar.gz\n", wrong);
        let err = verify_asset_checksum_text(&contents, "wvc-linux-x86_64.tar.gz", b"actual")
            .unwrap_err()
            .to_string();
    ...
    fn test_parse_sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-sha  wvc-linux-x86_64.tar.gz\n")
            .unwrap_err()
            .to_string();
    ...
    fn test_summarize_git_pull_failure_diverged() {
        ...
        assert_eq!(
            ...
            wvc_update_core::GIT_PULL_DIVERGED_SUMMARY
        );
        assert!(wvc_update_core::summary_is_divergence(
            &summarize_git_pull_failure(stderr)
        ));
    }
