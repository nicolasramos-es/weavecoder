...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "mcp" || trimmed == "mcp:servers" {
        ...
        let config_path = if let Ok(wvc_dir) = crate::storage::wvc_dir() {
            let path = wvc_dir.join("mcp.json");
            if path.exists() {
        ...
        let default_model = match provider.as_str() {
            "claude" | "anthropic" => {
                if claude_usage_exhausted {
                    ...
                    wvc_provider_core::DEFAULT_CLAUDE_MODEL
                }
            }
            "openai" | "codex" => wvc_provider_core::DEFAULT_OPENAI_MODEL,
    ...
    if trimmed == "reload" {
        ...
        let wvc_dir = crate::storage::wvc_dir()?;
        let info_path = wvc_dir.join("reload-info");
