...
//
// Unlike the engine unit tests (which exercise `wvc_plan::dag` in isolation),
...
async fn e2e_deep_assignment_carries_fanout_and_artifact_contract() {
    ...
    assert!(
        prompt.contains(wvc_swarm_core::SWARM_DEEP_NODE_MARKER),
    ...
    assert!(
        !light_prompt.contains(wvc_swarm_core::SWARM_DEEP_NODE_MARKER),
...
async fn e2e_deep_gate_assignment_carries_inject_gap_contract() {
    ...
    assert!(prompt.contains(wvc_swarm_core::SWARM_DEEP_NODE_MARKER));
    assert!(
...
async fn e2e_complete_flows_artifact_to_downstream_assignment() {
    ...
    // api is now completed; ui should be runnable.
    {
        ...
        let ready = wvc_plan::next_runnable_item_ids(&plan.items, None);
        assert!(
