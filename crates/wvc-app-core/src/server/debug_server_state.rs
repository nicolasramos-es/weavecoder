...
pub(super) async fn maybe_handle_server_state_command(
    ...
    shutdown_signals: &Arc<RwLock<HashMap<String, wvc_agent_runtime::InterruptSignal>>>,
    ...
    if cmd == "memory-judge" || cmd == "memory:judge" || cmd == "server:memory-judge" {
        ...
        return Ok(Some(
            serde_json::to_string_pretty(&wvc_base::memory_judge_metrics::snapshot())
                .unwrap_or_else(|_| "{}".to_string()),
        ));
    }
    ...
    let (summary, actions) = match decision.primary_cause {
        "runaway_live_session_population" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["wvc debug 'swarm:list'", "wvc debug 'server:memory-incident'"]
                }),
                serde_json::json!({
                    "priority": 2,
                ...
                serde_json::json!({
                    ...
                    "commands": ["wvc debug 'server:memory-incident'", "python scripts/analyze_runtime_memory_log.py --days 1"]
                }),
                serde_json::json!({
                    "priority": 4,
                    ...
                    "commands": ["wvc debug 'allocator:purge'"]
                }),
            ],
        ),
        "allocator_retention" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["wvc debug 'allocator:purge'", "wvc debug 'server:memory-incident'"]
                }),
                serde_json::json!({
                    "priority": 2,
                    "action": "If retention repeatedly regrows, inspect allocation churn and allocator decay settings.",
                    "commands": ["wvc debug 'allocator'", "wvc debug 'allocator:decay:1000'"]
                }),
            ],
        ),
        "unattributed_live_heap" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["wvc debug 'server:memory'", "python scripts/analyze_runtime_memory_log.py --days 1"]
                }),
                serde_json::json!({
                    "priority": 2,
                    "action": "Escalate to a jemalloc-prof build and heap dump if attribution remains below 50% of live heap.",
                    "commands": ["wvc debug 'allocator:profile:on'", "wvc debug 'allocator:profile:dump /tmp/jcode-server.heap'"]
                }),
            ],
        ),
        ...
        _ => (
            ...
            vec![serde_json::json!({
                ...
                "commands": ["wvc debug 'server:memory-incident'"]
            })],
        ),
    };
