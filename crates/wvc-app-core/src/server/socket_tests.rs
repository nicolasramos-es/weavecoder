...
fn sibling_socket_path_roundtrip() {
    let main = std::path::PathBuf::from("/tmp/wvc.sock");
    let debug = std::path::PathBuf::from("/tmp/wvc-debug.sock");
...
fn cleanup_socket_pair_removes_main_and_debug_files() {
    ...
    let main = dir.join(format!("wvc-test-{}.sock", stamp));
    let debug = dir.join(format!("wvc-test-{}-debug.sock", stamp));
...
async fn reap_stale_socket_removes_dead_socket_pair_and_lock() {
    ...
    let socket = temp.path().join("wvc.sock");
    let debug = temp.path().join("wvc-debug.sock");
...
fn existing_server_start_errors_are_detected() {
    assert!(server_start_matches_existing_server(
        "Error: Another wvc server process is already running for runtime dir /run/user/1000"
    ));
    assert!(server_start_matches_existing_server(
        "Error: Refusing to replace active server socket at /run/user/1000/wvc.sock"
    ));
    assert!(!server_start_matches_existing_server(
