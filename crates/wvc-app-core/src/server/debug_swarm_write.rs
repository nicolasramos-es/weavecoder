...
fn debug_specs(specs: Vec<DebugNodeSpec>) -> Vec<wvc_plan::dag::NodeSpec> {
    specs
        .into_iter()
        .map(|s| wvc_plan::dag::NodeSpec {
            id: Some(s.id),
            content: s.content,
            kind: wvc_plan::bridge::parse_kind(s.kind.as_deref()),
...
async fn handle_debug_graph_op(arg: &str, ctx: &DebugSwarmWriteContext<'_>) -> String {
    use wvc_plan::bridge::{apply_task_graph, to_task_graph};
    use wvc_plan::dag;
...
mod tests {
    ...
    fn isolated_runtime(dir: &tempfile::TempDir) -> EnvGuard {
        ...
        let runtime = std::env::var_os("WVC_RUNTIME_DIR");
        crate::env::set_var("WVC_RUNTIME_DIR", dir.path());
        EnvGuard {
            _lock: lock,
            runtime,
        }
