...
fn deep_mode_assignment_content(
    ...
    if is_gate {
        ...
        let low_confidence_siblings: Vec<String> =
            wvc_plan::bridge::low_confidence_completed_ids(plan)
                .into_iter()
                .filter(|id| audited.contains(id.as_str()))
                .collect();
        wvc_swarm_core::append_deep_gate_instructions(
            content,
            item_id,
            &audited_ids,
        ...
        wvc_swarm_core::append_deep_node_instructions(content, item_id)
    }
}
...
async fn require_plan_driver_swarm(
    ...
    let is_deep_participant = {
        ...
        plans
            .get(&swarm_id)
            .map(|plan| {
                wvc_plan::bridge::parse_mode(&plan.mode) == wvc_plan::dag::Mode::Deep
