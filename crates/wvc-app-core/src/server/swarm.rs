...
/// `wvc_swarm_core` so the server, tools, and prompts all agree on the one
...
/// cap still apply.
pub(super) use wvc_swarm_core::MAX_SWARM_MEMBERS;
...
pub(super) async fn set_member_task_label(
    ...
    let Some(label) = wvc_swarm_core::derive_swarm_task_label(task_text) else {
        return;
    };
    let mut members = swarm_members.write().await;
    if let Some(member) = members.get_mut(session_id) {
