...
fn load_testers() -> Result<Vec<serde_json::Value>> {
    let path = crate::storage::wvc_dir()?.join("testers.json");
    if path.exists() {
...
async fn spawn_tester(opts: serde_json::Value) -> Result<String> {
    ...
    let debug_cmd = std::env::temp_dir().join(format!("wvc_debug_cmd_{}", id));
    let debug_resp = std::env::temp_dir().join(format!("wvc_debug_response_{}", id));
    let stdout_path = std::env::temp_dir().join(format!("wvc_tester_stdout_{}", id));
    let stderr_path = std::env::temp_dir().join(format!("wvc_tester_stderr_{}", id));
    ...
    cmd.current_dir(cwd);
    cmd.env(wvc_selfdev_types::CLIENT_SELFDEV_ENV, "1");
    cmd.env(
        "WVC_DEBUG_CMD_PATH",
        debug_cmd.to_string_lossy().to_string(),
    );
    cmd.env(
        "WVC_DEBUG_RESPONSE_PATH",
