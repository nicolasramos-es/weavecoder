...
fn test_side_pane_images_event_roundtrip() -> Result<()> {
    let event = ServerEvent::SidePaneImages {
        ...
        images: vec![wvc_session_types::RenderedImage {
            media_type: "image/png".to_string(),
            data: "base64-data".to_string(),
            label: Some("openclaw.png".to_string()),
            source: wvc_session_types::RenderedImageSource::ToolResult {
    ...
    assert_eq!(
        images[0].source,
        wvc_session_types::RenderedImageSource::ToolResult {
            tool_name: "read".to_string(),
        }
    );
    Ok(())
...
fn test_history_event_decodes_without_compaction_mode_for_older_servers() -> Result<()> {
    ...
    assert_eq!(
        compaction_mode,
        wvc_config_types::CompactionMode::Reactive
    );
...
fn test_history_event_roundtrip_preserves_side_panel_snapshot() -> Result<()> {
    let event = ServerEvent::History {
        ...
        compaction_mode: wvc_config_types::CompactionMode::Reactive,
        activity: None,
        side_panel: wvc_side_panel_types::SidePanelSnapshot {
            focused_page_id: Some("page-1".to_string()),
            pages: vec![wvc_side_panel_types::SidePanelPage {
                id: "page-1".to_string(),
                title: "Notes".to_string(),
                file_path: "/tmp/notes.md".to_string(),
                format: wvc_side_panel_types::SidePanelPageFormat::Markdown,
                source: wvc_side_panel_types::SidePanelPageSource::Managed,
...
fn test_side_panel_state_event_roundtrip() -> Result<()> {
    let event = ServerEvent::SidePanelState {
        snapshot: wvc_side_panel_types::SidePanelSnapshot {
            focused_page_id: Some("page-1".to_string()),
            pages: vec![wvc_side_panel_types::SidePanelPage {
                id: "page-1".to_string(),
                title: "Notes".to_string(),
