...
pub fn collect_wvc_session_locations(sessions_dir: &Path) -> Vec<SessionLocation> {
    use std::fs;
...
pub fn plan_launch_hotkeys_from_sessions(
    ...
    let locations = collect_wvc_session_locations(sessions_dir);
...
mod tests {
    ...
    fn collect_reads_working_dirs_and_skips_junk() {
        ...
        let mut locs = collect_wvc_session_locations(dir.path());
