...
mod tests {
    ...
    fn summarize_update_error_is_always_one_short_line() {
        let inputs = [
            "Update failed: Update check failed: error sending request for url (https://api.github.com/repos/nicolasramos-es/weavecoder/releases/latest)\n\nCaused by:\n    dns error: failed to lookup address information",
    ...
    fn asset_name_is_supported() {
        assert_ne!(get_asset_name(), "wvc-unknown");
    }
    ...
    fn sha256sums_accepts_standard_and_binary_lines() {
        ...
        .unwrap();
        assert_eq!(
            checksums.get("wvc-linux-x86_64").map(String::as_str),
            Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        );
        assert_eq!(
            checksums.get("wvc-macos-aarch64").map(String::as_str),
