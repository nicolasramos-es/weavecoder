...
impl GeminiProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(wvc_base::storage::app_config_dir()?.join("gemini_models_cache.json"))
    }
                ...
                if !produced_output {
                    ...
                    if abnormal {
                        ...
                        let detail = candidate
                            ...
                            .filter(|msg| !msg.trim().is_empty())
                            .map(|msg| {
                                format!(": {}", wvc_base::util::truncate_str(msg.trim(), 300))
                            })
                            .unwrap_or_default();
                        let _ = tx
                            .send(Err(anyhow::anyhow!(
...
fn gemini_http_client() -> reqwest::Client {
    reqwest::Client::builder()
        ...
        .build()
        .unwrap_or_else(|_| wvc_provider_core::shared_http_client())
}
...
fn is_transient_gemini_transport_error(err: &reqwest::Error) -> bool {
    ...
    err.is_connect()
        || err.is_timeout()
        || wvc_provider_core::is_transient_transport_error(&err.to_string())
}
