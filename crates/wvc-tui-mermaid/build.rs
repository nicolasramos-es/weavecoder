fn main() {
    ...
    // WVC_MMDR_SIZE_API_DISABLE=1 to force the legacy SVG-retarget path
    // (e.g. when testing against an older renderer via a Cargo patch).
    println!("cargo:rerun-if-env-changed=WVC_MMDR_SIZE_API_DISABLE");
    // Legacy opt-in env var kept for compatibility with old build scripts.
    println!("cargo:rerun-if-env-changed=WVC_MMDR_SIZE_API_AVAILABLE");
    if std::env::var_os("WVC_MMDR_SIZE_API_DISABLE").is_none() {
