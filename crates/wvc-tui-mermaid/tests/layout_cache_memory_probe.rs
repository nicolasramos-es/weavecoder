...
//! `cargo test -p wvc-tui-mermaid --test layout_cache_memory_probe -- --ignored --nocapture`
...
fn layout_cache_stays_bounded_under_50_distinct_renders() {
    ...
    wvc_tui_mermaid::reset_debug_stats();
    let stats_before = wvc_tui_mermaid::debug_stats();
    ...
    for idx in 0..RENDERS {
        ...
        match wvc_tui_mermaid::render_mermaid_untracked(&content, Some(90)) {
            wvc_tui_mermaid::RenderResult::Image { .. } => {}
            wvc_tui_mermaid::RenderResult::Error(error) => {
                render_errors += 1;
    ...
    let stats = wvc_tui_mermaid::debug_stats();
    let memory = wvc_tui_mermaid::debug_memory_profile();
