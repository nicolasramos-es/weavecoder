//! Stress probe: build the mermaid source that `wvc_plan::mermaid::
...
//!
//!   scripts/dev_cargo.sh run -p wvc-tui-mermaid --features renderer \
//!       --example swarm_plan_stress
//!
//! Cases:
//!   (a) real deep-mode plan fixture (examples/swarm_plan_fixture.json,
//!       snapshot of ~/.wvc/state/swarm/_home_jeremy_wvc__git.json)
...
use wvc_plan::mermaid::swarm_plan_mermaid;
...
fn probe(name: &str, src: &str) -> bool {
    match wvc_tui_mermaid::render_mermaid_untracked(src, Some(100)) {
        ...
        wvc_tui_mermaid::RenderResult::Error(err) => {
