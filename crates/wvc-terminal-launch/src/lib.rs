...
pub struct TerminalCommand {
    ...
    /// Exported as `WVC_SPAWN_KIND` to spawn hooks and spawned terminals.
    pub kind: Option<String>,
    /// The wvc session this terminal will run, when known.
    /// Exported as `WVC_SPAWN_SESSION_ID`.
    pub session_id: Option<String>,
    /// Extra metadata env entries (e.g. `WVC_SPAWN_SWARM_ID`) exported to
    /// spawn hooks and spawned terminals. Applied after the first-class
    /// `WVC_SPAWN_*` keys, so entries here win on key collisions.
    ...
    /// and are also exported under a `WVC_CLIENT_*` prefix so spawn/focus
...
///
/// The wvc server process is long-lived and captures these at *its* startup,
...
fn resume_terminal_candidates_with_client_env(
    ...
    // WVC_TERMINAL and configured spawn hooks still take precedence.
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let configured_terminal = std::env::var("WVC_TERMINAL").ok();
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let mut candidates = Vec::new();
    if let Ok(term) = std::env::var("WVC_TERMINAL") {
        push_unique_terminal(&mut candidates, term);
    }
...
/// The `WVC_SPAWN_*` metadata env exported to spawn hooks and to terminals
/// launched by the built-in fallback:
///
/// - `WVC_SPAWN_KIND`: why this spawn happened ("resume", "selfdev",
///   "swarm-agent", ...), when known.
/// - `WVC_SPAWN_SESSION_ID`: the wvc session the window will run.
/// - `WVC_SPAWN_TITLE`: the suggested window/tab title.
/// - `WVC_SPAWN_CWD`: the working directory for the session.
/// - `WVC_SPAWN_PROGRAM`: path of the wvc binary to execute.
/// - `WVC_SPAWN_COMMAND`: the full command line, shell-escaped, for hooks
///   (like tmux) that take a single shell-command string.
///
/// `TerminalCommand::extra_env` entries (e.g. `WVC_SPAWN_SWARM_ID`,
/// `WVC_SPAWN_COORDINATOR_SESSION_ID`) are appended last and win collisions.
fn spawn_metadata_env(command: &TerminalCommand, cwd: &Path) -> Vec<(String, String)> {
    let mut env: Vec<(String, String)> = Vec::new();
    if let Some(kind) = &command.kind {
        env.push(("WVC_SPAWN_KIND".to_string(), kind.clone()));
    }
    if let Some(session_id) = &command.session_id {
        env.push(("WVC_SPAWN_SESSION_ID".to_string(), session_id.clone()));
    }
    if let Some(title) = &command.title {
        env.push(("WVC_SPAWN_TITLE".to_string(), title.clone()));
    }
    env.push((
        "WVC_SPAWN_CWD".to_string(),
        cwd.to_string_lossy().into_owned(),
    ));
    env.push((
        "WVC_SPAWN_PROGRAM".to_string(),
        command.program.to_string_lossy().into_owned(),
    ));
    env.push((
        "WVC_SPAWN_COMMAND".to_string(),
    ...
    // `WVC_CLIENT_<NAME>` alias so hooks can explicitly distinguish the
    // client's terminal from the server's.
    for (key, value) in &command.client_terminal_env {
        env.push((key.clone(), value.clone()));
        env.push((format!("WVC_CLIENT_{key}"), value.clone()));
    }
...
/// convention), so `hook --flag` becomes `hook --flag <wvc> <args...>`.
/// The hook runs in the session working directory with the full
/// `WVC_SPAWN_*` metadata env set (see [`spawn_metadata_env`]); hooks that
/// need a single shell-command string (tmux, kitty `@ launch`) can use
/// `$WVC_SPAWN_COMMAND` instead of the appended argv.
...
fn build_spawn_command(term: &str, command: &TerminalCommand, cwd: &Path) -> Option<Command> {
    let title = command.title.as_deref().unwrap_or("wvc");
    ...
    match term {
        ...
        "herdr" => {
            ...
            cmd.current_dir(cwd)
                ...
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .args(["-c", script, "wvc-herdr-spawn", &herdr])
                .arg(cwd)
                .arg(shell);
            if command.fresh_spawn {
                cmd.env("WVC_FRESH_SPAWN", "1");
        ...
        "terminal" => {
            ...
            // means split/resume spawns silently fail to launch wvc. Use AppleScript's
    ...
    // not inherit this env, matching the existing WVC_FRESH_SPAWN caveat.
    for (key, value) in spawn_metadata_env(command, cwd) {
        cmd.env(key, value);
    }
...
mod tests {
    ...
    fn spawn_metadata_env_reexports_client_terminal_env_with_native_and_client_keys() {
        ...
        // inherited/stale value) and under a `WVC_CLIENT_*` alias so hooks can
        // distinguish the client's terminal from the server's.
        let command = TerminalCommand::new("/usr/local/bin/wvc", vec!["--resume".to_string()])
            .kind("swarm-agent")
            .session_id("ses_405")
        ...
        assert_eq!(
            lookup("WVC_CLIENT_ZELLIJ_SESSION_NAME").as_deref(),
            Some("sessionB")
        );
        assert_eq!(lookup("DISPLAY").as_deref(), Some(":1"));
        assert_eq!(lookup("WVC_CLIENT_DISPLAY").as_deref(), Some(":1"));
        // The first-class spawn metadata still flows through.
        assert_eq!(lookup("WVC_SPAWN_KIND").as_deref(), Some("swarm-agent"));
    }
    ...
    fn spawn_metadata_env_without_client_env_has_no_client_keys() {
        ...
        let env = spawn_metadata_env(&command, Path::new("/tmp/work"));
        assert!(
            !env.iter().any(|(k, _)| k.starts_with("WVC_CLIENT_")),
            "no client terminal env should produce no WVC_CLIENT_* keys"
        );
    }
    ...
    fn missing_tmux_binary_falls_back_to_detected_emulator() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous_terminal = std::env::var_os("WVC_TERMINAL");
        unsafe {
        ...
        match previous_terminal {
            Some(value) => unsafe { std::env::set_var("WVC_TERMINAL", value) },
            None => unsafe { std::env::remove_var("WVC_TERMINAL") },
        }
    ...
    fn herdr_spawn_splits_calling_pane_and_runs_resume_command() {
        ...
        assert_eq!(args[2], "wvc-herdr-spawn");
        assert_eq!(args[3], "/opt/herdr/bin/herdr");
        assert_eq!(args[4], "/work/a b");
        assert_eq!(args[5], "'/usr/local/bin/wvc' '--resume' 'ses herdr'");
    }
    ...
    fn herdr_spawn_adapter_executes_split_then_run_with_quoted_values() {
        ...
        assert_eq!(
            lines[1],
            "pane run w9:p12 '/opt/Weavecoder App/wvc' '--resume' 'session with spaces'"
        );
    ...
    fn shell_command_quotes_arguments() {
        let shell = shell_command(&["wvc".to_string(), "it's ok".to_string()]);
        #[cfg(unix)]
        assert_eq!(shell, "'wvc' 'it'\"'\"'s ok'");
    }
    ...
    #[test]
    #[cfg(not(unix))]
    fn windows_cmd_fallback_runs_wvc_under_cmd_k() {
        let command = TerminalCommand::new(
            std::path::PathBuf::from(r"C:\Program Files\wvc\wvc.exe"),
        ...
        let args: Vec<String> = cmd
            ...
            .map(|arg| arg.to_string_lossy().into_owned())
            .collect();
        assert_eq!(&args[..5], ["/C", "start", "wvc", "cmd.exe", "/K"]);
        assert!(args[5].contains(r#""C:\Program Files\wvc\wvc.exe""#));
        assert!(args[5].contains("self-dev"));
    }

    #[test]
    #[cfg(unix)]
    fn macos_terminal_inner_script_runs_wvc() {
        let command = TerminalCommand::new(
            std::path::PathBuf::from("/usr/local/bin/wvc"),
            vec!["--resume".to_string(), "abc-123".to_string()],
        );
        let script = macos_terminal_inner_script(&command, Path::new("/work/dir"));
        assert_eq!(
            script,
            "cd '/work/dir' && exec '/usr/local/bin/wvc' '--resume' 'abc-123'"
        );
        // Must actually exec wvc, not the broken `open -a Terminal <file>` form.
        assert!(script.contains("exec '/usr/local/bin/wvc'"));
    }
    ...
    fn macos_terminal_inner_script_injects_fresh_spawn() {
        ...
        assert_eq!(
            script,
            "cd '/tmp' && exec env WVC_FRESH_SPAWN=1 '/usr/local/bin/wvc'"
        );
    }
    ...
    fn macos_terminal_applescript_uses_do_script() {
        ...
        assert!(applescript.contains("'/usr/local/bin/wvc'"));
    }
    ...
    fn hook_spawn_command_appends_program_args_and_exports_metadata() {
        ...
        .kind("swarm-agent")
        .session_id("ses_abc")
        .spawn_env("WVC_SPAWN_SWARM_ID", "swarm-1")
        .fresh_spawn();
        ...
        assert_eq!(
            args,
            vec!["--flag", "/usr/local/bin/wvc", "--resume", "ses_abc"]
        );
        assert_eq!(
            cmd.get_current_dir(),
        ...
        assert_eq!(
            env_value(&cmd, "WVC_SPAWN_TITLE").as_deref(),
            Some("🦊 wvc ses_abc")
        );
        assert_eq!(
            env_value(&cmd, "WVC_SPAWN_CWD").as_deref(),
            Some("/work/dir")
        );
        assert_eq!(
            env_value(&cmd, "WVC_SPAWN_PROGRAM").as_deref(),
            Some("/usr/local/bin/wvc")
        );
        #[cfg(unix)]
        assert_eq!(
            env_value(&cmd, "WVC_SPAWN_COMMAND").as_deref(),
            Some("'/usr/local/bin/wvc' '--resume' 'ses_abc'")
        );
        assert_eq!(
            env_value(&cmd, "WVC_SPAWN_SWARM_ID").as_deref(),
            Some("swarm-1")
        );
        assert_eq!(env_value(&cmd, "WVC_FRESH_SPAWN").as_deref(), Some("1"));
    }
