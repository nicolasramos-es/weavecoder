...
//! coordinated build queue), the freshly built `target/selfdev/wvc` has no
...
//!
//! ```sh
//! cargo run -p wvc-build-support --example write_dev_sidecar
//! ```
fn main() -> anyhow::Result<()> {
    let repo = std::env::current_dir()?;
    let state = wvc_build_support::current_source_state(&repo)?;
    let path = wvc_build_support::write_current_dev_binary_source_metadata(&repo, &state)?;
