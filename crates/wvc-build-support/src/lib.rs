...
pub use paths::{
    ...
    find_repo_in_ancestors, get_repo_dir, is_wvc_repo, launcher_binary_path, launcher_dir,
...
pub fn smoke_test_server_binary(binary: &Path) -> Result<()> {
    ...
    let socket_path = temp.path().join("wvc-smoke.sock");
    let stderr_path = temp.path().join("wvc-smoke.stderr.log");
