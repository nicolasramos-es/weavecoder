...
pub fn selfdev_build_command_for_target(
    ...
    let specs = match target {
        SelfDevBuildTarget::Tui => vec![("wvc", "wvc")],
        SelfDevBuildTarget::Desktop2 => vec![("wvc-desktop2", "wvc-desktop2")],
        SelfDevBuildTarget::All | SelfDevBuildTarget::Auto => {
            vec![("wvc", "wvc"), ("wvc-desktop2", "wvc-desktop2")]
        }
    };
...
pub fn launcher_dir() -> Result<PathBuf> {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("wvc").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
mod tests {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        ...
        let crate_dir = repo.path().join("crates").join("wvc-build-support");
    ...
    fn build_targets_parse_from_their_names() {
        for (name, expected) in [
            ...
            ("wvc-desktop2", SelfDevBuildTarget::Desktop2),
    ...
    #[test]
    fn is_wvc_repo_accepts_git_file_for_worktree() {
    ...
    fn resolve_binary_payload_keeps_real_binary() {
        ...
        let temp = tempfile::TempDir::new().expect("temp install");
        let binary = temp.path().join("wvc");
        std::fs::write(&binary, vec![0x7fu8; 8192]).expect("binary");
        std::fs::write(temp.path().join("wvc-linux-x86_64.bin"), [0u8; 8]).expect("bin");
        assert_eq!(
    ...
    fn resolve_binary_payload_refuses_ambiguous_payloads() {
        ...
        std::fs::write(temp.path().join("wvc-macos-aarch64.bin"), [0u8; 8]).expect("second bin");
        assert_eq!(
            resolve_binary_payload(&wrapper),
