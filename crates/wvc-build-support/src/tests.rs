...
fn test_binary_version_hash_mismatch_rejects_publish_candidate() {
    ...
    let error = validate_binary_version_matches_source_report(&report, Path::new("wvc"), &source)
...
fn test_find_repo_in_ancestors_walks_upward() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo = temp.path().join("wvc-repo");
...
#[test]
fn launcher_dir_uses_sandbox_bin_when_wvc_home_is_set() {
    with_temp_wvc_home(|| {
        let launcher_dir = launcher_dir().expect("launcher dir");
        let expected = storage::wvc_dir().expect("wvc dir").join("bin");
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_wvc_home(|| {
        ...
        let expected_launcher = storage::wvc_dir()
            .expect("wvc dir")
            .join("bin")
            .join(binary_name());
