...
pub fn render_markdown_with_width(text: &str, max_width: Option<usize>) -> Vec<Line<'static>> {
    ...
    let text = wvc_render_core::normalize_latex_math(text);
    ...
    let mut table_alignments: Vec<wvc_render_core::Alignment> = Vec::new();
                ...
                if in_table {
                    match latex_mode {
                        ...
                        LatexRenderingMode::Unicode | LatexRenderingMode::Image => {
                            current_cell.push_str(&wvc_render_core::render_inline_latex(&math));
                        }
                    }
                } else {
                    ensure_blockquote_prefix(&mut current_spans, blockquote_depth);
            ...
            Event::Start(Tag::Table(ref aligns)) => {
                table_alignments = aligns
                    ...
                    .map(|align| match align {
                        pulldown_cmark::Alignment::Right => wvc_render_core::Alignment::Right,
                        pulldown_cmark::Alignment::Center => wvc_render_core::Alignment::Center,
                        _ => wvc_render_core::Alignment::Left,
                    })
                    .collect();
                dbg_tables += 1;
