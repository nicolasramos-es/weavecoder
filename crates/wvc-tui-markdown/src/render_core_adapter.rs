//! Adapter: backend-neutral [`wvc_render_core::Document`] -> ratatui lines.
//!
...
fn push_math_display(lines: &mut Vec<Line<'static>>, block: &wvc_render_core::Block) {
    let dim = Style::default().fg(md_dim_color());
...
pub fn render_markdown_via_core(text: &str) -> Vec<Line<'static>> {
    document_to_lines_with_width(&wvc_render_core::parse_markdown(text), None)
}
...
pub fn render_markdown_via_core_wrapped(text: &str, width: usize) -> Vec<Line<'static>> {
    let doc = wvc_render_core::parse_markdown(text);
