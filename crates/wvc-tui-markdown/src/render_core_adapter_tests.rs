...
// WVC_MD_FUZZ_ITERS env var (default 5000) so CI stays fast while local deep
...
fn fuzz_random_documents_parity() {
    let iters: u64 = std::env::var("WVC_MD_FUZZ_ITERS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(5000);
    let base_seed: u64 = std::env::var("WVC_MD_FUZZ_SEED")
        .ok()
        .and_then(|v| v.parse().ok())
