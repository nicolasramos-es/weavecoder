...
fn a_deferred_formula_resolves_into_an_image_once_the_worker_finishes() {
    ...
    let render = || {
        wvc_tui_mermaid::with_image_protocol_override(Some(true), || {
            wvc_tui_markdown::render_markdown_with_width(&text, Some(90))
        })
    };
    ...
    let pending = first
        .iter()
        .any(wvc_tui_markdown::line_is_mermaid_pending_placeholder);
    if !pending {
...
fn rendered_a_formula_image(lines: &[ratatui::text::Line<'static>]) -> bool {
    let has_placeholder = lines.iter().any(|line| {
        wvc_tui_mermaid::parse_image_placeholder(line).is_some()
            || wvc_tui_mermaid::parse_inline_image_placeholder(line).is_some()
    });
