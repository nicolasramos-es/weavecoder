...
//!
//! Run with:
//!   cargo test -p wvc-tui-markdown --test highlight_backend_probe -- --ignored --nocapture

use wvc_tui_markdown::highlight_line;
...
fn workload() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (
            "sh",
            "for pid in $(pgrep -f wvc); do awk '/VmRSS/{print $2}' /proc/$pid/status; done",
        ),
        (
            "toml",
