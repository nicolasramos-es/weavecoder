...
use wvc_base::auth::test_sandbox::AuthTestSandbox;
use wvc_base::protocol::{
    AuthChanged, AuthCredentialSource, AuthMethod, CatalogNamespace, RuntimeProviderKey,
};
use wvc_base::provider::ModelRoute;
use wvc_base::provider_catalog::OpenAiCompatibleProfile;
...
impl AuthLifecycleSpec {
    pub(crate) fn cerebras_fixture(auth_path: AuthLifecycleAuthPath) -> Self {
        let mut spec = Self::openai_compatible_fixture(
            wvc_base::provider_catalog::CEREBRAS_PROFILE,
            auth_path,
        );
...
impl AuthLifecycleDriver {
    ...
    pub(crate) fn run_openai_compatible_fixture(
        ...
        let auth = AuthChanged {
            provider: wvc_base::protocol::AuthProviderId::new(spec.provider_id),
    ...
    fn apply_credentials(
        &self,
        spec: &AuthLifecycleSpec,
        resolved: &wvc_base::provider_catalog::ResolvedOpenAiCompatibleProfile,
        ...
        match spec.auth_path {
            ...
            AuthLifecycleAuthPath::ProcessEnvPreseeded => {
                wvc_base::env::set_var(&resolved.api_key_env, &spec.api_key);
                wvc_base::auth::AuthStatus::invalidate_cache();
...
mod tests {
    ...
    fn is_native_routed_compat_profile(
        profile: &wvc_base::provider_catalog::OpenAiCompatibleProfile,
    ) -> bool {
        wvc_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id))
    ...
    struct LiveTestApiKey {
        secret: String,
        auth: wvc_base::live_tests::LiveVerificationAuth,
    }
    ...
    fn live_opencode_zen_api_key() -> Option<LiveTestApiKey> {
        ...
        let resolved = wvc_base::provider_catalog::resolve_openai_compatible_profile(
            wvc_base::provider_catalog::OPENCODE_PROFILE,
        );
        wvc_base::provider_catalog::load_api_key_from_env_or_config(
            &resolved.api_key_env,
            &resolved.env_file,
        )
        .map(|secret| LiveTestApiKey {
    ...
    fn live_event<I, S>(
        ...
        model: Option<&str>,
        capabilities: I,
        result: wvc_base::live_tests::LiveVerificationResult,
    ) -> wvc_base::live_tests::LiveVerificationEvent
    where
        I: IntoIterator<Item = S>,
        S: Into<String>,
    {
        let resolved = wvc_base::provider_catalog::resolve_openai_compatible_profile(profile);
        let mut event = wvc_base::live_tests::LiveVerificationEvent::new(
            test_name,
            resolved.id,
    ...
    fn covered_stage_names(stages: &[wvc_base::live_tests::LiveVerificationStage]) -> Vec<String> {
        stages
            .iter()
            .filter(|stage| {
                stage.status != wvc_base::live_tests::LiveVerificationStageStatus::NotRun
            })
    ...
    fn openai_compatible_provider_matrix_preserves_identity_catalog_and_picker() {
        ...
        for profile in wvc_base::provider_catalog::openai_compatible_profiles()
            .iter()
            .copied()
    ...
    fn provider_switch_reauth_matrix_recovers_from_stale_previous_provider_state() {
        ...
        let profiles: Vec<_> = wvc_base::provider_catalog::openai_compatible_profiles()
            .iter()
            .copied()
            .filter(|profile| !is_native_routed_compat_profile(profile))
            .collect();
    ...
    async fn cerebras_live_opt_in_catalog_lifecycle_uses_isolated_sandbox() {
        ...
        let models = match models_result {
            Ok(models) => models,
            Err(error) => {
                stages.push(wvc_base::live_tests::LiveVerificationStage::failed(
                    wvc_base::live_tests::checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    error.to_string(),
                ));
                ...
                let event = live_event(
                    ...
                    None,
                    capabilities,
                    wvc_base::live_tests::LiveVerificationResult::Failed,
                )
                .with_stages(stages);
        ...
        stages.push(
            ...
            .with_evidence(
                "models",
                wvc_base::live_tests::concise_model_sample(&models, 12),
            ),
        );
        let default_model = wvc_base::provider_catalog::CEREBRAS_PROFILE.default_model;
        ...
        stages.push(
            wvc_base::live_tests::LiveVerificationStage::passed(
                wvc_base::live_tests::checkpoints::AUTH_UX_KEY_ENTRY,
            )
        ...
        stages.push(
            wvc_base::live_tests::LiveVerificationStage::passed(
                wvc_base::live_tests::checkpoints::CREDENTIAL_PERSISTENCE,
            )
            .with_evidence(
                "credential_location",
        ...
        stages.push(
            wvc_base::live_tests::LiveVerificationStage::passed(
                wvc_base::live_tests::checkpoints::CATALOG_HOT_RELOAD_CURRENT_SESSION,
            )
        ...
        stages.push(
            wvc_base::live_tests::LiveVerificationStage::passed(
                wvc_base::live_tests::checkpoints::PICKER_LIVE_MODELS,
            )
        ...
        stages.push(
            wvc_base::live_tests::LiveVerificationStage::passed(
                wvc_base::live_tests::checkpoints::PICKER_FALLBACK_LABELING,
            )
        ...
        stages.push(
            wvc_base::live_tests::LiveVerificationStage::passed(
                wvc_base::live_tests::checkpoints::MODEL_SWITCH_ROUTE,
            )
            .with_evidence(
                "switch_request",
        ...
        if spend_smoke {
            ...
            .await
            {
                ...
                Err(error) => {
                    stages.push(wvc_base::live_tests::LiveVerificationStage::failed(
                        wvc_base::live_tests::checkpoints::NON_STREAMING_CHAT_COMPLETION,
                        error.to_string(),
                    ));
                    let capabilities = covered_stage_names(&stages);
                    let event = live_event(
        ...
        if stream_smoke {
            ...
            .await
            {
                ...
                Err(error) => {
                    stages.push(wvc_base::live_tests::LiveVerificationStage::failed(
                        wvc_base::live_tests::checkpoints::STREAMING_CHAT_COMPLETION,
                        error.to_string(),
                    ));
                    let capabilities = covered_stage_names(&stages);
                    let event = live_event(
        ...
        let event = live_event(
            ...
            Some(&selected),
            capabilities,
            wvc_base::live_tests::LiveVerificationResult::Passed,
        )
        .with_stages(stages);
        append_live_event(&event);
    }
    ...
    async fn opencode_zen_live_opt_in_tool_call_smoke() {
        ...
        stages.push(
            ...
            .with_evidence(
                "models",
                wvc_base::live_tests::concise_model_sample(&models, 16),
            )
            .with_evidence(
        ...
        .await
        {
            ...
            Err(error) => {
                stages.push(wvc_base::live_tests::LiveVerificationStage::failed(
                    wvc_base::live_tests::checkpoints::TOOL_CALL_PARSE,
                    error.to_string(),
                ));
                let capabilities = covered_stage_names(&stages);
                let event = live_event(
    ...
    async fn issue_driven_openai_compatible_live_target_smoke() {
        ...
        let profile = wvc_base::provider_catalog::openai_compatible_profile_by_id(&provider_id)
    ...
    fn fresh_start_sandbox_is_unconfigured_then_tui_key_lifecycle_configures_provider() {
        ...
        assert!(
            !wvc_base::provider_catalog::openai_compatible_profile_is_configured(spec.profile),
            "fresh sandbox should report the provider as unconfigured before setup"
        );
        wvc_base::auth::AuthStatus::invalidate_cache();
        assert_eq!(
            wvc_base::auth::AuthStatus::check_fast().state_for_provider(provider),
            wvc_base::auth::AuthState::NotConfigured
        );
        ...
        assert_eq!(
            wvc_base::auth::AuthStatus::check_fast().state_for_provider(provider),
            wvc_base::auth::AuthState::Available
        );
    }
}
