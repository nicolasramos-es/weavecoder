...
use wvc_base::message::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
use wvc_base::provider::Provider;
use wvc_base::provider_catalog::{OpenAiCompatibleProfile, ResolvedOpenAiCompatibleProfile};
...
pub async fn fetch_live_openai_compatible_models(
    ...
    let request = wvc_base::provider::shared_http_client().get(&url);
...
pub async fn run_live_openai_compatible_smoke(
    ...
    api_key: &str,
    model: &str,
) -> anyhow::Result<wvc_base::live_tests::LiveVerificationStage> {
    ...
    let request = wvc_base::provider::shared_http_client()
        .post(&url)
        .json(&body);
    let request = apply_provider_auth(request, &resolved, api_key);
    ...
    let mut stage = wvc_base::live_tests::LiveVerificationStage::passed(
...
mod tests {
    use super::*;
    use wvc_base::provider_catalog::resolve_openai_compatible_profile;
    use wvc_provider_metadata::{
...
pub async fn run_live_openai_compatible_tool_smoke(
    ...
    let tool_calls = parsed
        ...
        resolved.display_name,
        wvc_base::util::truncate_str(text.trim(), 1200)
    );
    ...
    let parsed_arguments = wvc_base::message::ToolCall::parse_streamed_input_to_object(arguments);
    ensure!(
...
pub async fn run_live_claude_native_smoke(
    ...
    .await?;
        ...
        wvc_base::util::truncate_str(outcome.text.trim(), 200),
        outcome.diagnostics()
    );
...
/// `tool_execution_loop`, `tool_result_followup`, and `real_wvc_tool_smoke`
...
pub async fn run_live_claude_native_tool_smoke(
    ...
    .await?;
        ...
        first.stop_reason,
        wvc_base::util::truncate_str(first.text.trim(), 200)
    );
    ...
    let parsed_arguments = wvc_base::message::ToolCall::parse_streamed_input_to_object(
    ...
    .await?;
        ...
        wvc_base::util::truncate_str(second.text.trim(), 200)
    );
...
/// `real_wvc_tool_smoke` checkpoints.
...
pub async fn run_live_native_provider_reasoning_smoke(
    ...
    let mut stage = wvc_base::live_tests::LiveVerificationStage::passed(
        wvc_base::live_tests::checkpoints::REASONING_CAPABILITY,
    )
    .with_duration_ms(started.elapsed().as_millis() as u64)
    .with_evidence("model", serde_json::json!(model))
...
fn parse_tool_arguments(input_json: &str) -> serde_json::Value {
    wvc_base::message::ToolCall::parse_streamed_input_to_object(if input_json.trim().is_empty() {
        "{}"
    } else {
        input_json.trim()
    })
