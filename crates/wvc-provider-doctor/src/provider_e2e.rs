...
use wvc_base::protocol::{AuthChanged, CatalogNamespace, RuntimeProviderKey};
...
fn tool_stage_detail(stage: &wvc_base::live_tests::LiveVerificationStage) -> String {
...
/// doctor report.
fn reasoning_stage_detail(stage: &wvc_base::live_tests::LiveVerificationStage) -> String {
    match stage
        .evidence
...
///
/// The predicate itself lives in `wvc_base::auth::doctor` so base-internal
...
pub use wvc_base::auth::doctor::native_doctor_supports_provider;
...
pub async fn run_claude_native_e2e(
    ...
    use wvc_provider_anthropic_runtime::AnthropicProvider;

    let normalized = wvc_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id))
        .unwrap_or("claude");
    let provider_label = wvc_base::auth::lifecycle::provider_display_label(Some(normalized))
    ...
    let catalog_models: Vec<String> = if tier.requires_api_key() {
        match provider_runtime.fetch_live_model_ids_for_doctor().await {
            ...
            Ok(_) => {
                ...
                let fallback = wvc_base::provider::known_anthropic_model_ids();
                checks.push(DoctorCheck::passed(
                    checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    label_for(checkpoints::MODEL_CATALOG_LIVE_ENDPOINT),
                    format!(
...
pub async fn run_antigravity_native_e2e(
    ...
    use wvc_base::provider::Provider;
    use wvc_provider_antigravity_runtime::AntigravityProvider;
    ...
    let _ = wvc_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id));
    let provider_label = wvc_base::auth::lifecycle::provider_display_label(Some("antigravity"))
...
impl NativeProviderKind {
    ...
    fn spec(self) -> NativeProviderSpec {
        match self {
            Self::OpenAi => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "wvc login --provider openai",
            },
            Self::Gemini => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "wvc login --provider gemini",
            },
            Self::Cursor => NativeProviderSpec {
                ...
                login_hint: "wvc login --provider cursor",
            },
            Self::Copilot => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "wvc login --provider copilot",
            },
            ...
            Self::Azure => NativeProviderSpec {
                ...
                login_hint: "wvc login --provider azure",
            },
        }
    }
    ...
    fn build_runtime(self) -> anyhow::Result<std::sync::Arc<dyn wvc_base::provider::Provider>> {
        ...
        let runtime: std::sync::Arc<dyn Provider> = match self {
            ...
            Self::Azure => {
                ...
                wvc_base::auth::azure::apply_runtime_env()
                    .context("apply Azure OpenAI runtime env")?;
                let runtime = wvc_provider_openrouter_runtime::OpenRouterProvider::new()
                ...
                if let Some(model) = wvc_base::auth::azure::load_model() {
    ...
    async fn resolve_credential(self) -> anyhow::Result<String> {
        ...
        match self {
            ...
            Self::Gemini => {
                let tokens = wvc_base::auth::gemini::load_or_refresh_tokens()
                    .await
            ...
            Self::Bedrock => {
                if !wvc_base::provider::bedrock::BedrockProvider::has_credentials() {
                    anyhow::bail!(
            ...
            Self::Azure => {
                ...
                Ok(format!(
                    ...
                    wvc_base::auth::azure::method_detail()
                ))
            }
        }
    }
...
struct NativeProviderSpec {
    ...
    /// `wvc login` hint surfaced when the credential cannot be resolved.
...
async fn run_generic_native_api_checks(
    provider: &dyn wvc_base::provider::Provider,
    selected: &str,
    label: &str,
    checks: &mut Vec<DoctorCheck>,
    spend: &mut DoctorSpend,
...
fn wiring_contract(profile: OpenAiCompatibleProfile) -> WiringContract {
    match wvc_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id)) {
...
fn run_wiring_checks_for_contract(
    ...
    let auth = AuthChanged {
        provider: wvc_base::protocol::AuthProviderId::new(auth_provider_id),
...
mod tests {
    ...
    /// `native_doctor_supports_provider` lives in `wvc_base::auth::doctor`
