...
impl Default for LaunchOptions {
    fn default() -> Self {
        Self {
            ...
            client_name: concat!("wvc-sdk-rs/", env!("CARGO_PKG_VERSION")).to_string(),
...
impl LaunchedInstance {
    fn shutdown(&self) {
        ...
        stop_instance_daemon(&self.wvc_home, &self.runtime_dir);
        ...
        if self.ephemeral {
            remove_ephemeral_home(&self.wvc_home, self.cleanup_timeout);
        }
    }
}
...
pub fn launch_instance(options: &LaunchOptions) -> Result<LaunchedInstance> {
    let ephemeral = options.wvc_home.is_none();
    let wvc_home = match &options.wvc_home {
        Some(path) => {
            fs::create_dir_all(path).map_err(launch_io)?;
            path.clone()
        }
        None => tempfile::Builder::new()
            .prefix("wvc-sdk-instance-")
            .tempdir()
            .map_err(launch_io)?
            .keep(),
    };
    set_owner_only_dir(&wvc_home)?;

    let runtime_dir = wvc_home.join("run");
    fs::create_dir_all(&runtime_dir).map_err(launch_io)?;
    set_owner_only_dir(&runtime_dir)?;
    let socket_path = runtime_dir.join("wvc-api.sock");
    for stale in [
        "wvc-api.sock",
        "wvc.sock",
        "wvc-debug.sock",
    ...
    let binary = options
        .binary
        .clone()
        .unwrap_or_else(|| PathBuf::from("wvc"));
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                return Err(Error::new(
                    ...
                    format!(
                        "wvc exited during startup ({status}){}",
    ...
    Err(Error::new(
        ...
        format!(
            "wvc did not create its API socket at {} within {:?}{}",
            socket_path.display(),
...
pub fn inherit_credentials(from_home: &Path, to_home: &Path) -> Result<Vec<PathBuf>> {
    ...
    if let Ok(entries) = fs::read_dir(user_app_config_dir()) {
        for entry in entries.flatten() {
            ...
            let relative = PathBuf::from("config").join("wvc").join(&name);
...
pub fn ensure_runtime(options: &LaunchOptions, progress: &Progress<'_>) -> Result<()> {
    let api = options
        .api_socket
        .clone()
        .unwrap_or_else(wvc_harness_api::api_socket_path);
...
fn read_daemon_pid(home: &Path, runtime_dir: &Path) -> Option<i32> {
    ...
    let socket = runtime_dir.join("wvc.sock");
...
fn remove_ephemeral_home(home: &Path, timeout: Duration) {
    ...
    if home.parent() != Some(std::env::temp_dir().as_path())
        || !name.starts_with("wvc-sdk-instance-")
    {
        return;
    }
    let deadline = Instant::now() + timeout;
...
mod tests {
    ...
    fn dropping_an_ephemeral_instance_stops_its_bridge_and_removes_its_home() {
        ...
        let instance = LaunchedInstance {
            socket_path: runtime_dir.join("wvc-api.sock"),
            wvc_home: home.clone(),
            runtime_dir,
            child: Mutex::new(Some(child)),
            ephemeral: true,
