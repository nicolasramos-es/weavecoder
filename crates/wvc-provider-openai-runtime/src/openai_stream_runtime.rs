...
pub(super) async fn stream_response(
    ...
    if !response.status().is_success() {
        ...
        {
            wvc_base::provider::record_model_unavailable_for_account(model_name, &reason);
            wvc_base::logging::warn(&format!(
        ...
        return Err(OpenAIStreamFailure::Other(
            wvc_provider_core::retry_after::error_with_retry_after(msg, retry_after),
        ));
    }
...
pub(super) async fn try_persistent_ws_continuation(
    ...
    if state.connected_at.elapsed() >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS) {
        wvc_base::logging::info("Persistent WS connection too old; forcing reconnect");
        *guard = None;
        log_openai_stream_lifecycle(
            wvc_base::logging::LogLevel::Info,
    ...
    if incremental_items.is_empty() {
        wvc_base::logging::info("No incremental items to send; need fresh request");
        *guard = None;
        log_openai_stream_lifecycle(
            wvc_base::logging::LogLevel::Info,
            "persistent_state_reset",
    ...
    let continuation_payload = serde_json::json!({
        ...
        "previous_response_id_hash": wvc_provider_core::fingerprint::stable_hash_str(&previous_response_id),
    ...
    emit_connection_phase(tx, wvc_message_types::ConnectionPhase::SendingRequest).await;
    if let Err(e) = state.ws_stream.send(WsMessage::Text(request_text)).await {
        return PersistentWsResult::Failed(format!("send error: {}", e));
    }
    emit_connection_phase(tx, wvc_message_types::ConnectionPhase::WaitingForResponse).await;
    ...
    if let Some(resp_id) = new_response_id {
        ...
        wvc_base::logging::warn("Persistent WS: no response_id in response; breaking chain");
        *guard = None;
        log_openai_stream_lifecycle(
            wvc_base::logging::LogLevel::Warn,
            "persistent_state_reset",
...
fn maybe_record_runtime_model_unavailable_from_stream_error(model: &str, message: &str) {
    ...
    if let Some(reason) = reason {
        wvc_base::provider::record_model_unavailable_for_account(model, &reason);
        wvc_base::logging::warn(&format!(
