...
fn catalog_error_is_auth_rejection(err: &anyhow::Error) -> bool {
    err.downcast_ref::<wvc_base::provider::ModelCatalogHttpStatus>()
...
impl Provider for OpenAIProvider {
    ...
    fn set_credential_mode(&self, mode: wvc_provider_core::CredentialMode) -> anyhow::Result<()> {
    ...
    async fn complete(
        ...
        let prompt_cache_key_hash = request
            ...
            .map(wvc_provider_core::fingerprint::stable_hash_json);
        ...
        let usage_snapshot = wvc_base::usage::get_openai_usage_sync();
        log_openai_stream_lifecycle(
            wvc_base::logging::LogLevel::Info,
            "request_start",
            vec![
                ...
                if use_websocket_transport {
                    ...
                    match continuation_result {
                        ...
                        PersistentWsResult::Failed(err) => {
                            log_openai_stream_lifecycle(
                                wvc_base::logging::LogLevel::Warn,
                ...
                for attempt in 0..MAX_RETRIES {
                    if attempt > 0 {
                        emit_connection_phase(
                            &tx,
                            wvc_message_types::ConnectionPhase::Retrying {
                                attempt: attempt + 1,
                                max: MAX_RETRIES,
                            },
                        )
                    ...
                    match result {
                        ...
                        Err(OpenAIStreamFailure::Other(error)) => {
                            ...
                            log_openai_stream_lifecycle(
                                wvc_base::logging::LogLevel::Error,
                                "attempt_failed",
                                vec![
                                    ("model", model_for_transport.clone()),
                                    ("attempt", (attempt + 1).to_string()),
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        if !is_chatgpt_web_model(model)
            && !wvc_base::provider::known_openai_model_ids()
                .iter()
        ...
        if wvc_provider_core::is_openai_api_only_pro_model(model) {
        ...
        if let Ok(mut current) = self.model.try_write() {
            ...
            *current = model.to_string();
            wvc_base::provider::clear_model_unavailable_for_account(model);
            drop(current);
            if changed {
    ...
    fn available_models(&self) -> Vec<&'static str> {
        ...
        wvc_provider_core::ALL_OPENAI_MODELS.to_vec()
    }
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        ...
        let mut models =
            wvc_base::provider::cached_openai_model_ids().unwrap_or_else(|| vec![self.model()]);
        ...
        if wvc_base::provider::openai_platform_api_key_configured() {
            for pro in wvc_provider_core::OPENAI_API_ONLY_PRO_MODELS {
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let account_label = wvc_base::auth::codex::active_account_label();
        ...
        let catalog = if is_chatgpt_mode {
            ...
            match wvc_base::provider::fetch_openai_model_catalog(&access_token).await {
                Ok(catalog) => catalog,
                ...
                Err(err) if catalog_error_is_auth_rejection(&err) => {
                    ...
                    wvc_base::provider::fetch_openai_model_catalog(&refreshed).await?
                }
                Err(err) => return Err(err),
            }
        } else {
            wvc_base::provider::fetch_openai_api_key_model_catalog(&access_token).await?
        };
        ...
        if current_credential_identity != credential_identity
            || wvc_base::auth::codex::active_account_label() != account_label
        {
            wvc_base::logging::info(
        ...
        self.revalidate_reasoning_effort();
        wvc_base::provider::persist_openai_model_catalog(&catalog);
        ...
        if !catalog.available_models.is_empty() {
            wvc_base::provider::populate_account_models(catalog.available_models);
        }
        Ok(())
    }
    ...
    fn set_reasoning_effort(&self, effort: &str) -> Result<()> {
        ...
        if !requested.is_empty()
            && wvc_provider_core::canonical_reasoning_effort(&requested).is_none()
            && !wvc_base::prompt::is_swarm_effort(&requested)
        {
            anyhow::bail!(
            ...
            && !wvc_base::prompt::is_swarm_effort(requested)
        {
            ...
            if !available.contains(&requested) {
                anyhow::bail!(
                    ...
                    available
                        .into_iter()
                        .filter(|effort| !wvc_base::prompt::is_swarm_effort(effort))
                        .collect::<Vec<_>>()
                        .join(", ")
                );
            }
    ...
    fn available_efforts(&self) -> Vec<&'static str> {
        let model = wvc_provider_core::model_id::canonical(&self.model());
        ...
        if let Some(advertised) = advertised {
            let mut efforts: Vec<&'static str> = advertised
                .iter()
                .filter_map(|effort| wvc_provider_core::canonical_reasoning_effort(effort))
                .collect();
            efforts.extend(["swarm", "swarm-deep"]);
            return efforts;
        }
        wvc_provider_core::OPENAI_SELECTABLE_EFFORTS.to_vec()
    }
    ...
    async fn native_compact(
    ...
    ) -> Result<wvc_provider_core::NativeCompactionResult> {
        ...
        if let Some(encrypted_content) = existing_openai_encrypted_content {
            if !wvc_base::provider::openai_request::openai_encrypted_content_is_sendable(
                encrypted_content,
            ) {
                anyhow::bail!(
                    "OpenAI native compaction payload is too large to replay ({} chars > safe limit {} chars)",
                    encrypted_content.len(),
                    wvc_base::provider::openai_request::OPENAI_ENCRYPTED_CONTENT_SAFE_MAX_CHARS,
                );
            }
        ...
        if !response.status().is_success() {
            ...
            let body = wvc_base::util::http_error_body(response, "HTTP error").await;
        ...
        Ok(wvc_provider_core::NativeCompactionResult {
            summary_text: None,
    ...
    fn context_window(&self) -> usize {
        ...
        wvc_provider_core::context_limit_for_model_with_provider(&model, Some(self.name()))
            .unwrap_or(wvc_provider_core::DEFAULT_CONTEXT_LIMIT)
    }
