...
fn test_build_responses_input_replaces_oversized_native_compaction_with_text() {
    let oversized =
        "x".repeat(wvc_base::provider::openai_request::OPENAI_ENCRYPTED_CONTENT_SAFE_MAX_CHARS + 1);
...
fn test_openai_cache_ttl_is_model_aware() {
    assert_eq!(
        wvc_base::provider::cache_ttl_for_provider_model("openai", Some("gpt-5.6-sol")),
        Some(24 * 60 * 60)
    );
    assert_eq!(
        wvc_base::provider::cache_ttl_for_provider_model("openai", Some("gpt-5.5")),
        Some(24 * 60 * 60)
    );
    assert_eq!(
        wvc_base::provider::cache_ttl_for_provider_model("openai", Some("gpt-4o")),
        Some(300)
    );
}
