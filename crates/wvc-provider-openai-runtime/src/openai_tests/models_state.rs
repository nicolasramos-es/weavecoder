...
fn test_openai_supports_codex_models() {
    ...
    wvc_base::auth::codex::set_active_account_override(Some(
        "openai-supports-codex-models".to_string(),
    ));
    wvc_base::provider::populate_account_models(vec![
    ...
    wvc_base::auth::codex::set_active_account_override(None);
}
...
fn test_openai_switching_models_include_dynamic_catalog_entries() {
    ...
    wvc_base::auth::codex::set_active_account_override(Some("switching-test".to_string()));
    wvc_base::provider::populate_account_models(vec![
        "gpt-5.4".to_string(),
...
fn test_chatgpt_browser_only_runtime_rejects_api_models_and_uses_local_compaction() {
    ...
    assert!(provider.uses_wvc_compaction());
...
async fn test_set_model_clears_persistent_ws_state() {
    ...
    wvc_base::provider::populate_account_models(vec!["gpt-5.3-codex".to_string()]);
