...
async fn live_openai_catalog_lists_gpt_5_4_family() -> Result<()> {
    ...
    wvc_base::provider::populate_account_models(catalog.available_models.clone());
...
async fn live_openai_gpt_5_4_and_fast_requests_succeed() -> Result<()> {
    ...
    let Some(plain_response) = live_openai_smoke("gpt-5.4", "WVC_GPT54_OK").await? else {
        eprintln!("skipping live OpenAI response test: no real OAuth credentials");
        return Ok(());
    };
    assert!(
        plain_response.contains("WVC_GPT54_OK"),
        ...
        .any(|model| model == "gpt-5.3-codex-spark")
    {
        let Some(fast_response) =
            live_openai_smoke("gpt-5.3-codex-spark", "WVC_GPT53_SPARK_OK").await?
        else {
            eprintln!("skipping live OpenAI fast-model test: no real OAuth credentials");
            return Ok(());
        };
        assert!(
            fast_response.contains("WVC_GPT53_SPARK_OK"),
            "unexpected gpt-5.3-codex-spark response: {}",
            fast_response
        );
    }

    if wvc_base::provider::known_openai_model_ids()
        .iter()
        .any(|model| model == "gpt-5.4[1m]")
    {
        let Some(long_context_response) =
            live_openai_smoke("gpt-5.4[1m]", "WVC_GPT54_1M_OK").await?
        else {
            eprintln!("skipping live OpenAI 1m test: no real OAuth credentials");
            return Ok(());
        };
        assert!(
            long_context_response.contains("WVC_GPT54_1M_OK"),
...
fn test_websocket_completion_timeout_is_long_enough_for_reasoning() {
    let timeout = std::hint::black_box(wvc_provider_openai::websocket_health::WEBSOCKET_COMPLETION_TIMEOUT_SECS);
    assert!(
        timeout >= 120,
...
fn test_websocket_next_activity_timeout_resets_after_api_activity() {
    ...
    assert!(
        remaining >= wvc_provider_openai::websocket_health::WEBSOCKET_COMPLETION_TIMEOUT_SECS.saturating_sub(3),
