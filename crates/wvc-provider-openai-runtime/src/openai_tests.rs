...
impl LiveOpenAITestEnv {
    fn new() -> Result<Option<Self>> {
        let lock = wvc_base::storage::lock_test_env();
        let Some(source_auth) = real_codex_auth_path() else {
            return Ok(None);
        };

        let temp = tempfile::Builder::new()
            .prefix("wvc-openai-live-")
            .tempdir()?;
...
async fn live_openai_catalog() -> Result<Option<wvc_base::provider::OpenAIModelCatalog>> {
    let Some(_env) = LiveOpenAITestEnv::new()? else {
        return Ok(None);
    };
    let creds = wvc_base::auth::codex::load_credentials()?;
    ...
    Ok(Some(
        wvc_base::provider::fetch_openai_model_catalog(&token).await?,
    ))
}
