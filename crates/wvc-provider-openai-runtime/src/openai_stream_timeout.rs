...
pub(crate) fn effective_https_idle_timeout(request: &Value) -> std::time::Duration {
    wvc_base::provider::stream_idle_timeout_for_effort(request_reasoning_effort(request))
}
...
pub(crate) fn effective_ws_completion_timeout_secs(request: &Value) -> u64 {
    let multiplier = u64::from(
        wvc_base::provider::stream_idle_timeout_multiplier_for_effort(request_reasoning_effort(
            request,
        )),
    );
    wvc_provider_openai::websocket_health::WEBSOCKET_COMPLETION_TIMEOUT_SECS
        .max(wvc_base::provider::stream_idle_timeout().as_secs())
