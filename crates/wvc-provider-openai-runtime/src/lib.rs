...
//!
//! Model-catalog/account-availability state stays in `wvc_base::provider`
//! (it is shared vocabulary for routing), as does the pure request shaping in
//! `wvc_base::provider::openai_request`.
...
use wvc_base::provider::openai_request::{build_responses_input, build_tools};
#[cfg(test)]
use wvc_message_types::TOOL_OUTPUT_MISSING_TEXT;
use wvc_message_types::{Message as ChatMessage, StreamEvent, ToolDefinition};
...
const DEFAULT_MODEL: &str = wvc_provider_core::DEFAULT_OPENAI_MODEL;
const ORIGINATOR: &str = "codex_cli_rs";

pub(crate) const CHATGPT_WEB_MODEL: &str = wvc_provider_core::CHATGPT_WEB_MODEL;
...
/// Shared dual-auth credential pin (see `wvc_provider_core::CredentialMode`).
/// The OpenAI-specific alias is kept so existing call sites read naturally.
pub(crate) use wvc_provider_core::CredentialMode as OpenAICredentialMode;
...
pub(crate) fn load_credentials_for_mode(mode: OpenAICredentialMode) -> Result<CodexCredentials> {
    match mode {
        OpenAICredentialMode::Auto => wvc_base::auth::codex::load_credentials(),
        OpenAICredentialMode::OAuth => wvc_base::auth::codex::load_oauth_credentials(),
        OpenAICredentialMode::ApiKey => wvc_base::auth::codex::load_api_key_credentials(),
    }
}
...
fn log_openai_stream_lifecycle(
    level: wvc_base::logging::LogLevel,
    phase: &str,
    ...
    wvc_base::logging::event(level, "PROVIDER_STREAM_LIFECYCLE", owned);
}
...
impl OpenAIProvider {
    pub(crate) fn supports_extended_prompt_cache_retention(model_id: &str) -> bool {
        wvc_base::provider::openai::supports_extended_prompt_cache_retention(model_id)
    }
    ...
    fn new_inner(credentials: CodexCredentials, browser_only: bool) -> Self {
        ...
        let native_compaction_threshold_tokens = wvc_base::config::config()
            .provider
            .openai_native_compaction_threshold_tokens
            .max(1000);
        let model_reasoning_efforts =
            wvc_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    fn reload_cached_reasoning_efforts(&self) {
        let cached = wvc_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    async fn clear_persistent_ws(&self, reason: &str) {
        ...
        if persistent_ws.is_some() {
            wvc_base::logging::info(&format!("Clearing persistent OpenAI WS state: {}", reason));
        }
        *persistent_ws = None;
    }
    ...
    fn default_reasoning_effort_for_model(model: &str) -> Option<String> {
        let key = wvc_provider_core::model_id::canonical(model);
    ...
    fn revalidate_reasoning_effort(&self) {
        ...
        if wvc_base::prompt::is_swarm_effort(&current)
    ...
    fn api_reasoning_effort(&self, effort: Option<&str>) -> Option<String> {
        match effort {
            Some(e) if wvc_base::prompt::is_swarm_effort(e) => {
                let available = self.available_efforts();
                wvc_provider_core::OPENAI_SELECTABLE_EFFORTS
                    .iter()
                    .rev()
                    .find(|candidate| {
                        !wvc_base::prompt::is_swarm_effort(candidate)
    ...
    fn load_max_output_tokens() -> Option<u32> {
        ...
        if raw.is_some() {
            match parsed {
                Some(value) => wvc_base::logging::info(&format!(
    ...
    pub(crate) fn resolve_api_base() -> String {
        wvc_base::provider::openai::resolve_api_base()
    }
    ...
    async fn model_id(&self) -> String {
        ...
        let availability = wvc_base::provider::model_availability_for_account(&current);
        ...
        match availability.state {
            wvc_base::provider::AccountModelAvailabilityState::Unavailable => {
                ...
                if let Some(fallback) = wvc_base::provider::get_best_available_openai_model()
                    && fallback != current
                {
                    wvc_base::logging::info(&format!(
            ...
            wvc_base::provider::AccountModelAvailabilityState::Unknown => {
                if wvc_base::provider::should_refresh_openai_model_catalog()
                    && wvc_base::provider::begin_openai_model_catalog_refresh()
                {
                    let is_chatgpt_mode = {
                    ...
                    wvc_base::provider::refresh_openai_model_catalog_in_background(
                        token,
                        is_chatgpt_mode,
            ...
            wvc_base::provider::AccountModelAvailabilityState::Available => {}
        }
