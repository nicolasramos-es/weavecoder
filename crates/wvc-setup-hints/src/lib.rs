...
use macos_terminal::{
    ...
    launch_command_for_macos_terminal, paused_wvc_shell_command, save_preferred_macos_terminal,
};
#[cfg(windows)]
use windows_setup::{
...
impl SetupHintsState {
    fn path() -> Result<PathBuf> {
        Ok(storage::wvc_dir()?.join("setup_hints.json"))
    }
...
fn load_launch_hotkeys_config() -> wvc_config_types::LaunchHotkeysConfig {
    #[derive(serde::Deserialize, Default)]
    struct Wrapper {
        #[serde(default)]
        launch_hotkeys: wvc_config_types::LaunchHotkeysConfig,
    }
    let Ok(dir) = storage::wvc_dir() else {
...
pub fn record_launch_dirs(dir: &std::path::Path, repo_dir: Option<&std::path::Path>) {
    #[cfg(any(target_os = "macos", target_os = "linux", windows))]
    {
        if let Err(err) = record_launch_dirs_inner(dir, repo_dir) {
            wvc_logging::warn(&format!("failed to record launch dirs for hotkeys: {err}"));
        }
    }
...
#[cfg(target_os = "macos")]
pub fn launch_wvc_in_macos_terminal(extra_args: &[String]) -> Result<()> {
    let terminal = effective_macos_terminal();
    let exe = std::env::current_exe()?;
    let exe_path = exe.to_string_lossy().into_owned();
    let shell_command = macos_terminal::paused_wvc_shell_command_with_args(&exe_path, extra_args);
    ...
    let command = match macos_terminal::no_automation_launch(terminal, &shell_command) {
        ...
        macos_terminal::NoAutomationLaunch::CommandFile { app } => {
            let dir = storage::wvc_dir()?.join("launcher");
...
fn run_macos_hotkey_listener() -> Result<()> {
    ...
    use wvc_terminal_launch::{TerminalCommand, spawn_command_in_new_terminal_with};
            ...
            && state.launch_hotkey_tracking_version < LAUNCH_HOTKEY_TRACKING_VERSION
        {
            match windows_setup::refresh_windows_launch_hotkeys() {
                Ok(()) => {
                    state.launch_hotkey_tracking_version = LAUNCH_HOTKEY_TRACKING_VERSION;
                    let _ = state.save();
                    wvc_logging::info("Migrated Windows launch hotkeys to usage tracking");
                }
                Err(err) => wvc_logging::warn(&format!(
...
fn linux_hotkeys_installed(comp: linux_env::LinuxCompositor) -> bool {
    ...
    match comp {
        ...
        LinuxCompositor::Mate => {
            dconf_list("/org/mate/desktop/keybindings/").contains("wvc-launch-")
        }
...
fn install_xfce_launch_hotkeys() -> Result<bool> {
    ...
    for line in existing.lines() {
        ...
        if value.trim().contains("/launch_wvc_") && !wanted.iter().any(|(p, _)| p == prop) {
...
fn reload_compositor_config(comp: linux_env::LinuxCompositor) {
        ...
        .status()
    {
        ...
        Ok(status) => wvc_logging::warn(&format!(
            "{} exited with {status} while reloading hotkey binds",
            cmd[0]
        )),
        Err(err) => wvc_logging::warn(&format!("failed to run {} reload: {err}", cmd[0])),
    }
}
...
pub fn maybe_show_keymap_conflict_hint(
    keybindings: &wvc_config_types::KeybindingsConfig,
) -> Option<StartupHints> {
    if !io::stdin().is_terminal() || !io::stderr().is_terminal() {
        return None;
    }
...
fn create_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    #[cfg(any(test, target_os = "macos"))]
    {
        ...
        wvc_logging::info(&format!("Created macOS app bundle: {}", app_dir.display()));
    }
...
pub fn reinstall_launch_hotkeys_after_config_change() {
    #[cfg(target_os = "macos")]
    {
        ...
        match install_macos_hotkey_listener(preferred) {
            Ok(terminal) => wvc_logging::info(&format!(
                "Reinstalled launch hotkeys after config change for {}",
                terminal.label()
            )),
            Err(err) => wvc_logging::warn(&format!("failed to reinstall launch hotkeys: {err}")),
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match install_linux_launch_hotkeys(comp) {
            Ok(true) => wvc_logging::info(&format!(
