...
//! low-level keyboard hook in the first-party Rust listener (`wvc setup-hotkey
...
/// Returns `None` for keys with no stable VK code. wvc `cmd` maps to Alt
...
/// Translate a canonical wvc key token into a Win32 virtual-key code.
...
/// User-facing chord rendering for Windows notices: wvc's `cmd` modifier
...
fn ps_dir_expr(dir: &str, last_dir_file: &str, last_repo_file: &str) -> String {
    match dir {
        "$HOME" => "(Resolve-WeavecoderDir $null)".to_string(),
        "$LAST_DIR" => format!("(Resolve-WeavecoderDir {})", ps_quote(last_dir_file)),
        "$LAST_REPO" => format!("(Resolve-WeavecoderDir {})", ps_quote(last_repo_file)),
        path => format!("(Resolve-WeavecoderFixedDir {})", ps_quote(path)),
    }
}

/// Render the full hotkey-listener PowerShell script.
///
/// `launch_exe`/`launch_args` describe the terminal command that opens wvc
/// (e.g. `wt.exe` + profile args, or `alacritty -e wvc`); `{DIR}` never
...
pub(crate) fn render_windows_listener_ps1(
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        registrations.push_str(&format!(
            "Register-WeavecoderHotkey -Id 0x{id:X} -Mods 0x{mods:X} -Vk 0x{vk:X} -Label {label}\n",
    ...
    Some(format!(
        r#"# wvc global launch hotkey listener
# Auto-generated by wvc setup-hotkey from [launch_hotkeys] config. Runs at
# login via a startup shortcut. Do not edit; re-run `wvc setup-hotkey`.
...
function Register-WeavecoderHotkey {{
    ...
    if ([HotKeyHelper]::RegisterHotKey([IntPtr]::Zero, $Id, $Mods -bor $MOD_NOREPEAT, $Vk)) {{
        ...
        Write-Warning "wvc: failed to register hotkey $Label (already claimed?)"
    }}
}}

# Resolve a dynamic launch dir from a tracking file, falling back to $HOME.
function Resolve-WeavecoderDir {{
    param([string]$File)
    if ($File) {{
        try {{
...
function Resolve-WeavecoderFixedDir {{
    param([string]$Dir)
    if ($Dir -and (Test-Path -LiteralPath $Dir -PathType Container)) {{ return $Dir }}
    return $env:USERPROFILE
}}
...
if ($script:RegisteredIds.Count -eq 0) {{
    Write-Error "wvc: no launch hotkeys could be registered"
    exit 1
}}
...
mod tests {
    ...
    fn args_for(entry: &WindowsHotkey) -> String {
        ...
        if entry.self_dev {
            format!(r#"-e "C:\wvc.exe" --spawn-hotkey "{hotkey}" self-dev"#)
        } else {
            format!(r#"-e "C:\wvc.exe" --spawn-hotkey "{hotkey}""#)
        }
    }
    ...
    fn listener_script_registers_each_entry_and_dispatches_dirs() {
        let entries = vec![
            hk("win+shift+f23", "C:\\Users\\u\\wvc", "wvc", false),
        ...
        let script = render_windows_listener_ps1(
            ...
            "wt.exe",
            args_for,
            "C:\\Users\\u\\.wvc\\hotkey\\last_dir",
            "C:\\Users\\u\\.wvc\\hotkey\\last_repo",
        )
        .unwrap();

        // Three registrations with distinct ids.
        assert_eq!(script.matches("Register-WeavecoderHotkey").count(), 3 + 1); // 3 calls + fn def
        ...
        assert!(script.contains("Resolve-WeavecoderFixedDir 'C:\\Users\\u\\wvc'"));
        assert!(script.contains("Resolve-WeavecoderDir $null"));
        assert!(script.contains("Resolve-WeavecoderDir 'C:\\Users\\u\\.wvc\\hotkey\\last_repo'"));
