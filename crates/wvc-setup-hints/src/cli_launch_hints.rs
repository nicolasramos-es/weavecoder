...
pub(super) fn maybe_notify(source: &str) -> Result<()> {
    ...
    *shown = shown.saturating_add(1);
    ...
    send_desktop_notification("Weavecoder shortcut", &body);
    Ok(())
}
...
fn install_hook(path: &Path, source: CliSource) -> Result<()> {
    ...
    bytes.push(b'\n');
    wvc_storage::write_bytes(path, &bytes)
...
fn hook_command(source: CliSource) -> Result<String> {
    let executable = trusted_wvc_executable()?;
    Ok(format!(
        "{} {HOOK_COMMAND_MARKER}{}",
...
fn trusted_wvc_executable() -> Result<PathBuf> {
    ...
    #[cfg(not(windows))]
    {
        if let Some(home) = dirs::home_dir() {
            let launcher = home.join(".local").join("bin").join("wvc");
            if launcher.is_file() {
                return Ok(launcher);
            }
        }
            ...
            "hooks": {
                "SessionStart": [{
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": "wvc setup-hotkey --notify-cli-launch old",
                            "timeout": 30
                        },
...
"#,
        ...
        assert!(!command.starts_with("wvc "));
