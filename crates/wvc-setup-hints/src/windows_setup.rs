...
fn resolve_windows_hotkeys() -> Vec<WindowsHotkey> {
    ...
    let exe_path = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "wvc".to_string());
...
fn startup_shortcut_path() -> PathBuf {
    startup_dir().join("wvc-hotkey.lnk")
}

fn hotkey_vbs_path() -> Result<PathBuf> {
    Ok(storage::wvc_dir()?
        .join("hotkey")
        .join("wvc-hotkey-launcher.vbs"))
}
...
Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
  Where-Object {{
    ($_.ProcessId -ne $current) -and ($_.ProcessId -ne $PID) -and (
      (($_.Name -eq 'powershell.exe' -or $_.Name -eq 'pwsh.exe') -and $_.CommandLine -like '*wvc-hotkey*') -or
      (($_.Name -eq 'wvc.exe') -and $_.CommandLine -like '*--listen-windows-hotkey*')
    )
  }} |
...
pub(super) fn uninstall_windows_hotkey_listener() -> Result<()> {
    ...
    state.save()?;
    eprintln!("  \x1b[32m✓\x1b[0m Removed wvc Windows launch-hotkey listener");
    Ok(())
}
...
fn create_hotkey_shortcut(_use_alacritty: bool) -> Result<()> {
    ...
    let hotkey_dir = storage::wvc_dir()?.join("hotkey");
...
fn launch_windows_hotkey(entry: &WindowsHotkey) -> Result<()> {
    ...
    let command = wvc_terminal_launch::TerminalCommand::new(exe, args)
        .title("wvc")
        .fresh_spawn()
        .kind("launch-hotkey");

    let launched =
        wvc_terminal_launch::spawn_command_in_new_terminal_with(&command, &cwd, |cmd| {
            cmd.spawn().map(|_| ())
        })?;
    if !launched {
        anyhow::bail!("no terminal found to launch wvc");
    }
    Ok(())
}
...
fn windows_native_hotkey_loop(entries: Vec<WindowsHotkey>) -> Result<()> {
    ...
    const MUTEX_NAME: &str = "Local\\WeavecoderLaunchHotkeyListener";
    ...
    let mutex = loop {
        ...
        if retry_count >= 40 {
            wvc_logging::warn("previous Windows launch-hotkey listener is still exiting");
            return Ok(());
        }
        retry_count += 1;
            ...
            if let Some(entry) = entry.cloned() {
                std::thread::spawn(move || {
                    if let Err(err) = launch_windows_hotkey(&entry) {
                        wvc_logging::warn(&format!(
                            "failed to launch wvc from Windows hotkey: {err}"
                        ));
                    }
                });
            }
...
/// interactive `wvc setup-hotkey` flow to install it.
...
pub(super) fn reinstall_windows_launch_hotkeys() {
    ...
    match refresh_windows_launch_hotkeys() {
        Ok(()) => wvc_logging::info("Reinstalled Windows launch hotkeys after config change"),
        Err(err) => wvc_logging::warn(&format!(
...
fn nudge_hotkey(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Set up global keys to launch wvc?\x1b[0m                  \x1b[36m│\x1b[0m"
    );
    eprintln!(
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m    Opens wvc in {:<39}    \x1b[36m│\x1b[0m",
        ...
        "y" | "yes" => {
            ...
            match create_hotkey_shortcut(using_alacritty) {
                Ok(()) => {
                    ...
                    let _ = state.save();
                    eprintln!(
                        "  \x1b[32m✓\x1b[0m Created hotkeys (\x1b[1mAlt+;\x1b[0m and \x1b[1mCopilot\x1b[0m) → {} + wvc",
                        terminal_name
                    );
                    eprintln!();
                    true
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "    You can set it up manually later with: \x1b[1mwvc setup-hotkey\x1b[0m"
                    );
                    eprintln!();
                    false
                }
...
fn nudge_alacritty(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!("\x1b[36m┌─────────────────────────────────────────────────────────────┐\x1b[0m");
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Alacritty: the fastest terminal for wvc\x1b[0m               \x1b[36m│\x1b[0m"
    );
    eprintln!(
        "\x1b[36m│\x1b[0m                                                             \x1b[36m│\x1b[0m"
    );
                ...
                Ok(()) => {
                    ...
                    if state.hotkey_configured {
                        ...
                        match create_hotkey_shortcut(true) {
                            Ok(()) => {
                                eprintln!(
                                    "  \x1b[32m✓\x1b[0m Hotkeys updated: \x1b[1mAlt+;\x1b[0m and \x1b[1mCopilot\x1b[0m → Alacritty + wvc"
                                );
                            }
                            Err(e) => {
...
fn prompt_try_it_out(installed_alacritty: bool) {
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    Press \x1b[1mAlt+;\x1b[0m or the \x1b[1mCopilot key\x1b[0m to launch wvc.       \x1b[32m│\x1b[0m"
    );
    eprintln!(
        "\x1b[32m│\x1b[0m    The listener is native Weavecoder, no AutoHotkey required.          \x1b[32m│\x1b[0m"
    );
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    \x1b[90m(Starting wvc normally in 3 seconds...)\x1b[0m                 \x1b[32m│\x1b[0m"
    );
    eprintln!("\x1b[32m└─────────────────────────────────────────────────────────────┘\x1b[0m");
    eprintln!();
...
mod tests {
    ...
    fn powershell_single_quote_escapes_embedded_quotes() {
        assert_eq!(ps_single_quote(r"C:\O'Hara\wvc"), r"'C:\O''Hara\wvc'");
    }

    #[test]
    fn listener_stop_sweep_excludes_wvc_and_powershell_processes_running_it() {
    ...
    fn startup_shortcut_uses_native_listener_without_vbscript_or_bypass() {
        let script = render_startup_shortcut_script(
            Path::new(r"C:\Users\O'Hara\Startup\wvc-hotkey.lnk"),
            Path::new(r"C:\Program Files\Weavecoder O'Hara\wvc.exe"),
        );
...
pub(super) fn run_setup_hotkey_windows() -> Result<()> {
    ...
    eprintln!(
        "  Setting up global launch hotkeys → {} + wvc...",
        terminal_name
    );
...
pub(super) fn create_windows_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    ...
    let shortcut_path = format!("{}\\Desktop\\wvc.lnk", desktop_dir);
...
$shortcut.Description = "wvc - AI coding agent"
$shortcut.Save()
Write-Output "OK"
"#,
    ...
    if output.status.success() {
        ...
        if stdout.contains("OK") {
            ...
            let _ = state.save();
            wvc_logging::info(&format!("Created desktop shortcut: {}", shortcut_path));
        }
    }
