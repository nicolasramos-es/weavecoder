...
#[cfg(any(test, target_os = "macos"))]
fn paused_wvc_shell_command_with_args(exe_path: &str, args: &[String]) -> String {
...
/// `launch_wvc_cmd_bracketleft.sh`. Collisions are avoided by appending the
...
pub(crate) fn shell_command_for(entry: &ResolvedLaunchHotkey, exe_path: &str) -> String {
    format!(
        "{}{}",
        entry.cd_prefix,
        paused_wvc_shell_command_with_args(exe_path, &entry.args)
    )
}
