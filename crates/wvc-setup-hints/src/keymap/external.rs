...
//! *before* the terminal (and therefore wvc) ever sees them.
//!
//! macOS lets window managers and automation tools register system-wide hotkeys.
//! When one of those overlaps a key wvc wants (the classic case is a tiling WM
//! binding `Cmd+J`/`Cmd+K` to window focus, which shadows wvc's prompt
//! navigation), the keystroke never reaches the terminal and the wvc binding
...
fn apply_modifier(token: &str, mods: &mut Mods, hyper_mods: Mods) -> bool {
    match token.trim().to_ascii_lowercase().as_str() {
        ...
        // "fn" is not representable as a wvc modifier; ignore it so the rest of
