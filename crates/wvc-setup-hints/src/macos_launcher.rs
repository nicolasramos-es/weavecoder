use super::{
    ...
    escape_shell_single_quotes, launch_command_for_macos_terminal, paused_wvc_shell_command,
...
pub(super) fn install_macos_app_launcher() -> Result<(PathBuf, MacTerminalKind)> {
    ...
    let launcher_path = macos_dir.join("wvc-launcher");
...
"#,
        version = wvc_build_meta::version(),
...
fn macos_app_launcher_dir() -> Result<PathBuf> {
    ...
    Ok(home.join("Applications").join("Weavecoder.app"))
}

fn legacy_macos_app_launcher_dir() -> Result<PathBuf> {
    let home = dirs::home_dir().context("Could not find home directory")?;
    Ok(home.join("Applications").join("wvc.app"))
}
...
display alert "Weavecoder launch failed" message "{missing_message}" as critical
APPLESCRIPT
}}

show_terminal_launch_failure() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "Weavecoder launch failed" message "{terminal_failure_message}" as critical
APPLESCRIPT
}}
