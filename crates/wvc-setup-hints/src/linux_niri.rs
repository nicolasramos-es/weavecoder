...
pub(crate) fn render_niri_bind_line(
    ...
    let title = if hotkey.self_dev {
        format!("wvc: {} (self-dev)", hotkey.label)
    } else {
        format!("wvc: {}", hotkey.label)
    };
...
mod tests {
    ...
    fn spliced_config_is_accepted_by_the_real_niri_parser() {
        ...
        let dir = std::env::temp_dir().join(format!("wvc-niri-719-{}", std::process::id()));
    ...
    fn fuzz_corpus_stays_valid_after_splicing() {
        ...
        let dir = std::env::temp_dir().join(format!("wvc-niri-fuzz-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
