...
fn truncated_stream_payload_context(data: &str) -> String {
    wvc_core::util::truncate_str(&data.trim().replace("\n", "\\n"), 240).to_string()
}
...
pub fn parse_openai_response_event(
    ...
    if is_websocket_fallback_notice(data) {
        wvc_logging::warn(&format!("OpenAI stream transport notice: {}", data.trim()));
        return None;
    }
...
fn handle_openai_image_generation_item(
    ...
    let image_bytes = match BASE64_STANDARD.decode(result_b64) {
        ...
        Err(err) => {
            ...
            return Some(StreamEvent::TextDelta(
                "\n[Generated image received, but Weavecoder could not decode it.]\n".to_string(),
            ));
        }
    };
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        return Some(StreamEvent::TextDelta(format!(
            "\n[Generated image received ({} bytes), but Weavecoder could not save it.]\n",
            image_bytes.len()
        )));
    }
    ...
    if let Err(err) = std::fs::write(&path, image_bytes) {
        wvc_logging::warn(&format!("Failed to save OpenAI generated image: {}", err));
        return Some(StreamEvent::TextDelta(
            "\n[Generated image received, but Weavecoder could not save it.]\n".to_string(),
        ));
    }
    ...
    let metadata_path_string = match serde_json::to_vec_pretty(&metadata).ok().and_then(|bytes| {
        ...
        None => {
            wvc_logging::warn("Failed to save OpenAI generated image metadata");
            None
        }
    };
...
pub struct OpenAIResponsesStream {
    ...
    utf8: wvc_core::util::Utf8StreamDecoder,
...
impl OpenAIResponsesStream {
    pub fn new(stream: impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static) -> Self {
        Self {
            ...
            buffer: String::new(),
            utf8: wvc_core::util::Utf8StreamDecoder::new(),
    ...
    fn parse_next_event(&mut self) -> Option<StreamEvent> {
        ...
        while let Some(pos) = self.buffer.find("\n\n") {
            ...
            for line in event_str.lines() {
                if let Some(data) = wvc_core::util::sse_data_line(line) {
