...
pub(super) fn telemetry_id_path() -> Option<PathBuf> {
    storage::wvc_dir().ok().map(|d| d.join("telemetry_id"))
}
...
pub(super) fn telemetry_state_path(name: &str) -> Option<PathBuf> {
    storage::wvc_dir().ok().map(|d| d.join(name))
}
...
pub(super) fn version() -> String {
    wvc_build_meta::pkg_version().to_string()
}
...
pub(super) fn is_wvc_repo_dir(dir: &Path) -> bool {
    ...
    std::fs::read_to_string(cargo_toml)
        .map(|content| content.contains("name = \"wvc\""))
        .unwrap_or(false)
}

fn find_wvc_repo_in_ancestors(start: &Path) -> Option<PathBuf> {
    start
        .ancestors()
        .find(|dir| is_wvc_repo_dir(dir))
...
fn telemetry_wvc_repo_dir() -> Option<PathBuf> {
    ...
    if let Some(repo) = find_wvc_repo_in_ancestors(&manifest_dir) {
        return Some(repo);
    }
    ...
    if let Ok(exe) = std::env::current_exe()
        && let Some(repo) = exe
            ...
            .and_then(Path::parent)
            .filter(|dir| is_wvc_repo_dir(dir))
    {
    ...
    std::env::current_dir()
        .ok()
        .and_then(|cwd| find_wvc_repo_in_ancestors(&cwd))
}

pub(super) fn build_channel() -> String {
    if std::env::var(wvc_selfdev_types::CLIENT_SELFDEV_ENV).is_ok() {
    ...
    if telemetry_wvc_repo_dir().is_some() {
...
pub(super) fn is_git_checkout() -> bool {
    telemetry_wvc_repo_dir().is_some()
}
