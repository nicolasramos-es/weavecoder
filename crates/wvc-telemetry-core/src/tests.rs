...
fn test_do_not_track() {
    let _guard = lock_test_env();
    wvc_core::env::set_var("DO_NOT_TRACK", "1");
    assert!(!is_enabled());
    wvc_core::env::remove_var("DO_NOT_TRACK");
}
...
fn test_is_ci_detects_ci_env() {
    ...
    wvc_core::env::set_var("NEXTEST", "1");
    assert!(is_ci(), "nextest runs should be tagged as automation");
    wvc_core::env::remove_var("NEXTEST");
    assert!(!is_ci());
}
...
fn test_onboarding_step_milestone_key_includes_provider_and_method() {
    assert_eq!(
        onboarding_step_milestone_key("auth_success", Some("wvc"), Some("API key")),
        "auth_success_wvc_api_key"
    );
    assert_eq!(
