...
fn opt_out_marker_path() -> Option<std::path::PathBuf> {
    storage::wvc_dir().ok().map(|d| d.join("no_telemetry"))
}
...
fn detect_project_profile() -> ProjectProfile {
    ...
    profile.repo_present = root.join(".git").exists() || is_wvc_repo_dir(root);
...
pub fn record_auth_started(provider: &str, method: &str) {
    wvc_logging::auth_event("auth_started", provider, &[("method", method)]);
...
pub fn record_auth_cancelled(provider: &str, method: &str) {
    wvc_logging::auth_event("auth_cancelled", provider, &[("method", method)]);
...
pub fn record_auth_surface_blocked(provider: &str, method: &str) {
    wvc_logging::auth_event("auth_surface_blocked", provider, &[("method", method)]);
...
pub fn record_auth_success(provider: &str, method: &str) {
    wvc_logging::auth_event("auth_success", provider, &[("method", method)]);
    if !is_enabled() {
        return;
    }
    let Some(id) = get_or_create_id() else {
...
fn show_first_run_notice() {
    ...
    let (dim, reset) = if wvc_core::console::stderr_supports_ansi() {
        ("\x1b[90m", "\x1b[0m")
    } else {
        ("", "")
    };
    ...
    eprintln!("  Details: https://github.com/nicolasramos-es/weavecoder/blob/master/TELEMETRY.md");
    eprintln!("{reset}");
}
