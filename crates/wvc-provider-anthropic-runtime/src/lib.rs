...
//! loading, cache-TTL toggles) stay in `wvc_base::provider::anthropic`;
...
use wvc_base::auth;
use wvc_base::auth::oauth;
use wvc_provider_core::{EventStream, NativeToolResultSender, Provider};
fn oauth_beta_headers(model: &str) -> &'static str {
    wvc_provider_core::anthropic_oauth_beta_headers(model)
}
...
#[cfg(test)]
use wvc_base::provider::anthropic::{OAUTH_BETA_HEADERS, effectively_1m};
#[cfg(test)]
use wvc_message_types::{ContentBlock, Role};
use wvc_message_types::{Message, StreamEvent, ToolDefinition};
#[cfg(test)]
use wvc_provider_anthropic::{ApiContentBlock, ToolResultContent, ToolResultContentBlock};
use wvc_provider_anthropic::{
...
#[cfg(test)]
pub(crate) const OAUTH_BETA_HEADERS_1M: &str = wvc_provider_core::ANTHROPIC_OAUTH_BETA_HEADERS_1M;
...
fn load_official_claude_client_metadata() -> OAuthClientMetadata {
    let path = match wvc_base::storage::user_home_path(".claude.json") {
        Ok(path) => path,
...
async fn oauth_preflight_get(
    ...
    if !resp.status().is_success() {
        ...
        let body = wvc_base::util::http_error_body(resp, "HTTP error").await;
...
async fn ensure_oauth_preflight(
    ...
    let eval = OAuthEvalRequest {
        attributes: OAuthEvalAttributes {
            ...
            subscription_type: wvc_base::auth::claude::get_subscription_type()
    ...
    if all_ok {
        wvc_base::logging::info("Claude OAuth preflight completed successfully");
    }
    Ok(())
}

/// Quality-first default shared with model routing and first-run selection.
const DEFAULT_MODEL: &str = wvc_provider_core::DEFAULT_CLAUDE_MODEL;
...
impl AnthropicProvider {
    fn is_usage_exhausted() -> bool {
        let usage = wvc_base::usage::get_sync();
    ...
    pub async fn fetch_live_model_ids_for_doctor(&self) -> Result<Vec<String>> {
        ...
        let catalog = if is_oauth {
            wvc_base::provider::fetch_anthropic_model_catalog_oauth(&token).await?
        } else {
            wvc_base::provider::fetch_anthropic_model_catalog(&token).await?
        };
        // Persist so the rest of the process benefits from the warm catalog,
        // exactly like the runtime's own prefetch.
        wvc_base::provider::persist_anthropic_model_catalog(&catalog);
        if !catalog.context_limits.is_empty() {
            wvc_base::provider::populate_context_limits(catalog.context_limits.clone());
        }
        if !catalog.available_models.is_empty() {
            wvc_base::provider::populate_anthropic_models(catalog.available_models.clone());
        }
    ...
    pub fn new() -> Self {
        ...
        let _ = tokio::runtime::Handle::try_current().map(|_| {
            tokio::spawn(async {
                let _ = wvc_base::usage::get().await;
            })
        });
    ...
    fn model_supports_output_effort(model: &str) -> bool {
        ...
        // generations); see `wvc_provider_core::anthropic_reasoning_caps`.
        // Fable 5 verified live 2026-07-01; Sonnet 5 verified live 2026-07-07.
        wvc_provider_core::anthropic_reasoning_caps(model).output_effort
    }

    fn model_supports_adaptive_thinking(model: &str) -> bool {
        wvc_provider_core::anthropic_reasoning_caps(model).adaptive_thinking
    }

    fn model_supports_manual_thinking(model: &str) -> bool {
        wvc_provider_core::anthropic_reasoning_caps(model).manual_thinking
    }

    fn model_supports_xhigh_effort(model: &str) -> bool {
        wvc_provider_core::anthropic_reasoning_caps(model).xhigh_effort
    }
    ...
    fn model_supports_max_effort(model: &str) -> bool {
        wvc_provider_core::anthropic_reasoning_caps(model).max_effort
    }

    fn model_supports_reasoning_effort(model: &str) -> bool {
        wvc_provider_core::anthropic_reasoning_caps(model).supports_reasoning_effort()
    }
    ...
    fn actual_effort_for_model(model: &str, effort: &str) -> String {
        if wvc_base::prompt::is_swarm_effort(effort) {
    ...
    fn store_effort_for_model(model: &str, effort: &str) -> String {
        if wvc_base::prompt::is_deep_swarm_effort(effort) {
            wvc_base::prompt::SWARM_DEEP_EFFORT.to_string()
        } else if wvc_base::prompt::is_swarm_effort(effort) {
            wvc_base::prompt::SWARM_EFFORT.to_string()
        } else {
    ...
    fn max_tokens_for(&self, model: &str) -> u32 {
        self.max_tokens_override
            .unwrap_or_else(|| wvc_provider_core::anthropic::anthropic_max_output_tokens(model))
    }
    ...
    fn manual_thinking_budget(effort: &str, max_tokens: u32) -> Option<u32> {
        let desired = match effort {
            ...
            "high" => 8_192,
            "xhigh" | "max" => 16_384,
            e if wvc_base::prompt::is_swarm_effort(e) => 16_384,
            _ => return None,
        };
    ...
    fn build_reasoning_request_parts(
        ...
        let show_thinking = wvc_base::config::config().display.show_thinking;
    ...
    async fn get_oauth_access_token(&self) -> Result<(String, bool)> {
        ...
        if fresh_creds.expires_at < now + 300_000 && !fresh_creds.refresh_token.is_empty() {
            ...
            {
                Ok(refreshed) => {
                    wvc_base::logging::info("OAuth token refreshed successfully");
                ...
                Err(e) => {
                    wvc_base::logging::error(&format!("OAuth token refresh failed: {}", e));
    ...
    pub(crate) fn set_credential_mode(&self, mode: AnthropicCredentialMode) -> Result<()> {
        ...
        *mode_guard = mode;
        ...
        wvc_base::auth::AuthStatus::invalidate_cached_status();
        Ok(())
    }
    ...
    fn format_messages(&self, messages: &[Message], is_oauth: bool) -> Vec<ApiMessage> {
        wvc_provider_anthropic::format_messages(messages, is_oauth)
    }
    ...
    fn format_content_blocks(
        ...
        wvc_provider_anthropic::format_content_blocks(blocks, is_oauth)
    }
    ...
    fn format_tools(&self, tools: &[ToolDefinition], is_oauth: bool) -> Vec<ApiTool> {
        wvc_provider_anthropic::format_tools(tools, is_oauth, is_cache_ttl_1h())
    }
}
...
fn log_anthropic_canonical_input(
    ...
    wvc_provider_core::fingerprint::log_provider_canonical_input(
        "anthropic",
        model,
        format,
        &payload,
...
impl Provider for AnthropicProvider {
    ...
    fn credential_mode(&self) -> wvc_provider_core::CredentialMode {
        self.credential_mode_snapshot()
    }

    fn set_credential_mode(&self, mode: wvc_provider_core::CredentialMode) -> Result<()> {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let model: &str = if is_1m_model(model)
            && matches!(
                wvc_provider_core::anthropic_context_mode(model),
                wvc_provider_core::AnthropicContextMode::Native1M
            ) {
            strip_1m_suffix(model)
        } else {
            model
        };
        if !wvc_base::provider::known_anthropic_model_ids()
            .iter()
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        wvc_base::provider::cached_anthropic_model_ids()
            .unwrap_or_else(wvc_base::provider::known_anthropic_model_ids)
    }
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let catalog = if is_oauth {
            match wvc_base::provider::fetch_anthropic_model_catalog_oauth(&token).await {
        ...
        wvc_base::provider::persist_anthropic_model_catalog(&catalog);
        if !catalog.context_limits.is_empty() {
            wvc_base::provider::populate_context_limits(catalog.context_limits);
        }
        if !catalog.available_models.is_empty() {
            wvc_base::provider::populate_anthropic_models(catalog.available_models);
        }
        Ok(())
    }
...
async fn run_stream_with_retries(
    ...
    for attempt in 0..MAX_RETRIES {
        if attempt > 0 {
            ...
            let delay = wvc_provider_core::retry_after::retry_delay(
                attempt,
                RETRY_BASE_DELAY_MS,
            ...
            let _ = tx
                .send(Ok(StreamEvent::ConnectionPhase {
                    phase: wvc_message_types::ConnectionPhase::Retrying {
                        attempt: attempt + 1,
                        max: MAX_RETRIES,
                    },
                }))
        ...
        let (attempt_tx, attempt_guard) =
            wvc_provider_core::attempt_tracker::track_attempt_output(tx.clone());
        ...
        let attempt_client = if attempt == 0 {
            ...
            wvc_provider_core::fresh_transport_client()
        };
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                if is_oauth && is_oauth_auth_error(&error_str) && !attempted_forced_refresh {
                    ...
                    let _ = tx
                        .send(Ok(StreamEvent::ConnectionPhase {
                            phase: wvc_message_types::ConnectionPhase::Authenticating,
                        }))
                        .await;
...
async fn stream_response(
    ...
    {
        wvc_base::logging::info(&format!("Anthropic request payload:\n{}", json));
    }
    ...
    let stream_idle_timeout = wvc_base::provider::stream_idle_timeout();
    ...
    let response = wvc_provider_core::transport::send_with_initial_response_timeout(
        req.json(&request),
        stream_idle_timeout,
    )
    .await
    ...
    if !response.status().is_success() {
        ...
        let retry_after = wvc_provider_core::retry_after::retry_after(response.headers());
        let error_text = wvc_base::util::http_error_body(response, "HTTP error").await;
        return Err(wvc_provider_core::retry_after::error_with_retry_after(
...
fn is_retryable_error(error_str: &str) -> bool {
    wvc_provider_core::is_transient_transport_error(error_str)
        // Server errors (5xx)
...
fn anthropic_model_quality_rank(model: &str) -> usize {
    ...
    let normalized = wvc_provider_core::model_id::strip_date_suffix(
        &wvc_provider_core::model_id::canonical(model),
    )
    .to_string();
    wvc_provider_core::ALL_CLAUDE_MODELS
        ...
        // available.
        .unwrap_or(wvc_provider_core::ALL_CLAUDE_MODELS.len())
}
...
fn anthropic_recommended_model_from_error(error_str: &str) -> Option<String> {
    ...
    wvc_base::provider::known_anthropic_model_ids()
        .into_iter()
...
fn parse_sse_event(buffer: &mut String) -> Option<SseEvent> {
    ...
    for line in event_str.lines() {
        ...
        } else if let Some(rest) = wvc_base::util::sse_data_line(line) {
            data = rest.to_string();
        }
    }
    ...
    match event.event_type.as_str() {
        ...
        "ping" => {
            ...
            events.push(StreamEvent::ConnectionPhase {
                phase: wvc_message_types::ConnectionPhase::Streaming,
            });
        }
        "error" => {
            wvc_base::logging::error(&format!("Anthropic stream error: {}", event.data));
...
fn build_system_param(system: &str, is_oauth: bool) -> Option<ApiSystem> {
    wvc_provider_anthropic::build_system_param(system, is_oauth, is_cache_ttl_1h())
}
...
fn build_system_param_split(
    ...
    wvc_provider_anthropic::build_system_param_split(
        static_part,
        dynamic_part,
        is_oauth,
        is_cache_ttl_1h(),
...
fn format_messages_with_identity(messages: Vec<ApiMessage>, is_oauth: bool) -> Vec<ApiMessage> {
    wvc_provider_anthropic::format_messages_with_identity(messages, is_oauth, is_cache_ttl_1h())
}

#[cfg(test)]
fn add_message_cache_breakpoint(messages: &mut [ApiMessage]) {
    wvc_provider_anthropic::add_message_cache_breakpoint(messages, is_cache_ttl_1h())
}
