...
fn stalled_process_emits_watchdog_stall_event() {
    let dir = std::env::temp_dir().join(format!("wvc-wd-probe-{}", std::process::id()));
    ...
    wvc_logging::init();
    let _work = wvc_logging::watchdog::begin_work("probe.phase");
    wvc_logging::watchdog::set_detail("probe detail");
    ...
    while Instant::now() < deadline {
        ...
        let Some(path) = wvc_logging::log_path() else {
            continue;
        };
