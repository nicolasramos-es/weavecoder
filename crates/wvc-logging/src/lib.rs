...
fn log_dir() -> Option<PathBuf> {
    wvc_storage::logs_dir().ok()
}

impl Logger {
    fn new() -> Option<Self> {
        let log_dir = log_dir()?;
        wvc_storage::ensure_dir(&log_dir).ok()?;

        // Use date-based log file
        let date = Local::now().format("%Y-%m-%d");
        let path = log_dir.join(format!("wvc-{}.log", date));
    ...
    fn write(&mut self, level: &str, message: &str) {
        ...
        if let Err(err) = self.file.write_all(line.as_bytes()) {
            eprintln!("wvc logger write failed: {err}");
            return;
        }
        if let Err(err) = self.file.flush() {
            eprintln!("wvc logger flush failed: {err}");
        }
    }
}
...
pub fn init() {
    ...
    if !CLEANUP_STARTED.swap(true, Ordering::SeqCst) {
        std::thread::Builder::new()
            .name("wvc-log-cleanup".to_string())
...
fn cleanup_old_logs_in(log_dir: &std::path::Path, now: chrono::DateTime<Local>) {
    ...
    for entry in entries.flatten() {
        ...
        let is_wvc_log = (name.starts_with("wvc-") || name.starts_with("wvc-desktop-"))
            && name.ends_with(".log");
        if !is_wvc_log {
            continue;
        }
            ...
            && let Ok(modified) = metadata.modified()
        {
            ...
            {
                eprintln!("wvc logger cleanup failed: {err}");
            }
        }
    }
}
...
fn truncate(s: &str, max_len: usize) -> String {
    if s.len() > max_len {
        format!("{}...", wvc_core::util::truncate_str(s, max_len))
    } else {
        s.to_string()
    }
}
...
mod tests {
    ...
    #[test]
    fn cleanup_removes_only_old_wvc_logs() {
        use std::time::{Duration, SystemTime};

        let dir = std::env::temp_dir().join(format!(
            "wvc-log-cleanup-test-{}-{}",
            std::process::id(),
            SystemTime::now()
        ...
        let old_log = write("wvc-2000-01-01.log", true);
        let old_desktop = write("wvc-desktop-2000-01-01.log", true);
        // Recent log file that SHOULD survive.
        let new_log = write("wvc-2099-01-01.log", false);
