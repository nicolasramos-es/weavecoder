...
//!
//! Historically wvc used a single shared list of default keybindings for
...
pub const KEYBINDING_DEFAULTS: &[KeybindingDefault] = &[
    ...
    KeybindingDefault {
        id: "new_terminal",
        description: "Spawn a fresh wvc session in a new terminal window",
        // Companion to the system-wide launch hotkey (Cmd+; / Alt+;): the
        // shifted variant spawns a new session from inside wvc, reusing the
    ...
    KeybindingDefault {
        ...
        // Cmd+B is free in wvc on macOS. On Windows/Linux, Ctrl+R is already
