...
use wvc_provider_metadata::{is_safe_env_file_name, is_safe_env_key_name};
...
pub fn load_api_key_from_env_or_config(env_key: &str, file_name: &str) -> Option<String> {
    ...
    let config_path = wvc_storage::app_config_dir().ok()?.join(file_name);
    wvc_storage::harden_secret_file_permissions(&config_path);
...
/// Load a value only from the saved env file under the wvc config dir,
...
pub fn save_env_value_to_env_file(
    ...
    let config_dir = wvc_storage::app_config_dir()?;
    let file_path = config_dir.join(file_name);
    wvc_storage::upsert_env_file_value(&file_path, env_key, value)?;

    if let Some(value) = value {
        wvc_core::env::set_var(env_key, value);
    } else {
        wvc_core::env::remove_var(env_key);
    }
...
mod tests {
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            for (key, value) in self.saved.drain(..) {
                match value {
                    Some(value) => wvc_core::env::set_var(key, value),
                    None => wvc_core::env::remove_var(key),
                }
            }
        }
    }
    ...
    fn loads_api_key_from_env_before_config_file() {
        ...
        let _guard = EnvGuard::new(&["WVC_HOME", "WVC_PROVIDER_ENV_TEST_KEY"]);
        wvc_core::env::set_var("WVC_HOME", temp.path());

        save_env_value_to_env_file(
            "WVC_PROVIDER_ENV_TEST_KEY",
            "provider-env-test.env",
            Some("file-key"),
        )
        .expect("save file key");
        wvc_core::env::set_var("WVC_PROVIDER_ENV_TEST_KEY", "env-key");

        assert_eq!(
            load_api_key_from_env_or_config("WVC_PROVIDER_ENV_TEST_KEY", "provider-env-test.env")
                .as_deref(),
            Some("env-key")
        );
    }
    ...
    fn loads_and_removes_values_from_sandboxed_config_file() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["WVC_HOME", "WVC_PROVIDER_ENV_TEST_VALUE"]);
        wvc_core::env::set_var("WVC_HOME", temp.path());
        ...
        wvc_core::env::remove_var("WVC_PROVIDER_ENV_TEST_VALUE");
        assert_eq!(
    ...
    fn accepts_legacy_zai_key_for_zhipu() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["WVC_HOME", "ZHIPU_API_KEY", "ZAI_API_KEY"]);
        wvc_core::env::set_var("WVC_HOME", temp.path());

        save_env_value_to_env_file("ZAI_API_KEY", "zai.env", Some("legacy-zai-key"))
            .expect("save legacy key");
        wvc_core::env::remove_var("ZAI_API_KEY");
    ...
    fn loads_api_key_with_zero_width_space_from_config_file() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["WVC_HOME", "WVC_PROVIDER_FOO_API_KEY"]);
        wvc_core::env::set_var("WVC_HOME", temp.path());

        // Write an env file with a U+200B zero-width space prefixed onto the key,
        // mirroring issue #376's reproduction.
        let config_dir = wvc_storage::app_config_dir().expect("config dir");
        std::fs::create_dir_all(&config_dir).expect("create config dir");
        std::fs::write(
            config_dir.join("provider-foo.env"),
            "WVC_PROVIDER_FOO_API_KEY=\u{200B}sk-mykey123\n",
        )
        .expect("write env file");

        assert_eq!(
            load_api_key_from_env_or_config("WVC_PROVIDER_FOO_API_KEY", "provider-foo.env")
                .as_deref(),
            Some("sk-mykey123")
        );
    }
    ...
    fn loads_api_key_with_invisible_chars_from_env_var() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["WVC_HOME", "WVC_PROVIDER_BAR_API_KEY"]);
        wvc_core::env::set_var("WVC_HOME", temp.path());
        // NBSP + BOM padding around the env-provided key.
        wvc_core::env::set_var("WVC_PROVIDER_BAR_API_KEY", "\u{00A0}sk-env-key\u{FEFF}");

        assert_eq!(
            load_api_key_from_env_or_config("WVC_PROVIDER_BAR_API_KEY", "provider-bar.env")
                .as_deref(),
            Some("sk-env-key")
        );
    }
