...
fn download_model_blocking(model_dir: &Path) -> Result<()> {
    let client = reqwest::blocking::Client::builder()
        .user_agent(concat!("wvc-embedding/", env!("CARGO_PKG_VERSION")))
...
mod tests {
    ...
    fn alt_model_related_beats_unrelated_if_present() {
        let dir = std::env::var_os("HOME")
            .map(|h| std::path::PathBuf::from(h).join("wvc-memory-bench/models/e5-small-v2"))
            .filter(|d| is_model_available(d));
        let Some(d) = dir else {
    ...
    fn cross_encoder_scores_relevant_higher_if_present() {
        let dir = std::env::var_os("HOME")
            .map(|h| std::path::PathBuf::from(h).join("wvc-memory-bench/models/ce-minilm-l6"))
