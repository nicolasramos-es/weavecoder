...
fn make_test_provider(fetched: Vec<String>) -> CopilotApiProvider {
    CopilotApiProvider {
        client: wvc_base::provider::shared_http_client(),
...
fn context_window_handles_dot_and_dash_names() {
    ...
    assert_eq!(
        wvc_base::provider::context_limit_for_model_with_provider("gpt-5.4", Some("copilot")),
        Some(128_000)
    );
    assert_eq!(
        wvc_base::provider::context_limit_for_model_with_provider("gpt-5.4-pro", Some("copilot")),
        Some(128_000)
    );
    assert_eq!(
        wvc_base::provider::context_limit_for_model_with_provider(
