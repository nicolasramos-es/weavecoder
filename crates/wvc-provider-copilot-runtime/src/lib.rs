...
impl CopilotApiProvider {
    ...
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(wvc_base::storage::app_config_dir()?.join("copilot_models_cache.json"))
    }

    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        let path = Self::persisted_catalog_path().ok()?;
        wvc_base::storage::read_json(&path)
            .ok()
    ...
    fn persist_catalog(models: &[String]) {
        ...
        if let Err(error) = wvc_base::storage::write_json(&path, &payload) {
    ...
    pub fn set_premium_mode(&self, mode: PremiumMode) {
        ...
        if mode != PremiumMode::Normal {
            wvc_base::logging::info(&format!("Copilot premium mode set to {:?}", mode));
        }
    }
    ...
    async fn stream_request(
        ...
        for attempt in 0..MAX_RETRIES {
            if attempt > 0 {
                let delay = wvc_provider_core::attempt_tracker::retry_backoff_delay(
                    attempt,
                    RETRY_BASE_DELAY_MS,
                );
                wvc_base::logging::info(&format!(
            ...
            if Self::is_auth_error(status) && !attempted_auth_refresh {
                ...
                *self.bearer_token.write().await = None;
                wvc_base::logging::info("Copilot bearer token expired, refreshing...");
                last_error = Some(anyhow::anyhow!("Copilot auth error (HTTP {})", status));
                continue;
            }

            if !status.is_success() {
                let body_text = wvc_base::util::http_error_body(resp, "HTTP error").await;
                let error_str =
    ...
    async fn process_sse_stream(
        ...
        loop {
            ...
            while let Some(line_end) = buffer.find('\n') {
                ...
                if let Some(data) = wvc_base::util::sse_data_line(&line) {
                    if data.trim() == "[DONE]" {
...
impl Provider for CopilotApiProvider {
    ...
    fn context_window(&self) -> usize {
        wvc_provider_core::context_limit_for_model_with_provider(&self.model(), Some(self.name()))
            .unwrap_or(128_000)
    }
