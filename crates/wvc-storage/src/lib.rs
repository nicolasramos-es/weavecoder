...
fn fallback_runtime_dir() -> PathBuf {
    std::env::temp_dir().join(format!("wvc-{}", runtime_user_discriminator()))
}
...
fn ensure_private_runtime_dir(path: &Path) {
    ...
    #[cfg(unix)]
    {
        let _ = wvc_core::fs::set_directory_permissions_owner_only(path);
    }
}
...
pub fn harden_user_config_permissions() {
    #[cfg(windows)]
    {
        if let Some(config_dir) = dirs::config_dir() {
            let wvc_config_dir = config_dir.join("wvc");
            if wvc_config_dir.exists() {
                schedule_windows_path_hardening(&wvc_config_dir, true);
            }
        }

        if let Ok(wvc_home) = wvc_dir()
            && wvc_home.exists()
        {
            schedule_windows_path_hardening(&wvc_home, true);
        }
        return;
    }
    ...
    #[cfg(not(windows))]
    {
        if let Some(config_dir) = dirs::config_dir() {
            let wvc_config_dir = config_dir.join("wvc");
            if wvc_config_dir.exists() {
                let _ = wvc_core::fs::set_directory_permissions_owner_only(&wvc_config_dir);
            }
        }
        ...
        if let Ok(wvc_home) = wvc_dir()
            && wvc_home.exists()
        {
            let _ = wvc_core::fs::set_directory_permissions_owner_only(&wvc_home);
        }
    }
}
...
pub fn harden_secret_file_permissions(path: &Path) {
    ...
    #[cfg(not(windows))]
    {
        if let Some(parent) = path.parent() {
            let _ = wvc_core::fs::set_directory_permissions_owner_only(parent);
        }
        if path.exists() {
            let _ = wvc_core::fs::set_permissions_owner_only(path);
        }
    }
}
...
fn schedule_windows_path_hardening(path: &Path, directory: bool) {
    ...
    if std::thread::Builder::new()
        .name("wvc-windows-acl-harden".to_string())
        .spawn(|| {
...
fn run_windows_hardening_worker() {
    loop {
        ...
        for path in &directories {
            let succeeded = wvc_core::fs::set_directory_permissions_owner_only(path).is_ok();
            directory_results.push((path.clone(), succeeded));
        }
        let mut file_results = Vec::with_capacity(files.len());
        for path in &files {
            let succeeded =
                !path.exists() || wvc_core::fs::set_permissions_owner_only(path).is_ok();
...
pub fn ensure_dir(path: &Path) -> Result<()> {
    if !path.exists() {
        ...
        wvc_core::fs::set_directory_permissions_owner_only(path)?;
    }
    Ok(())
}
...
fn write_bytes_inner(path: &Path, bytes: &[u8], durable: bool, secret: bool) -> Result<()> {
    if let Some(parent) = path.parent() {
        ...
        if secret {
            ...
            wvc_core::fs::set_directory_permissions_owner_only(parent)?;
        }
    }
    ...
    let result = (|| -> Result<()> {
        ...
        if secret {
            wvc_core::fs::set_permissions_owner_only(&tmp_path)?;
        }
        ...
        if path.exists() {
            ...
            if secret && bak_path.exists() {
                wvc_core::fs::set_permissions_owner_only(&bak_path)?;
            }
        }
