...
impl AntigravityProvider {
    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        wvc_base::provider::antigravity::load_persisted_catalog()
    }

    fn persist_catalog(snapshot: &CatalogSnapshot) {
        wvc_base::provider::antigravity::persist_catalog(snapshot);
    }
    ...
    async fn fetch_available_models(&self) -> Result<CatalogSnapshot> {
        wvc_base::provider::antigravity::fetch_catalog_snapshot(&self.client).await
    }
    ...
    async fn generate_content(
        ...
        signature_policy: wvc_provider_gemini::SignaturePolicy,
        ...
        let mut tools = wvc_provider_gemini::build_tools(tools);
        ...
        let request = CodeAssistGenerateRequest {
            ...
            request: VertexGenerateContentRequest {
                contents: wvc_provider_gemini::build_contents_with_signature_policy(
                    messages,
                    signature_policy,
                ),
                system_instruction: wvc_provider_gemini::build_system_instruction_with_tool_guard(
                    system,
                    !tools_is_empty,
                ),
                tools,
...
impl Provider for AntigravityProvider {
    async fn complete(
        ...
        let (tx, rx) = mpsc::channel::<Result<wvc_message_types::StreamEvent>>(100);
        ...
        tokio::spawn(async move {
            ...
            let mut signature_policy = wvc_provider_gemini::SignaturePolicy::ReplayCarriedForward;
                ...
                .await
            {
                ...
                Err(err) => {
                    ...
                    signature_policy =
                        wvc_provider_gemini::SignaturePolicy::DowngradeToolCallsToText;
                    match provider
                        .generate_content(
                            &model,
                            &messages,
            ...
            if let Some(content) = candidate.content {
                ...
                for part in content.parts {
                    ...
                    if let Some(function_call) = part.function_call {
                        ...
                        let call_id = wvc_message_types::sanitize_tool_id(&raw_call_id);
                        let _ = tx
            ...
            if !produced_output {
                ...
                if abnormal {
                    ...
                    let detail = candidate
                        ...
                        .map(|msg| format!(": {}", wvc_base::util::truncate_str(msg.trim(), 300)))
                        .unwrap_or_default();
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
    ...
    fn model_routes(&self) -> Vec<wvc_provider_core::ModelRoute> {
        ...
        if !catalog.is_empty() {
            return catalog
                .into_iter()
                .map(|model| wvc_provider_core::ModelRoute {
