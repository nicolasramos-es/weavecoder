...
fn wvc_home_test_lock() -> MutexGuard<'static, ()> {
...
#[test]
fn create_session_in_a_wvc_checkout_requests_selfdev() {
...
fn credential_provisioning_normalizes_gemini_and_supports_jcode() {
    ...
    let [Outbound::Legacy(notify)] = outbound.as_slice() else {
        panic!("wvc credential should notify the daemon: {outbound:?}");
    };
...
fn owner_only_writes_refuse_symlink_targets_and_directories() {
    ...
    assert!(!outside_dir.join("wvc-subscription.env").exists());
}
