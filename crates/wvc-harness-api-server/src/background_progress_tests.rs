...
fn the_daemons_own_formatter_round_trips() {
    ...
    use wvc_base::bus::BackgroundTaskProgressEvent;
    ...
    let event = BackgroundTaskProgressEvent {
        ...
        progress: BackgroundTaskProgress {
            ...
            message: Some("Running wvc-desktop2 tests".into()),
            eta_seconds: Some(90),
    ...
    let rendered = wvc_base::message::format_background_task_progress_markdown(&event);
    let parsed = parse(&rendered);
    assert_eq!(parsed.task_id, "t42");
    assert_eq!(parsed.label, "workspace tests");
    assert_eq!(parsed.percent, Some(37.0));
    assert!(
        parsed.summary.contains("Running wvc-desktop2 tests"),
        "the status line was lost: {rendered}"
    );
    assert!(!parsed.done);

    // And the completion shape, from the same source of truth.
    let done = wvc_base::message::format_background_task_notification_markdown(
        &wvc_base::bus::BackgroundTaskCompleted {
            session_id: "s1".into(),
            task_id: "t42".into(),
            tool_name: "bash".into(),
            display_name: Some("workspace tests".into()),
            status: wvc_background_types::BackgroundTaskStatus::Completed,
            exit_code: Some(0),
            duration_secs: 12.5,
