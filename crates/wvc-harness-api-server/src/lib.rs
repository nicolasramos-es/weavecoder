...
async fn handle_api_client(stream: Stream, legacy_socket: PathBuf) -> Result<()> {
    ...
    let hello_ok = ServerFrame::reply(
        ...
        ApiEvent::HelloOk {
            ...
            server: format!("wvc-harness-api-bridge/{}", env!("CARGO_PKG_VERSION")),
            capabilities: [
                "sessions",
                "streaming",
...
mod single_instance_tests {
    ...
    fn a_second_bridge_cannot_take_the_socket() {
        let dir = std::env::temp_dir().join(format!("wvc-bridge-lock-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let socket = dir.join("wvc-api.sock");
...
mod socket_permission_tests {
    ...
    async fn the_api_socket_is_owner_only() {
        ...
        let dir = std::env::temp_dir().join(format!("wvc-api-perm-{}", std::process::id()));
