...
use wvc_message_types::{CacheControl, ContentBlock, Message, Role, StreamEvent, ToolDefinition};
use wvc_provider_core::{EventStream, Provider};
pub use wvc_provider_openrouter::{
...
-> Option<wvc_base::provider_catalog::ResolvedOpenAiCompatibleProfile> {
    ...
    let mut matches = openai_compatible_profiles()
        ...
        .filter_map(|profile| {
            ...
            if wvc_base::provider_catalog::openai_compatible_profile_is_configured(*profile) {
                Some(resolved)
            } else {
                None
            }
...
fn load_named_profile_api_key(
    env_key: &str,
    profile: &wvc_base::config::NamedProviderConfig,
) -> Option<String> {
...
impl OpenRouterTransportState {
    ...
    fn runtime_provider_is_direct_compatible(runtime_provider: Option<&str>) -> bool {
        matches!(runtime_provider, Some("openai-compatible" | "azure-openai"))
            || runtime_provider
                .and_then(wvc_base::provider_catalog::openai_compatible_profile_by_id)
                .is_some()
    }
...
impl ProviderAuth {
    async fn apply(&self, req: reqwest::RequestBuilder) -> Result<reqwest::RequestBuilder> {
        match self {
            ...
            Self::AzureEntra { .. } => {
                let token = wvc_base::auth::azure::get_bearer_token().await?;
...
async fn fetch_models_from_api(
    ...
    let mut models = parse_openai_compatible_models_response(&raw_body).with_context(|| {
            format!(
                ...
                url,
                auth.label(),
                wvc_base::util::truncate_str(&raw_body.trim().replace('\n', "\\n"), 1200)
            )
        })?;
...
pub fn maybe_schedule_openai_compatible_profile_catalog_refresh(
    ...
    let previous_fingerprint =
        wvc_provider_openrouter::load_disk_cache_entry_for_namespace(&profile_id)
    ...
    handle.spawn(async move {
        ...
        let result = fetch_models_from_api(
            wvc_provider_core::shared_http_client(),
            api_base,
            auth,
            models_cache,
        ...
        match result {
            Ok(models) => {
                ...
                if updated {
                    ...
                    wvc_base::bus::Bus::global().publish_models_updated();
                } else {
                    wvc_base::logging::info(&format!(
            ...
            Err(error) => wvc_base::logging::info(&format!(
...
pub fn maybe_schedule_standard_openrouter_catalog_refresh(context: &'static str) -> bool {
    ...
    let cache_is_fresh = current_unix_secs()
        .zip(wvc_provider_openrouter::load_disk_cache_entry_for_namespace(namespace))
        .map(|(now, cache)| {
    ...
    let previous_fingerprint =
        wvc_provider_openrouter::load_disk_cache_entry_for_namespace(namespace)
            .map(|cache| models_fingerprint(&cache.models))
            .unwrap_or_default();
    handle.spawn(async move {
        let models_cache = Arc::new(RwLock::new(ModelsCache::default()));
...
impl OpenRouterProvider {
    ...
    ///
    /// Unlike the env-var based [`wvc_base::provider_catalog::runtime_provider_display_name`],
    ...
    pub fn runtime_display_name(&self) -> String {
        ...
        if !self.supports_provider_features {
            if let Some(profile_id) =
                wvc_base::provider_catalog::openai_compatible_profile_id_for_api_base(
                    &self.api_base,
                )
    ...
    pub fn new_named_openai_compatible(
        ...
        let auth = match profile.auth {
            wvc_base::config::NamedProviderAuth::None => ProviderAuth::None {
                label: "local endpoint (no auth)".to_string(),
            },
            wvc_base::config::NamedProviderAuth::Bearer => ProviderAuth::AuthorizationBearer {
                token: key
                    .ok_or_else(|| anyhow::anyhow!("{} not found in environment", key_label))?,
                label: key_label,
            },
            wvc_base::config::NamedProviderAuth::Header => ProviderAuth::HeaderValue {
        ...
        Ok(Self {
            ...
            supports_provider_features: matches!(
                ...
                wvc_base::config::NamedProviderType::OpenRouter
    ...
    fn is_kimi_model(model: &str) -> bool {
        wvc_provider_openrouter::is_kimi_model(model)
    }
    ...
    fn strip_session_profile_prefix<'a>(&self, model: &'a str) -> &'a str {
        ...
        if wvc_provider_core::explicit_model_provider_prefix(model).is_some() {
            return model;
        }
        let rest = rest.trim();
        if rest.is_empty() {
    ...
    pub fn new_openrouter_api_key_runtime() -> Result<Self> {
        let api_key = load_api_key_from_env_or_config(DEFAULT_API_KEY_NAME, DEFAULT_ENV_FILE)
            .ok_or_else(|| {
                let path = wvc_base::storage::app_config_dir()
    ...
    pub(crate) fn filter_profile_chat_supported_models(&self, models: Vec<String>) -> Vec<String> {
        ...
        models
            .into_iter()
            .filter(|model| {
                wvc_base::provider_catalog::openai_compatible_profile_model_supports_chat(
                    profile_id, model,
                )
            })
            .collect()
    ...
    fn model_disk_cache_source_matches(
        &self,
        cache_entry: &wvc_provider_openrouter::DiskCache,
    ) -> bool {
    ...
    fn maybe_schedule_model_catalog_refresh(&self, cache_age_secs: u64, context: &'static str) {
        ...
        handle.spawn(async move {
            match fetch_models_from_api(client, api_base, auth, models_cache, ns).await {
                ...
                Err(e) => wvc_base::logging::info(&format!(
    ...
    fn parse_provider_routing() -> ProviderRouting {
        wvc_provider_openrouter::parse_provider_routing_from_env()
    }
    ...
    fn rank_providers_from_endpoints(endpoints: &[EndpointInfo]) -> Vec<String> {
        wvc_provider_openrouter::rank_providers_from_endpoints(endpoints)
    }
    ...
    pub fn has_credentials() -> bool {
        if matches!(
            ...
            return wvc_base::auth::azure::has_configuration();
        }
        if configured_allow_no_auth() {
            return true;
        }
