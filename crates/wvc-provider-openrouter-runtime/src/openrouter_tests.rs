...
impl SharedEnvLock {
    ...
    pub(crate) fn lock(&self) -> std::sync::MutexGuard<'static, ()> {
        wvc_base::storage::lock_test_env()
    }
}
...
impl EnvVarGuard {
    pub(crate) fn set(key: &'static str, value: impl AsRef<std::ffi::OsStr>) -> Self {
        ...
        wvc_base::env::set_var(key, value);
        Self { key, previous }
    }

    pub(crate) fn remove(key: &'static str) -> Self {
        let previous = std::env::var_os(key);
        wvc_base::env::remove_var(key);
        Self { key, previous }
    }
}
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(previous) = &self.previous {
            wvc_base::env::set_var(self.key, previous);
        } else {
            wvc_base::env::remove_var(self.key);
        }
    }
}
...
fn write_test_api_key(temp: &TempDir, env_file: &str, env_key: &str, value: &str) {
    let config_dir = test_config_dir(temp).join("wvc");
...
fn named_openai_compatible_provider_exposes_static_models_as_routes() {
    ...
    let profile = wvc_base::config::NamedProviderConfig {
        ...
        models: vec![wvc_base::config::NamedProviderModelConfig {
...
fn named_openai_compatible_provider_uses_per_model_image_input_support() {
    ...
    let profile = wvc_base::config::NamedProviderConfig {
        ...
        models: vec![
            wvc_base::config::NamedProviderModelConfig {
...
fn minimax_profile_exposes_static_models_before_catalog_refresh() {
    let models = wvc_base::provider_catalog::openai_compatible_profile_static_models(
        wvc_provider_metadata::MINIMAX_PROFILE,
    );
...
fn cerebras_profile_exposes_live_chat_models_before_catalog_refresh() {
    assert_eq!(
        wvc_provider_metadata::CEREBRAS_PROFILE.default_model,
        Some("gpt-oss-120b")
    );

    let models = wvc_base::provider_catalog::openai_compatible_profile_static_models(
        wvc_provider_metadata::CEREBRAS_PROFILE,
    );
...
fn openai_compatible_profiles_with_unverified_live_catalogs_have_static_fallbacks() {
    let cases = [
        (wvc_provider_metadata::OPENCODE_PROFILE, "minimax-m2.7"),
        (wvc_provider_metadata::OPENCODE_GO_PROFILE, "kimi-k2.5"),
        (wvc_provider_metadata::ZAI_PROFILE, "glm-4.7"),
        (
            wvc_provider_metadata::AI302_PROFILE,
            "qwen3-235b-a22b-instruct-2507",
        ),
        (wvc_provider_metadata::BASETEN_PROFILE, "zai-org/GLM-4.7"),
        (wvc_provider_metadata::CORTECS_PROFILE, "kimi-k2.5"),
        (wvc_provider_metadata::KIMI_PROFILE, "kimi-for-coding"),
        (wvc_provider_metadata::FIRMWARE_PROFILE, "kimi-k2.5"),
        (
            wvc_provider_metadata::HUGGING_FACE_PROFILE,
        ...
        (
            wvc_provider_metadata::STACKIT_PROFILE,
            "openai/gpt-oss-120b",
        ),
        (wvc_provider_metadata::PERPLEXITY_PROFILE, "sonar"),
        (
            wvc_provider_metadata::DEEPINFRA_PROFILE,
            "moonshotai/Kimi-K2-Instruct",
        ),
        (
            wvc_provider_metadata::FIREWORKS_PROFILE,
            "accounts/fireworks/routers/kimi-k2p5-turbo",
        ),
        (wvc_provider_metadata::XIAOMI_MIMO_PROFILE, "mimo-v2.5"),
        (
            wvc_provider_metadata::ALIBABA_CODING_PLAN_PROFILE,
            "qwen3-coder-plus",
        ),
    ];

    for (profile, expected_model) in cases {
        let models = wvc_base::provider_catalog::openai_compatible_profile_static_models(profile);
        assert!(
...
fn autodetects_single_saved_openai_compatible_profile() {
    ...
    let opencode = wvc_base::provider_catalog::resolve_openai_compatible_profile(
...
fn autodetects_single_saved_local_openai_compatible_profile() {
    ...
    let lmstudio = wvc_base::provider_catalog::resolve_openai_compatible_profile(
        wvc_base::provider_catalog::LMSTUDIO_PROFILE,
    );
    let config_dir = test_config_dir(&temp).join("wvc");
    std::fs::create_dir_all(&config_dir).expect("create test config dir");
    std::fs::write(
        ...
        format!(
            "{}=1\n",
            wvc_base::provider_catalog::OPENAI_COMPAT_LOCAL_ENABLED_ENV
        ),
    )
...
fn does_not_guess_when_multiple_saved_openai_compatible_profiles_exist() {
    ...
    let chutes = wvc_base::provider_catalog::resolve_openai_compatible_profile(
        wvc_base::provider_catalog::CHUTES_PROFILE,
    );
    write_test_api_key(
        &temp,
        &opencode.env_file,
...
fn autodetected_profile_seeds_default_model_and_cache_namespace() {
    ...
    let zai = wvc_base::provider_catalog::resolve_openai_compatible_profile(
        wvc_base::provider_catalog::ZAI_PROFILE,
    );
...
fn runtime_display_name_for_profile_runtime_instance() {
    ...
    let nim = OpenRouterProvider::new_openai_compatible_profile_runtime(
        wvc_base::provider_catalog::NVIDIA_NIM_PROFILE,
    )
...
#[test]
fn custom_endpoint_using_wvc_key_name_is_not_a_subscription_runtime() {
    let _lock = ENV_LOCK.lock();
    let temp = TempDir::new().expect("create temp home");
    let wvc_home = temp.path().join("wvc-home");
    let _wvc_home = EnvVarGuard::set("JCODE_HOME", &wvc_home);
...
fn named_profile_construction_reads_openai_reasoning_effort_config() {
    ...
    let configured = wvc_base::config::config()
        .provider
...
/// `wvc run` (which binds profile_id in-process) works fine.
#[test]
fn user_named_profile_prefix_is_stripped_even_without_profile_id() {
    ...
    std::fs::create_dir_all(&wvc_home).expect("create test config dir");
    std::fs::write(
        wvc_home.join("config.toml"),
        r#"
[provider]
...
"#,
    ...
    wvc_base::config::invalidate_config_cache();
