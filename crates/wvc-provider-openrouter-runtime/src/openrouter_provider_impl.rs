...
use super::*;
use wvc_base::provider::{ModelCatalogRefreshSummary, summarize_model_catalog_refresh};
...
impl Provider for OpenRouterProvider {
    ...
    async fn complete(
        ...
        let api_messages = wvc_provider_openrouter::request::build_chat_messages(
            &effective_messages,
            system,
            allow_reasoning,
            ...
            .map(|t| {
                serde_json::json!({
                    ...
                    "function": {
                        ...
                        "parameters": wvc_provider_openrouter::request::sanitize_tool_parameters_schema(&t.input_schema),
                    }
                })
            })
            .collect();
        ...
        if let Some(effort) = reasoning_effort.as_deref() {
            if self.supports_deepseek_reasoning_effort() {
                ...
                let effort = if wvc_base::prompt::is_swarm_effort(effort) {
                    "max"
                } else {
                    effort
                };
        ...
        wvc_base::logging::info("OpenRouter transport: HTTPS (SSE)");
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let (model_id, provider) = if self.supports_provider_features {
            ...
            let model_id = if provider.is_some() {
                wvc_base::provider::openrouter_catalog_model_id(&model_id).unwrap_or(model_id)
            } else {
                model_id
            };
            (model_id, provider)
        ...
        if let Some(stored_effort) = stored_effort
            && !wvc_base::prompt::is_swarm_effort(&stored_effort)
    ...
    fn available_efforts(&self) -> Vec<&'static str> {
        if self.supports_deepseek_reasoning_effort() {
            wvc_provider_core::DEEPSEEK_SELECTABLE_EFFORTS.to_vec()
        } else if self.supports_openai_reasoning_effort() {
            ...
            wvc_provider_core::OPENROUTER_SELECTABLE_EFFORTS.to_vec()
        } else {
            vec![]
        }
    }
    ...
    fn model_routes(&self) -> Vec<wvc_provider_core::ModelRoute> {
        ...
        self.available_models_display()
            .into_iter()
            .filter(|model| wvc_base::provider::is_listable_model_name(model))
            .map(|model| {
    ...
    fn context_window(&self) -> usize {
        ...
        wvc_provider_core::context_limit_for_model_with_provider(&model_id, Some(self.name()))
...
impl OpenRouterProvider {
    ...
    pub(crate) fn load_usable_model_disk_cache_entry(
        &self,
    ) -> Option<wvc_provider_openrouter::DiskCache> {
    ...
    pub(crate) fn load_disk_cache_entry_for_this_profile(
        ...
        match self.foreground_cache_namespace() {
            ...
            None => wvc_provider_openrouter::load_disk_cache_entry(),
        }
    }
    ...
    pub(crate) fn is_user_named_profile(&self) -> bool {
        ...
        match wvc_base::provider_catalog::openai_compatible_profile_by_id(id) {
