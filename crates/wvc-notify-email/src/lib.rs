...
pub async fn send_email(request: SendEmailRequest<'_>) -> Result<()> {
    ...
    if let Some(cid) = request.cycle_id {
        let msg_id = format!("<ambient-{}@wvc>", cid);
...
pub fn poll_imap_once(host: &str, port: u16, user: &str, pass: &str) -> Result<Vec<ReplyAction>> {
    ...
    let reply_search = session.search("UNSEEN HEADER In-Reply-To \"@wvc>\"")?;
    let button_search = session.search("UNSEEN SUBJECT \"[wvc-perm:\"")?;
    ...
    for message in messages.iter() {
            ...
            && let Some(parsed) = mail_parser::MessageParser::default().parse(body)
        {
            ...
            let cycle_id = if in_reply_to.contains("@wvc>") {
                in_reply_to
                    .trim_start_matches("<ambient-")
                    .trim_end_matches("@wvc>")
                    .to_string()
            } else if let Some(start) = subject.find("[wvc-perm:") {
                let rest = &subject[start + "[wvc-perm:".len()..];
...
pub fn build_permission_email_html(
    ...
    let approve_subj_raw = format!("[wvc-perm:{}] Approved", request_id);
    let deny_subj_raw = format!("[wvc-perm:{}] Denied", request_id);
...
mod tests {
    ...
    fn test_build_permission_email_html() {
        ...
        assert!(html.contains("mailto:wvc@example.com"));
    }
}
