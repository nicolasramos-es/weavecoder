...
pub fn add_message_cache_breakpoint(messages: &mut [ApiMessage], cache_ttl_1h: bool) {
    ...
    if messages.len() < 3 {
        ...
        wvc_logging::info("Conversation caching: too few messages, skipping");
        return;
    }
    ...
    if assistant_indices.is_empty() {
        wvc_logging::info("Conversation caching: no assistant message found");
        return;
    }
...
mod cache_prefix_invariant_tests {
    ...
    use super::*;
    use wvc_message_types::{ContentBlock, Message, Role};
