...
fn auth_issue_runtime_display_name_tracks_direct_compatible_profiles() {
    ...
    crate::env::set_var("WVC_RUNTIME_PROVIDER", "azure-openai");
...
fn auth_profile_env_application_flushes_stale_openrouter_catalog_state() {
    ...
    crate::env::set_var("WVC_OPENROUTER_API_BASE", "https://openrouter.ai/api/v1");
    crate::env::set_var("WVC_OPENROUTER_API_KEY_NAME", "OPENROUTER_API_KEY");
    crate::env::set_var("WVC_OPENROUTER_ENV_FILE", "openrouter.env");
    crate::env::set_var("WVC_OPENROUTER_CACHE_NAMESPACE", "openrouter");
    crate::env::set_var("WVC_OPENROUTER_PROVIDER_FEATURES", "1");
    crate::env::set_var("WVC_OPENROUTER_TRANSPORT_STATE", "stale");
    crate::env::set_var("WVC_OPENROUTER_ALLOW_NO_AUTH", "1");
    crate::env::set_var(
        "WVC_OPENROUTER_MODEL_CATALOG",
        "stale-openrouter-catalog.json",
    );
    crate::env::set_var("WVC_OPENROUTER_MODEL", "gpt-5.5");
    crate::env::set_var(
        "WVC_OPENROUTER_STATIC_MODELS",
        "stale-openrouter-only-model",
    );
    crate::env::set_var("WVC_OPENROUTER_AUTH_HEADER", "Bearer stale");
    crate::env::set_var("WVC_OPENROUTER_AUTH_HEADER_NAME", "Authorization");
    crate::env::set_var("WVC_OPENROUTER_DYNAMIC_BEARER_PROVIDER", "openrouter");
    crate::env::set_var("WVC_OPENROUTER_PROVIDER", "openrouter");
    crate::env::set_var("WVC_OPENROUTER_NO_FALLBACK", "1");
    crate::env::set_var("WVC_NAMED_PROVIDER_PROFILE", "openrouter");
    crate::env::set_var("WVC_PROVIDER_PROFILE_ACTIVE", "1");
    crate::env::set_var("WVC_PROVIDER_PROFILE_NAME", "openrouter");
    ...
    assert_eq!(
        std::env::var("WVC_OPENROUTER_PROVIDER_FEATURES").as_deref(),
        Ok("0")
    );
    assert_eq!(
        std::env::var("WVC_OPENROUTER_TRANSPORT_STATE").as_deref(),
        Ok("direct-api-key")
    );
    assert!(std::env::var_os("WVC_OPENROUTER_ALLOW_NO_AUTH").is_none());
    assert!(std::env::var_os("WVC_OPENROUTER_MODEL_CATALOG").is_none());
    assert!(std::env::var_os("WVC_OPENROUTER_MODEL").is_none());
    assert!(std::env::var_os("WVC_OPENROUTER_AUTH_HEADER").is_none());
    assert!(std::env::var_os("WVC_OPENROUTER_AUTH_HEADER_NAME").is_none());
    assert!(std::env::var_os("WVC_OPENROUTER_DYNAMIC_BEARER_PROVIDER").is_none());
    assert!(std::env::var_os("WVC_OPENROUTER_PROVIDER").is_none());
    assert!(std::env::var_os("WVC_OPENROUTER_NO_FALLBACK").is_none());
    assert!(std::env::var_os("WVC_NAMED_PROVIDER_PROFILE").is_none());
    assert!(std::env::var_os("WVC_PROVIDER_PROFILE_ACTIVE").is_none());
    assert!(std::env::var_os("WVC_PROVIDER_PROFILE_NAME").is_none());
    assert_ne!(
        std::env::var("WVC_OPENROUTER_STATIC_MODELS")
            .ok()
            .as_deref(),
...
fn matrix_openrouter_like_sources_include_all_static_profiles() {
    ...
    crate::env::remove_var("WVC_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("WVC_OPENAI_COMPAT_ENV_FILE");
...
fn matrix_openrouter_like_sources_accept_valid_overrides() {
    ...
    crate::env::set_var("WVC_OPENROUTER_API_KEY_NAME", "ALT_OPENROUTER_KEY");
    crate::env::set_var("WVC_OPENROUTER_ENV_FILE", "alt-openrouter.env");
    crate::env::set_var("WVC_OPENAI_COMPAT_API_KEY_NAME", "ALT_COMPAT_KEY");
    crate::env::set_var("WVC_OPENAI_COMPAT_ENV_FILE", "alt-compat.env");
...
fn named_provider_profile_maps_to_openai_compatible_runtime_env() {
    ...
    assert_eq!(
        std::env::var("WVC_OPENROUTER_TRANSPORT_STATE")
            .ok()
            .as_deref(),
        Some("direct-api-key")
    );
    assert_eq!(
        std::env::var("WVC_OPENROUTER_MODEL_CATALOG")
            .ok()
            .as_deref(),
        Some("0")
    );
    ...
    assert_eq!(
        std::env::var("WVC_OPENROUTER_AUTH_HEADER_NAME")
            .ok()
            .as_deref(),
        Some("x-api-key")
    );
...
fn named_provider_inline_api_key_is_private_runtime_fallback() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "WVC_OPENROUTER_MODEL_CATALOG",
        "WVC_NAMED_PROVIDER_PROFILE",
        "WVC_PROVIDER_MY_GATEWAY_API_KEY",
    ]);
    ...
    assert_eq!(
        std::env::var("WVC_PROVIDER_MY_GATEWAY_API_KEY")
            .ok()
            .as_deref(),
        Some("inline-secret")
    );
...
fn matrix_openrouter_like_sources_reject_invalid_overrides() {
    ...
    crate::env::set_var("WVC_OPENROUTER_API_KEY_NAME", "bad-key-name");
    crate::env::set_var("WVC_OPENROUTER_ENV_FILE", "../bad.env");
    crate::env::set_var("WVC_OPENAI_COMPAT_API_KEY_NAME", "bad key");
    crate::env::set_var("WVC_OPENAI_COMPAT_ENV_FILE", "../bad-compat.env");
...
fn matrix_openai_compatible_profile_overrides_apply_when_valid() {
    ...
    crate::env::set_var("WVC_OPENAI_COMPAT_API_KEY_NAME", "GROQ_API_KEY");
    crate::env::set_var("WVC_OPENAI_COMPAT_ENV_FILE", "groq.env");
    crate::env::set_var("WVC_OPENAI_COMPAT_DEFAULT_MODEL", "openai/gpt-oss-120b");
...
fn matrix_openai_compatible_profile_overrides_reject_invalid_values() {
    ...
    crate::env::set_var("WVC_OPENAI_COMPAT_API_BASE", "http://example.com/v1");
    crate::env::set_var("WVC_OPENAI_COMPAT_API_KEY_NAME", "bad-key-name");
    crate::env::set_var("WVC_OPENAI_COMPAT_ENV_FILE", "../bad.env");
...
fn matrix_openai_compatible_profile_overrides_read_from_env_file() {
    ...
    crate::env::set_var("WVC_HOME", temp.path());
    crate::env::remove_var("WVC_OPENAI_COMPAT_API_BASE");
    crate::env::remove_var("WVC_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("WVC_OPENAI_COMPAT_ENV_FILE");
    crate::env::remove_var("WVC_OPENAI_COMPAT_DEFAULT_MODEL");
    std::fs::write(
        config_root.join(OPENAI_COMPAT_PROFILE.env_file),
        concat!(
            "WVC_OPENAI_COMPAT_API_BASE=https://api.example.com/v1\n",
            "WVC_OPENAI_COMPAT_API_KEY_NAME=EXAMPLE_API_KEY\n",
            "WVC_OPENAI_COMPAT_ENV_FILE=example.env\n",
            "WVC_OPENAI_COMPAT_DEFAULT_MODEL=example/model\n",
        ),
    )
    .expect("env file");
...
fn matrix_openai_compatible_localhost_override_allows_no_auth() {
    ...
    crate::env::set_var("WVC_OPENAI_COMPAT_API_BASE", "http://localhost:11434/v1");
    crate::env::remove_var("WVC_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("WVC_OPENAI_COMPAT_ENV_FILE");
    crate::env::remove_var("WVC_OPENAI_COMPAT_LOCAL_ENABLED");
...
fn matrix_load_api_key_from_env_or_config_prefers_env() {
    ...
    let _guard = EnvGuard::save(&["WVC_HOME", "OPENCODE_API_KEY"]);
    crate::env::set_var("WVC_HOME", temp.path());
...
fn load_api_key_accepts_legacy_zai_key_name() {
    ...
    let _guard = EnvGuard::save(&["WVC_HOME", "ZHIPU_API_KEY", "ZAI_API_KEY"]);
    crate::env::set_var("WVC_HOME", temp.path());
...
fn newest_release_picker_prefers_strongest_tier_over_newest_cheap() {
    ...
    let _lock = crate::storage::lock_test_env();
    let _env = EnvGuard::save(&["WVC_HOME"]);
    let temp = tempfile::tempdir().expect("tempdir");
    crate::env::set_var("WVC_HOME", temp.path());
...
fn every_static_profile_model_has_a_known_context_limit() {
    use wvc_provider_core::models::context_limit_for_model_with_provider;
    ...
    for profile in wvc_provider_metadata::openai_compatible_profiles()
        .iter()
        .copied()
    {
...
fn open_weight_family_context_limits_match_published_windows() {
    use wvc_provider_core::models::open_weight_family_context_limit as f;
