...
pub struct ModelRefreshCompleted {
    pub session_id: String,
    pub result: std::result::Result<wvc_provider_core::ModelCatalogRefreshSummary, String>,
}
...
/// background tasks, without re-coupling `wvc-base` to swarm internals.
#[derive(Clone, Debug)]
...
pub enum BusEvent {
    ...
    UsageReport(Vec<wvc_usage_types::ProviderUsage>),
    /// Progressive usage report update while providers are still loading
    UsageReportProgress(wvc_usage_types::ProviderUsageProgress),
