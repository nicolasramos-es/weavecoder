...
impl GeneratedImagePanelInfo {
    fn from_inputs(
        ...
        Self {
            path: path.to_string(),
            file_label: wvc_terminal_image::metadata::compact_path_label(path),
    ...
    fn title(&self) -> String {
        ...
        if let Some((width, height)) = self.dimensions {
            title.push_str(" · ");
            title.push_str(&wvc_terminal_image::metadata::format_dimensions(
                width, height,
            ));
        }
        title
    ...
    fn summary_parts(&self) -> Vec<String> {
        ...
        if let Some((width, height)) = self.dimensions {
            parts.push(wvc_terminal_image::metadata::format_dimensions(
                width, height,
            ));
        }
        parts.push(wvc_terminal_image::metadata::compact_image_format(
            &self.output_format,
        ));
    ...
    fn markdown(&self) -> String {
        ...
        if let Some((width, height)) = self.dimensions {
            let mut dimensions = wvc_terminal_image::metadata::format_dimensions(width, height);
            if let Some(ratio) = wvc_terminal_image::metadata::aspect_ratio(width, height) {
        ...
        markdown.push_str(&format!(
            "- Format: {}\n",
            markdown_code(&wvc_terminal_image::metadata::compact_image_format(
                &self.output_format,
            ))
        ));
        if let Some(byte_count) = self.byte_count {
...
mod tests {
    ...
    fn generated_image_side_panel_markdown_prefers_compact_useful_metadata() {
        let dir = std::env::temp_dir().join(format!(
            "wvc-generated-image-side-panel-test-{}",
            std::process::id()
        ));
    ...
    fn generated_image_side_panel_title_includes_filename_and_dimensions() {
        let dir = std::env::temp_dir().join(format!(
            "wvc-generated-image-title-test-{}",
            std::process::id()
        ));
        std::fs::create_dir_all(&dir).expect("create temp generated image dir");
