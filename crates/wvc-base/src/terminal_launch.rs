...
///
/// Resolution order: `WVC_SPAWN_HOOK` env (handled by config env overrides;
...
///
/// When a spawn hook is configured (`[terminal] spawn_hook` / `WVC_SPAWN_HOOK`),
/// the hook takes over the spawn: wvc runs `<hook> <program> <args...>` with
/// `WVC_SPAWN_*` metadata env vars so external programs (tmux, kitty remote,
/// herd, window managers) control where and how the session appears. If the
/// hook cannot be started, wvc falls back to its built-in terminal detection.
...
mod tests {
    ...
    fn spawn_via_hook_runs_hook_with_metadata_env() {
        ...
        std::fs::write(
            &hook_path,
            format!(
                "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$WVC_SPAWN_KIND\" \"$WVC_SPAWN_SESSION_ID\" \"$WVC_SPAWN_SWARM_ID\" \"$*\" > {}\n",
        ...
        .spawn_env("WVC_SPAWN_SWARM_ID", "swarm-7");
        ...
        assert_eq!(
            recorded, "swarm-agent|ses_hooked|swarm-7|/usr/local/bin/wvc --resume ses_hooked",
            "hook should receive metadata env and the wvc command as argv"
        );
    }
}
