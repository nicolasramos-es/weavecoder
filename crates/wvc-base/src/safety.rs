...
impl SafetySystem {
    ...
    /// Persist a transcript to ~/.wvc/ambient/transcripts/{timestamp}.json
    pub fn save_transcript(&self, transcript: &AmbientTranscript) -> Result<()> {
        let dir = storage::wvc_dir()?.join("ambient").join("transcripts");
...
fn queue_path() -> Result<std::path::PathBuf> {
    Ok(storage::wvc_dir()?.join("safety").join("queue.json"))
}

fn history_path() -> Result<std::path::PathBuf> {
    Ok(storage::wvc_dir()?.join("safety").join("history.json"))
}
