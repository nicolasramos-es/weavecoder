...
impl MemoryManager {
    ...
    fn project_memory_path(&self) -> Result<Option<PathBuf>> {
        ...
        let memory_dir = storage::wvc_dir()?.join("memory").join("projects");
        Ok(Some(memory_dir.join(format!("{}.json", project_hash))))
    }

    fn legacy_notes_path(&self) -> Result<Option<PathBuf>> {
        if self.test_mode {
            let test_dir = storage::wvc_dir()?.join("notes").join("test");
            std::fs::create_dir_all(&test_dir)?;
    ...
    fn global_memory_path(&self) -> Result<PathBuf> {
        if self.test_mode {
            ...
            Ok(storage::wvc_dir()?.join("memory").join("global.json"))
        }
    }
