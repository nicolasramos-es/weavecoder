...
//!   `~/.wvc/logs/memory-events-YYYY-MM-DD.jsonl`
//!
...
fn log_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".wvc").join("logs"))
}
...
pub fn log_pending_prepared(session_id: &str, prompt: &str, count: usize, memory_ids: &[String]) {
    write_log(
        ...
        Some(serde_json::json!({
            ...
            "prompt_preview": wvc_core::util::truncate_str(prompt, 500),
...
pub fn log_candidate_filter(
    ...
    write_log(
        ...
        Some(serde_json::json!({
            ...
            "context_preview": wvc_core::util::truncate_str(context_preview, 200),
        })),
    );
}
...
mod tests {
    ...
    fn memory_log_cleanup_respects_14_day_window() {
        ...
        let other = write("wvc-2000-01-01.log", 20);
