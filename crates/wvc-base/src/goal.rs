...
pub use wvc_task_types::{Goal, GoalMilestone, GoalScope, GoalStatus, GoalStep, GoalUpdate};
...
pub fn create_goal(input: GoalCreateInput, working_dir: Option<&Path>) -> Result<Goal> {
    ...
    if let Some(id) = input.id.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        goal.id = wvc_task_types::sanitize_goal_id(id);
    }
...
pub fn load_goal(
    ...
    let id = wvc_task_types::sanitize_goal_id(id);
...
pub fn goal_page_id(id: &str) -> String {
    format!("goal.{}", wvc_task_types::sanitize_goal_id(id))
}
...
fn goal_file_in_dir(dir: &Path, id: &str) -> PathBuf {
    dir.join(format!("{}.json", wvc_task_types::sanitize_goal_id(id)))
}

fn global_goals_dir() -> Result<PathBuf> {
    Ok(crate::storage::wvc_dir()?.join("goals").join("global"))
}
...
fn next_available_goal_id(
    ...
    let mut candidate = wvc_task_types::sanitize_goal_id(base);
    let mut idx = 2;
    while load_goal(&candidate, Some(scope), working_dir)?.is_some() {
        candidate = format!("{}-{}", wvc_task_types::sanitize_goal_id(base), idx);
        idx += 1;
    }
    Ok(candidate)
}
