...
#[cfg(target_os = "linux")]
const KILLALL_PROCESS_NAME: &str = "wvc";
...
pub fn set_server_title(server_name: &str) {
    set_title(compact_process_title("wvc:s:", Some(server_name)));
}
...
pub fn set_client_display_title(session_name: &str, is_selfdev: bool) {
    let prefix = if is_selfdev { "wvc:d:" } else { "wvc:c:" };
...
pub fn set_client_remote_display_title(server_name: &str, session_name: &str, is_selfdev: bool) {
    if server_name.is_empty() || server_name.eq_ignore_ascii_case("wvc") {
