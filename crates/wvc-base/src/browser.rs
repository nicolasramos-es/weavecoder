...
const REQUIRED_BRIDGE_ACTION_PROBES: &[(&str, &str)] = &[
    ...
    (
        "uploadFile",
        r#"{"selector":"input[type=file]","filePath":"/tmp/wvc-browser-capability-probe"}"#,
    ),
];

fn wvc_dir() -> PathBuf {
    storage::wvc_dir().unwrap_or_else(|_| {
        dirs::home_dir()
...
pub async fn ensure_browser_setup() -> Result<String> {
    ...
    if initial_status.responding && !initial_status.compatible {
        log.push_str("Browser bridge is connected, but the live Firefox extension is out of date for this wvc build. Attempting repair steps...\n");
    ...
    match check_browser_ping().await {
        ...
        Ok(false) => {
            ...
            if should_prompt_extension_install(&initial_status) {
                    ...
                    Ok(msg) => {
                        ...
                        match wait_for_ping(15).await {
                            ...
                            Ok(false) => {
                                log.push_str("timed out\n");
                                log.push_str(
                                    "       Extension not detected. You can retry with: wvc browser setup\n",
                                );
                                log.push_str(
                ...
                log.push_str(
                    "       Make sure Firefox is running with the Browser Agent Bridge extension enabled, then re-run `wvc browser status`.\n",
                );
            }
        }
        Err(e) => {
    ...
    if final_status.ready {
        ...
        log.push_str("\nSetup is not complete yet. The Firefox extension is connected, but it is still missing required actions for this wvc build.\n");
        ...
        log.push_str("Use `wvc browser status` to verify readiness after updating the extension in Firefox.\n");
        ...
        log.push_str(
            "Use `wvc browser status` to re-check readiness after any manual Firefox step.\n",
        );
    } else {
...
fn install_native_host_manifest() -> Result<bool> {
    ...
    let manifest = serde_json::json!({
        ...
        "description": "Native host for Firefox Agent Bridge (managed by wvc)",
        "path": effective_host,
        "type": "stdio",
        "allowed_extensions": [
            EXTENSION_ID_LOCAL,
