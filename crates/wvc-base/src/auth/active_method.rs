...
//! [`AuthStatus`] plus the `WVC_RUNTIME_PROVIDER` override on its own, keyed on
...
use wvc_provider_core::ActiveProvider;
...
impl From<wvc_provider_core::ResolvedCredential> for ActiveCredential {
    fn from(value: wvc_provider_core::ResolvedCredential) -> Self {
        match value {
            wvc_provider_core::ResolvedCredential::Oauth => Self::OAuth,
            wvc_provider_core::ResolvedCredential::ApiKey => Self::ApiKey,
        }
    }
}
...
pub struct ResolvedProviderAuth {
    ...
    /// True when the active credential was pinned via `WVC_RUNTIME_PROVIDER`
...
///
/// `runtime_provider` is the raw `WVC_RUNTIME_PROVIDER` value (if any); it
...
pub fn resolve_dual_credential_auth(
    ...
    let dual = wvc_provider_core::DualAuthProvider::from_active_provider(provider)?;
    ...
    let forced =
        wvc_provider_core::pinned_mode_for(dual, runtime_provider).map(|mode| match mode {
            wvc_provider_core::AuthMode::Oauth => ActiveCredential::OAuth,
            wvc_provider_core::AuthMode::ApiKey => ActiveCredential::ApiKey,
        });
