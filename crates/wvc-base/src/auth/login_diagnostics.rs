...
pub fn auth_failure_recovery_hint(provider_id: &str, reason: AuthFailureReason) -> Option<String> {
    ...
    let hint = match reason {
        ...
        | AuthFailureReason::NonInteractiveTerminal => format!(
            "Try a manual-safe fallback: `wvc login --provider {} --print-auth-url`, then complete with `--callback-url` or `--auth-code`.",
            provider
        ),
        AuthFailureReason::ManualInputMissing => {
        ...
        AuthFailureReason::SaveFailed => {
            "Check whether wvc can write its config directory, or retry inside an isolated sandbox with `bash scripts/onboarding_sandbox.sh fresh`.".to_string()
        }
        AuthFailureReason::PostLoginValidationFailed => format!(
            "Credentials were saved, but runtime verification failed. Run `wvc auth-test --provider {}` and `wvc auth doctor {}` for guided diagnosis.",
            provider, provider
        ),
        AuthFailureReason::ImportUnavailable => {
        ...
        AuthFailureReason::RateLimited => format!(
            "The provider accepted the browser callback but rate-limited the token exchange. Wait before retrying, avoid repeated immediate attempts, and keep using existing credentials if they still validate with `wvc auth doctor {} --validate`.",
            provider
        ),
        AuthFailureReason::OAuthExchangeFailed => format!(
            "Retry the OAuth flow, and if it keeps failing use `wvc login --provider {} --print-auth-url` so the callback can be completed manually.",
            provider
        ),
        AuthFailureReason::Unknown => {
            "Run `wvc auth status`, then `wvc auth doctor` for a structured diagnosis.".to_string()
        }
    };
