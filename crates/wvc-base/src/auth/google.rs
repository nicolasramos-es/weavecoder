...
pub fn credentials_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::wvc_dir()?.join("google_credentials.json"))
}

pub fn tokens_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::wvc_dir()?.join("google_oauth.json"))
}
...
pub fn load_tokens() -> Result<GoogleTokens> {
    ...
    if !path.exists() {
        anyhow::bail!("No Google tokens found. Run `wvc login google` first.");
    }
    crate::storage::harden_secret_file_permissions(&path);
    crate::storage::read_json(&path)
        .map_err(|_| anyhow::anyhow!("No Google tokens found. Run `wvc login google` first."))
}
