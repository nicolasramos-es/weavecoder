...
fn globally_preferred_model_rank(model: &str) -> (u8, usize) {
    ...
    let openai_default = normalize_model_for_preference(wvc_provider_core::DEFAULT_OPENAI_MODEL);
    let claude_default = normalize_model_for_preference(wvc_provider_core::DEFAULT_CLAUDE_MODEL);
...
fn normalize_model_for_preference(model: &str) -> String {
    let mut id = wvc_provider_core::model_id::canonical(model);
    ...
    id = wvc_provider_core::model_id::strip_date_suffix(&id).to_string();
...
/// `wvc_provider_core::auto_default_provider`: Claude wins when both families
