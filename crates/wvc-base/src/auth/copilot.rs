...
pub const EDITOR_VERSION: &str = "wvc/1.0";
pub const EDITOR_PLUGIN_VERSION: &str = "wvc/1.0";
...
/// 8. optional `gh auth token` fallback when WVC_COPILOT_ALLOW_GH_AUTH_TOKEN=1
pub fn load_github_token() -> Result<String> {
    ...
    anyhow::bail!(
        "GitHub Copilot token not found. \
         Set COPILOT_GITHUB_TOKEN/GH_TOKEN/GITHUB_TOKEN, run `wvc login --provider copilot`, \
         or set WVC_COPILOT_ALLOW_GH_AUTH_TOKEN=1 to explicitly reuse `gh auth token`."
    )
}

fn allow_gh_cli_fallback() -> bool {
    std::env::var("WVC_COPILOT_ALLOW_GH_AUTH_TOKEN")
        .ok()
        .map(|value| {
...
pub fn save_github_token(token: &str, username: &str) -> Result<()> {
    ...
    // A token written by wvc's own device-login flow should be immediately
