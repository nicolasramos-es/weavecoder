...
fn ensure_claude_inference_scope(scopes: &[String], action: &str) -> Result<()> {
    ...
    anyhow::bail!(
        "Claude OAuth {} returned a token without the required user:inference scope (scopes: {}). Re-run `wvc login --provider claude` so wvc opens the Claude.ai OAuth flow, or import/use a fresh Claude Code login.",
        action,
        scopes.join(" ")
    )
}
...
fn bad_request_response(message: &str) -> String {
    let body = format!(
        "<html><body><h1>Authentication not completed</h1><p>{}</p><p>You can close this tab and return to wvc.</p></body></html>",
        message
    );
    format!(
...
pub async fn wait_for_callback_async_on_listener(
    ...
    loop {
        ...
        let body = "<html><body><h1>Success!</h1><p>You can close this window and return to wvc.</p></body></html>";
        let response = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: close\r\nContent-Length: {}\r\n\r\n{}",
            body.len(),
            body
...
pub async fn login_claude(no_browser: bool) -> Result<OAuthTokens> {
    ...
    if let Ok(code) = std::env::var("WVC_CLAUDE_AUTH_CODE") {
        let trimmed = code.trim();
        if trimmed.is_empty() {
            anyhow::bail!("WVC_CLAUDE_AUTH_CODE is set but empty");
        }
    ...
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "Claude login needs an authorization code from stdin. Re-run in an interactive terminal, or set WVC_CLAUDE_AUTH_CODE."
        );
    }
...
pub fn parse_callback_input_with_state(input: &str) -> Result<(String, String)> {
    ...
    let state = state
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "Please paste the full callback URL or query string so wvc can verify the login state."
            )
        })?;
    Ok((code, state))
}
...
async fn exchange_claude_code_at_url(
    ...
    if !resp.status().is_success() {
        ...
        if status == reqwest::StatusCode::FORBIDDEN && looks_like_cloudflare_challenge(&text) {
            anyhow::bail!(
                "Token exchange was blocked by Cloudflare before Anthropic returned OAuth tokens. wvc now matches Claude Code's JSON token exchange, but this network/IP is still being challenged. Switch VPN exit IP or network, then retry with `wvc login --provider claude --no-browser` and paste the callback URL."
            );
        }
...
/// Save Claude tokens to wvc's credentials file (active account or first numbered account).
...
/// Load Claude tokens from wvc's credentials file (active account).
pub fn load_claude_tokens() -> Result<OAuthTokens> {
    ...
    anyhow::bail!("No Claude Max OAuth credentials found. Run 'wvc login --provider claude'.");
}
...
pub async fn refresh_openai_tokens(refresh_token: &str) -> Result<OAuthTokens> {
    match crate::auth::codex::active_account_label() {
        ...
        // External token (not stored in wvc auth): nothing on disk to
...
pub async fn refresh_openai_tokens_for_account(
    ...
    crate::auth::refresh_state::ensure_refresh_allowed(
        "openai",
        refresh_token,
        "Run `wvc login --provider openai` to mint a fresh token.",
    )?;
...
async fn refresh_openai_tokens_inner(
    ...
    let result: Result<OAuthTokens> = async {
        ...
        if let Some(label) = label {
            ...
            crate::logging::info(
                "Refreshed OpenAI/Codex tokens from an external source without storing them in wvc auth",
            );
        }
        Ok(oauth_tokens)
    }
