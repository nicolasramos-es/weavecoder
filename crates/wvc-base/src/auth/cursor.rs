...
// `WVC_CURSOR_CLIENT_VERSION` if Cursor moves the floor again.
...
pub fn has_authenticated_cli_session() -> bool {
    let command = std::env::var_os("WVC_CURSOR_CLI_PATH")
...
pub fn cursor_direct_client_version() -> String {
    std::env::var("WVC_CURSOR_CLIENT_VERSION")
        .ok()
...
/// 2. Saved key in `~/.config/wvc/cursor.env`
pub fn load_api_key() -> Result<String> {
    if let Ok(key) = std::env::var("CURSOR_API_KEY") {
        let trimmed = wvc_provider_env::sanitize_secret_value(&key);
        if !trimmed.is_empty() {
    ...
    if file_path.exists() {
        ...
        for line in content.lines() {
            ...
            if let Some(key) = line.strip_prefix("CURSOR_API_KEY=") {
                let key = wvc_provider_env::sanitize_secret_value(key);
                if !key.is_empty() {
...
/// Save a Cursor API key to `~/.config/wvc/cursor.env`.
...
/// Remove the saved Cursor API key from `~/.config/wvc/cursor.env` and the
...
pub fn cursor_auth_file_path() -> Result<PathBuf> {
    #[cfg(target_os = "windows")]
    {
        ...
        // branch, APPDATA points at the real user profile even when WVC_HOME
    ...
    {
        // Honor WVC_HOME isolation (used by the onboarding sandbox and tests)
        ...
        // `$WVC_HOME/external/...`, so a fresh-install sandbox would show only
