...
//!
//! Tracks running servers in `~/.wvc/servers.json` for discovery by clients.
...
pub fn registry_path() -> Result<PathBuf> {
    Ok(wvc_dir()?.join("servers.json"))
}

/// Get the socket directory path
pub fn socket_dir() -> Result<PathBuf> {
    Ok(crate::storage::runtime_dir().join("wvc"))
}
...
pub fn server_socket_path(name: &str) -> PathBuf {
    socket_dir()
        ...
        .unwrap_or_else(|_| std::env::temp_dir().join(format!("wvc-{}.sock", name)))
}
...
pub fn server_debug_socket_path(name: &str) -> PathBuf {
    socket_dir()
        ...
        .unwrap_or_else(|_| std::env::temp_dir().join(format!("wvc-{}-debug.sock", name)))
}
