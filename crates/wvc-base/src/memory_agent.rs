...
impl MemoryAgent {
    ...
    async fn process_context(
        ...
        let retrieval_ctx = RetrievalContext {
            ...
            rejected_ids,
            context_snippet: wvc_core::util::truncate_str(&context, 200).to_string(),
        };
    ...
    fn select_top_candidates_no_sidecar(
        ...
        for (entry, sim) in &selected {
            crate::logging::info(&format!(
                ...
                session_id,
                sim,
                wvc_core::util::truncate_str(&entry.content, 40)
            ));
        }
        ...
        tokio::spawn(async move {
            ...
            if ctx.verified_ids.is_empty() && !ctx.rejected_ids.is_empty() {
                ...
                crate::logging::info(&format!(
                    ...
                    wvc_core::util::truncate_str(&ctx.context_snippet, 100)
                ));
            }
        ...
        if stats.clusters_touched > 0 {
                    ...
                    .unwrap_or(false)
            {
                let member_contents: Vec<String> = project_ids
                    ...
                    .map(|m| wvc_core::util::truncate_str(&m.content, 80).to_string())
                    .collect();
