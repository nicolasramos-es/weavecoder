...
//! This lets iOS/web clients interact with wvc sessions identically
//! to TUI clients.
//!
//! Architecture:
...
async fn handle_pair_request(
    ...
    // Reload from disk - pairing codes are generated by `wvc pair` CLI
    *reg = DeviceRegistry::load();
    ...
    let body = serde_json::json!({
        "token": token,
        "server_name": "wvc",
        "server_version": wvc_build_meta::version(),
    });
    http_response(200, "OK", &body.to_string())
}
...
pub fn resolve_connect_host(bind_addr: &str) -> String {
    ...
    if let Some(host) = std::env::var("WVC_GATEWAY_HOST")
        .ok()
