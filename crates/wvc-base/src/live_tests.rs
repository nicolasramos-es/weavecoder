...
const END_TO_END_CHECKPOINTS: &[LiveVerificationCheckpointDefinition] = &[
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: false,
        description: "Weavecoder can load the credential from the expected env/config source and records only a fingerprint.",
    },
    LiveVerificationCheckpointDefinition {
...
pub struct LiveVerificationBuild {
    pub wvc_version: String,
    pub wvc_git_hash: String,
    pub wvc_git_date: String,
    pub wvc_git_dirty: bool,
    pub wvc_semver: String,
    pub os: String,
    pub arch: String,
    pub pid: u32,
}
...
impl LiveVerificationBuild {
    pub fn current() -> Self {
        let version = wvc_build_meta::version().to_string();
        Self {
            wvc_git_dirty: version.contains("dirty"),
            wvc_version: version,
            wvc_git_hash: wvc_build_meta::git_hash().to_string(),
            wvc_git_date: wvc_build_meta::git_date().to_string(),
            wvc_semver: wvc_build_meta::semver().to_string(),
...
fn update_coverage(event: &LiveVerificationEvent, path: &Path) -> Result<()> {
    ...
    coverage.latest.insert(
        ...
        LiveVerificationCoverageEntry {
            ...
            wvc_version: event.build.wvc_version.clone(),
            wvc_git_hash: event.build.wvc_git_hash.clone(),
            wvc_git_dirty: event.build.wvc_git_dirty,
...
pub fn format_provider_test_coverage_report(
    ...
    out.push_str(&format!(
        ...
        coverage_actor_label(entry.wvc_git_dirty, &entry.wvc_version),
    ));
...
struct ProviderModelCoverageBuilder {
    ...
    latest_wvc_version: String,
    latest_wvc_dirty: bool,
    entries: usize,
...
impl ProviderModelCoverageBuilder {
    fn record(&mut self, entry: &LiveVerificationCoverageEntry) {
            ...
            .unwrap_or(true)
        {
            ...
            self.latest_wvc_version = entry.wvc_version.clone();
            self.latest_wvc_dirty = entry.wvc_git_dirty;
        }
    ...
    fn finish(self) -> LiveProviderModelCoveragePair {
        ...
        LiveProviderModelCoveragePair {
            ...
            latest_wvc_version: self.latest_wvc_version,
            latest_wvc_dirty: self.latest_wvc_dirty,
    ...
    if all_pairs.is_empty() {
        ...
        for pair in &shown_pairs {
            ...
            let tested = format!(
                ...
                coverage_actor_label(pair.latest_wvc_dirty, &pair.latest_wvc_version),
            );
...
mod tests {
    ...
    fn coverage_entry(
        ...
        LiveVerificationCoverageEntry {
            ...
            wvc_version: "test".to_string(),
            wvc_git_hash: "test".to_string(),
            wvc_git_dirty: false,
            user_ready: false,
