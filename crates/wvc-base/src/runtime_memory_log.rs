...
pub fn server_logging_enabled() -> bool {
    match std::env::var("WVC_RUNTIME_MEMORY_LOG") {
        Ok(value) => !matches!(
...
pub fn server_logging_config() -> RuntimeMemoryLogConfig {
    let legacy_interval_secs = env_u64("WVC_RUNTIME_MEMORY_LOG_INTERVAL_SECS");
    let process_interval_secs = env_u64("WVC_RUNTIME_MEMORY_LOG_PROCESS_INTERVAL_SECS")
        .or(legacy_interval_secs)
        .filter(|value| *value >= MIN_PROCESS_INTERVAL_SECS)
        .unwrap_or(DEFAULT_PROCESS_INTERVAL_SECS);
    let attribution_interval_secs = env_u64("WVC_RUNTIME_MEMORY_LOG_ATTRIBUTION_INTERVAL_SECS")
    ...
    let attribution_min_spacing_secs =
        env_u64("WVC_RUNTIME_MEMORY_LOG_ATTRIBUTION_MIN_SPACING_SECS")
    ...
    let event_process_min_spacing_secs =
        env_u64("WVC_RUNTIME_MEMORY_LOG_EVENT_PROCESS_MIN_SPACING_SECS")
            .filter(|value| *value >= MIN_EVENT_PROCESS_MIN_SPACING_SECS)
            .unwrap_or(DEFAULT_EVENT_PROCESS_MIN_SPACING_SECS);
    let pss_delta_threshold_bytes = env_u64("WVC_RUNTIME_MEMORY_LOG_PSS_DELTA_THRESHOLD_MB")
        .unwrap_or(DEFAULT_PSS_DELTA_THRESHOLD_MB)
        .saturating_mul(1024 * 1024);
    let attribution_json_delta_threshold_bytes =
        env_u64("WVC_RUNTIME_MEMORY_LOG_ATTRIBUTION_JSON_DELTA_THRESHOLD_MB")
...
pub fn client_logging_enabled() -> bool {
    match std::env::var("WVC_CLIENT_RUNTIME_MEMORY_LOG") {
        Ok(value) => !matches!(
            value.trim().to_ascii_lowercase().as_str(),
            "0" | "false" | "no" | "off"
        ),
...
pub fn client_logging_config() -> RuntimeMemoryLogConfig {
    let process_interval_secs = env_u64("WVC_CLIENT_RUNTIME_MEMORY_LOG_PROCESS_INTERVAL_SECS")
        .filter(|value| *value >= MIN_PROCESS_INTERVAL_SECS)
        .unwrap_or(DEFAULT_CLIENT_PROCESS_INTERVAL_SECS);
    let attribution_interval_secs =
        env_u64("WVC_CLIENT_RUNTIME_MEMORY_LOG_ATTRIBUTION_INTERVAL_SECS")
            .filter(|value| *value >= MIN_ATTRIBUTION_INTERVAL_SECS)
            .unwrap_or(DEFAULT_CLIENT_ATTRIBUTION_INTERVAL_SECS);
    let attribution_min_spacing_secs =
        env_u64("WVC_CLIENT_RUNTIME_MEMORY_LOG_ATTRIBUTION_MIN_SPACING_SECS")
            .filter(|value| *value >= MIN_ATTRIBUTION_MIN_SPACING_SECS)
