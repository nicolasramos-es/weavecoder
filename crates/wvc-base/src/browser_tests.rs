...
fn test_rewrite_command_with_full_path() {
    ...
    if browser_binary_path().exists() {
        ...
        assert!(result.contains(".wvc/browser"));
    } else {
...
fn test_paths() {
    ...
    assert!(bdir.to_string_lossy().contains(".wvc"));
...
async fn test_inspect_browser_status_without_binary() {
    // Hold the test-env lock: this reads WVC_HOME-derived paths, and other
    // tests mutate WVC_HOME (and write browser fixture files) under the
    // lock. Without it, the status snapshot and the exists() check below can
    // observe different WVC_HOME values mid-test.
    let _guard = crate::storage::lock_test_env();
...
async fn test_ensure_browser_ready_noninteractive_without_binary() {
    ...
    // that mutate WVC_HOME under the test-env lock.
    let _guard = crate::storage::lock_test_env();
...
//
// Bug B: once ~/.wvc/browser/.setup-complete existed, setup could never
