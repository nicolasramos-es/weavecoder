...
const CLIENT_TITLE_PREFIXES: &[&str] = &["wvc:d:", "wvc:c:"];
...
pub async fn run_configured() -> Result<DictationRun> {
    ...
    if command.is_empty() {
        anyhow::bail!(
            "Dictation is not configured. Set `[dictation].command` in `~/.wvc/config.toml`."
        );
    }
...
pub fn remember_last_focused_session(session_id: &str) -> Result<()> {
    ...
    std::fs::write(&path, session_id).context("failed to persist last focused wvc session")?;
...
pub fn last_focused_session() -> Result<Option<String>> {
    ...
    let session_id = match std::fs::read_to_string(path) {
        ...
        Err(err) => return Err(err).context("failed to read last focused wvc session"),
    };
    if session_id.is_empty() {
        return Ok(None);
    }
...
pub fn focused_wvc_session() -> Result<Option<String>> {
...
fn last_focused_session_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::wvc_dir()?.join("last_focused_client_session"))
}
