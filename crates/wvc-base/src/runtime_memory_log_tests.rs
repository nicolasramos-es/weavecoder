...
fn server_logging_enabled_defaults_on_and_respects_falsey_env() {
    ...
    let prev = std::env::var_os("WVC_RUNTIME_MEMORY_LOG");

    crate::env::remove_var("WVC_RUNTIME_MEMORY_LOG");
    assert!(server_logging_enabled());

    crate::env::set_var("WVC_RUNTIME_MEMORY_LOG", "0");
    assert!(!server_logging_enabled());

    crate::env::set_var("WVC_RUNTIME_MEMORY_LOG", "false");
    assert!(!server_logging_enabled());

    crate::env::set_var("WVC_RUNTIME_MEMORY_LOG", "1");
    assert!(server_logging_enabled());

    if let Some(prev) = prev {
        crate::env::set_var("WVC_RUNTIME_MEMORY_LOG", prev);
    } else {
        crate::env::remove_var("WVC_RUNTIME_MEMORY_LOG");
    }
}
