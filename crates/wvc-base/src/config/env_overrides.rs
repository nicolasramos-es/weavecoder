...
impl Config {
    ...
    pub(crate) fn apply_env_overrides(&mut self) {
        // Keybindings
        if let Ok(v) = std::env::var("WVC_SCROLL_UP_KEY") {
            self.keybindings.scroll_up = v;
        }
        if let Ok(v) = std::env::var("WVC_SCROLL_DOWN_KEY") {
            self.keybindings.scroll_down = v;
        }
        if let Ok(v) = std::env::var("WVC_SCROLL_PAGE_UP_KEY") {
            self.keybindings.scroll_page_up = v;
        }
        if let Ok(v) = std::env::var("WVC_SCROLL_PAGE_DOWN_KEY") {
            self.keybindings.scroll_page_down = v;
        }
        if let Ok(v) = std::env::var("WVC_MODEL_SWITCH_KEY") {
            self.keybindings.model_switch_next = v;
        }
        if let Ok(v) = std::env::var("WVC_MODEL_SWITCH_PREV_KEY") {
            self.keybindings.model_switch_prev = v;
        }
        if let Ok(v) = std::env::var("WVC_EFFORT_INCREASE_KEY") {
            self.keybindings.effort_increase = v;
        }
        if let Ok(v) = std::env::var("WVC_EFFORT_DECREASE_KEY") {
            self.keybindings.effort_decrease = v;
        }
        if let Ok(v) = std::env::var("WVC_CENTERED_TOGGLE_KEY") {
            self.keybindings.centered_toggle = v;
        }
        if let Ok(v) = std::env::var("WVC_SCROLL_PROMPT_UP_KEY") {
            self.keybindings.scroll_prompt_up = v;
        }
        if let Ok(v) = std::env::var("WVC_SCROLL_PROMPT_DOWN_KEY") {
            self.keybindings.scroll_prompt_down = v;
        }
        if let Ok(v) = std::env::var("WVC_SCROLL_BOOKMARK_KEY") {
            self.keybindings.scroll_bookmark = v;
        }
        if let Ok(v) = std::env::var("WVC_SCROLL_UP_FALLBACK_KEY") {
            self.keybindings.scroll_up_fallback = v;
        }
        if let Ok(v) = std::env::var("WVC_SCROLL_DOWN_FALLBACK_KEY") {
            self.keybindings.scroll_down_fallback = v;
        }
        if let Ok(v) = std::env::var("WVC_WORKSPACE_LEFT_KEY") {
            self.keybindings.workspace_left = v;
        }
        if let Ok(v) = std::env::var("WVC_WORKSPACE_DOWN_KEY") {
            self.keybindings.workspace_down = v;
        }
        if let Ok(v) = std::env::var("WVC_WORKSPACE_UP_KEY") {
            self.keybindings.workspace_up = v;
        }
        if let Ok(v) = std::env::var("WVC_WORKSPACE_RIGHT_KEY") {
            self.keybindings.workspace_right = v;
        }
        if let Ok(v) = std::env::var("WVC_SIDE_PANEL_TOGGLE_KEY") {
            self.keybindings.side_panel_toggle = v;
        }
        if let Ok(v) = std::env::var("WVC_COPY_SELECTION_TOGGLE_KEY") {
            self.keybindings.copy_selection_toggle = v;
        }
        if let Ok(v) = std::env::var("WVC_DIAGRAM_PANE_TOGGLE_KEY") {
            self.keybindings.diagram_pane_toggle = v;
        }
        if let Ok(v) = std::env::var("WVC_TYPING_SCROLL_LOCK_TOGGLE_KEY") {
            self.keybindings.typing_scroll_lock_toggle = v;
        }
        if let Ok(v) = std::env::var("WVC_DIFF_MODE_CYCLE_KEY") {
            self.keybindings.diff_mode_cycle = v;
        }
        if let Ok(v) = std::env::var("WVC_INFO_WIDGET_TOGGLE_KEY") {
            self.keybindings.info_widget_toggle = v;
        }
        if let Ok(v) = std::env::var("WVC_NEW_TERMINAL_KEY") {
            self.keybindings.new_terminal = v;
        }

        // Dictation
        if let Ok(v) = std::env::var("WVC_DICTATION_COMMAND") {
            self.dictation.command = v;
        }
        if let Ok(v) = std::env::var("WVC_DICTATION_MODE")
        ...
        if let Ok(v) = std::env::var("WVC_DICTATION_KEY") {
            self.dictation.key = v;
        }
        if let Ok(v) = std::env::var("WVC_DICTATION_TIMEOUT_SECS")
        ...
        // Tools
        if let Ok(v) = std::env::var("WVC_TOOL_PROFILE") {
            self.tools.profile = v;
        }
        if let Ok(v) = std::env::var("WVC_TOOLS") {
            self.tools.enabled = parse_env_list(&v);
        }
        if let Ok(v) = std::env::var("WVC_DISABLED_TOOLS") {
            self.tools.disabled = parse_env_list(&v);
        }
        if let Ok(v) = std::env::var("WVC_DISABLE_BASE_TOOLS")
        ...
        // ACP adapter
        if let Ok(v) = std::env::var("WVC_ACP_PROFILE") {
            let trimmed = v.trim().to_ascii_lowercase();
            if matches!(trimmed.as_str(), "standard" | "extended" | "full") {
                self.acp.profile = trimmed;
            }
        }
        if let Ok(v) = std::env::var("WVC_ACP_TOOL_PROFILE") {
            let trimmed = v.trim();
            if !trimmed.is_empty() {
        ...
        // Display
        if let Ok(v) = std::env::var("WVC_DIFF_MODE") {
        ...
        } else if let Ok(v) = std::env::var("WVC_SHOW_DIFFS")
            && let Some(parsed) = parse_env_bool(&v)
        {
        ...
        if let Ok(v) = std::env::var("WVC_PIN_IMAGES")
            && let Some(parsed) = parse_env_bool(&v)
        {
            self.display.pin_images = parsed;
        }
        if let Ok(v) = std::env::var("WVC_PIN_TODOS")
            && let Some(parsed) = parse_env_bool(&v)
        {
            self.display.pin_todos = parsed;
        }
        if let Ok(v) = std::env::var("WVC_DISPLAY_CENTERED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.display.centered = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_DIFF_LINE_WRAP") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.display.diff_line_wrap = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_QUEUE_MODE") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.display.queue_mode = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_AUTO_SERVER_RELOAD") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.display.auto_server_reload = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_MOUSE_CAPTURE") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.display.mouse_capture = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_DEBUG_SOCKET") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.display.debug_socket = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_NO_EMOJI")
            && let Some(parsed) = parse_env_bool(&v)
        {
            self.display.emoji = !parsed;
        }
        if let Ok(v) = std::env::var("WVC_SHOW_THINKING") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.display.show_thinking = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_REASONING_DISPLAY") {
        ...
        if !self.display.has_explicit_reasoning_display()
            && let Ok(v) = std::env::var("WVC_DEFAULT_REASONING_DISPLAY")
            && let Some(mode) = crate::config::ReasoningDisplayMode::parse(&v)
        {
            self.display.set_reasoning_display(mode);
        }
        if let Ok(v) = std::env::var("WVC_MARKDOWN_SPACING") {
        ...
        // Features
        if let Ok(v) = std::env::var("WVC_MEMORY_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.features.memory = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_SWARM_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.features.swarm = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_ENABLE_MERMAID") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.features.mermaid = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_AUTO_POKE") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.features.auto_poke = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_MESSAGE_TIMESTAMPS") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.features.message_timestamps = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_PERSIST_MEMORY_INJECTIONS") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.features.persist_memory_injections = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_KV_CACHE_MISS_NOTICES") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.features.kv_cache_miss_notices = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_UPDATE_CHANNEL")
        ...
        if let Ok(v) = std::env::var("WVC_SWARM_MODEL") {
            let trimmed = v.trim();
        ...
        if let Ok(v) = std::env::var("WVC_MEMORY_SIDECAR_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.agents.memory_sidecar_enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_MEMORY_EMBEDDING_BACKEND") {
            let trimmed = v.trim();
            if !trimmed.is_empty() {
                self.agents.memory_embedding_backend = trimmed.to_string();
            }
        }
        if let Ok(v) = std::env::var("WVC_MEMORY_EMBEDDING_MODEL") {
            let trimmed = v.trim();
        ...
        if let Ok(v) = std::env::var("WVC_MEMORY_EMBEDDING_BASE_URL") {
            let trimmed = v.trim();
        ...
        if let Ok(v) = std::env::var("WVC_MEMORY_EMBEDDING_DIM") {
            if let Ok(parsed) = v.trim().parse::<usize>() {
        ...
        // Terminal spawning
        if let Ok(v) = std::env::var("WVC_SPAWN_HOOK") {
            let trimmed = v.trim();
        ...
        if let Ok(v) = std::env::var("WVC_FOCUS_HOOK") {
            let trimmed = v.trim();
            // An explicitly empty env value disables a config-file hook.
        ...
        hook_env_override(&mut self.hooks.turn_start, "WVC_HOOK_TURN_START");
        hook_env_override(&mut self.hooks.turn_end, "WVC_HOOK_TURN_END");
        hook_env_override(&mut self.hooks.session_start, "WVC_HOOK_SESSION_START");
        hook_env_override(&mut self.hooks.session_end, "WVC_HOOK_SESSION_END");
        hook_env_override(&mut self.hooks.pre_tool, "WVC_HOOK_PRE_TOOL");
        hook_env_override(&mut self.hooks.post_tool, "WVC_HOOK_POST_TOOL");
        if let Ok(v) = std::env::var("WVC_HOOK_PRE_TOOL_TIMEOUT_MS") {
        ...
        // Web search
        if let Ok(v) = std::env::var("WVC_WEBSEARCH_ENGINE")
            && let Some(engine) = WebSearchEngine::parse(&v)
        {
            self.websearch.engine = engine;
        }
        if let Ok(v) = std::env::var("WVC_WEBSEARCH_FALLBACK_ENGINES") {
        ...
        if let Ok(v) = std::env::var("WVC_BING_API_KEY")
            && !v.trim().is_empty()
        {
            self.websearch.bing_api_key = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_BING_API_KEY_ENV")
            && !v.trim().is_empty()
        {
            self.websearch.bing_api_key_env = v;
        }
        if let Ok(v) = std::env::var("WVC_BING_MARKET")
            && !v.trim().is_empty()
        {
            self.websearch.bing_market = v;
        }
        if let Ok(v) = std::env::var("WVC_SEARXNG_URL")
            && !v.trim().is_empty()
        {
            self.websearch.searxng_url = Some(v);
        }

        if let Ok(v) = std::env::var("WVC_TRUSTED_EXTERNAL_AUTH_SOURCES") {
        ...
        // Autoreview
        if let Ok(v) = std::env::var("WVC_AUTOREVIEW_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.autoreview.enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_AUTOREVIEW_MODEL") {
            let trimmed = v.trim();
        ...
        // Autojudge
        if let Ok(v) = std::env::var("WVC_AUTOJUDGE_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.autojudge.enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_AUTOJUDGE_MODEL") {
            let trimmed = v.trim();
        ...
        // Ambient
        if let Ok(v) = std::env::var("WVC_AMBIENT_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.ambient.enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_AMBIENT_PROVIDER") {
            self.ambient.provider = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_AMBIENT_MODEL") {
            self.ambient.model = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_AMBIENT_MIN_INTERVAL") {
            if let Ok(parsed) = v.trim().parse::<u32>() {
                self.ambient.min_interval_minutes = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_AMBIENT_MAX_INTERVAL") {
            if let Ok(parsed) = v.trim().parse::<u32>() {
                self.ambient.max_interval_minutes = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_AMBIENT_PROACTIVE") {
            if let Some(parsed) = parse_env_bool(&v) {
        ...
        if let Ok(v) = std::env::var("WVC_NTFY_TOPIC") {
            self.safety.ntfy_topic = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_NTFY_SERVER") {
            self.safety.ntfy_server = v;
        }
        if let Ok(v) = std::env::var("WVC_SMTP_PASSWORD") {
            self.safety.email_password = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_EMAIL_TO") {
            self.safety.email_to = Some(v);
            self.safety.email_enabled = true;
        }
        if let Ok(v) = std::env::var("WVC_IMAP_HOST") {
            self.safety.email_imap_host = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_EMAIL_REPLY_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.safety.email_reply_enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_TELEGRAM_BOT_TOKEN") {
            self.safety.telegram_bot_token = Some(v);
            self.safety.telegram_enabled = true;
        }
        if let Ok(v) = std::env::var("WVC_TELEGRAM_CHAT_ID") {
            self.safety.telegram_chat_id = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_TELEGRAM_REPLY_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.safety.telegram_reply_enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_DISCORD_BOT_TOKEN") {
            self.safety.discord_bot_token = Some(v);
            self.safety.discord_enabled = true;
        }
        if let Ok(v) = std::env::var("WVC_DISCORD_CHANNEL_ID") {
            self.safety.discord_channel_id = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_DISCORD_BOT_USER_ID") {
            self.safety.discord_bot_user_id = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_DISCORD_REPLY_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.safety.discord_reply_enabled = parsed;
            }
        }
        // Jade cloud relay channel
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_API_BASE") {
            self.safety.jade_relay_api_base = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_TOKEN") {
            self.safety.jade_relay_token = Some(v);
            self.safety.jade_relay_enabled = true;
        }
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_TOKEN_ID") {
            self.safety.jade_relay_token_id = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_USER_ID") {
            self.safety.jade_relay_user_id = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_SESSION_ID") {
            self.safety.jade_relay_session_id = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.safety.jade_relay_enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_REPLY_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.safety.jade_relay_reply_enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_LAUNCH_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.safety.jade_relay_launch_enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_JADE_RELAY_LAUNCH_WORKING_DIR") {
            let trimmed = v.trim();
            if !trimmed.is_empty() {
                self.safety.jade_relay_launch_working_dir = Some(trimmed.to_string());
            }
        }
        if let Ok(v) = std::env::var("WVC_AMBIENT_VISIBLE") {
            if let Some(parsed) = parse_env_bool(&v) {
        ...
        // Gateway (iOS/web)
        if let Ok(v) = std::env::var("WVC_GATEWAY_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.gateway.enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_GATEWAY_PORT") {
            if let Ok(parsed) = v.trim().parse::<u16>() {
                self.gateway.port = parsed;
            }
        }
        if let Ok(v) = std::env::var("WVC_GATEWAY_BIND_ADDR") {
            let trimmed = v.trim();
            if !trimmed.is_empty() {
        ...
        // Power management
        if let Ok(v) = std::env::var("WVC_PREVENT_SLEEP_WHILE_STREAMING") {
            if let Some(parsed) = parse_env_bool(&v) {
        ...
        // Provider
        if let Ok(v) = std::env::var("WVC_MODEL") {
            self.provider.default_model = Some(v);
        }
        if let Ok(v) = std::env::var("WVC_PROVIDER") {
            let trimmed = v.trim().to_lowercase();
            if !trimmed.is_empty() {
                self.provider.default_provider = Some(trimmed);
            }
        }
        if let Ok(v) = std::env::var("WVC_OPENAI_REASONING_EFFORT") {
            let trimmed = v.trim().to_string();
            if !trimmed.is_empty() {
                self.provider.openai_reasoning_effort = Some(trimmed);
            }
        }
        if let Ok(v) = std::env::var("WVC_ANTHROPIC_REASONING_EFFORT") {
            let trimmed = v.trim().to_string();
            if !trimmed.is_empty() {
                self.provider.anthropic_reasoning_effort = Some(trimmed);
            }
        }
        if let Ok(v) = std::env::var("WVC_OPENAI_TRANSPORT") {
            let trimmed = v.trim().to_string();
            if !trimmed.is_empty() {
                self.provider.openai_transport = Some(trimmed);
            }
        }
        if let Ok(v) = std::env::var("WVC_OPENAI_SERVICE_TIER") {
            let trimmed = v.trim().to_string();
            if !trimmed.is_empty() {
                self.provider.openai_service_tier = Some(trimmed);
            }
        }
        if let Ok(v) = std::env::var("WVC_OPENAI_NATIVE_COMPACTION_MODE") {
            let trimmed = v.trim().to_ascii_lowercase();
            if !trimmed.is_empty() {
                self.provider.openai_native_compaction_mode = trimmed;
            }
        }
        if let Ok(v) = std::env::var("WVC_OPENAI_NATIVE_COMPACTION_THRESHOLD_TOKENS") {
            if let Ok(parsed) = v.trim().parse::<usize>() {
                if parsed > 0 {
        ...
        if let Ok(v) = std::env::var("WVC_COPILOT_PREMIUM") {
            ...
            if !env_val.is_empty() {
                crate::env::set_var("WVC_COPILOT_PREMIUM", env_val);
            }
        }
    }
}
