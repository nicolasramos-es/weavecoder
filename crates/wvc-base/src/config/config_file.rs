...
impl Config {
    ...
    pub fn path() -> Option<PathBuf> {
        wvc_dir().ok().map(|d| d.join("config.toml"))
    }
    ...
    pub fn bake_launch_hotkeys_once() -> bool {
        use wvc_import_core::repo_ranking;

        let cfg = Self::load();
        if cfg.launch_hotkeys.imported {
            return false;
        }
        let Ok(wvc_dir) = wvc_dir() else {
            return false;
        };
        let sessions_dir = wvc_dir.join("sessions");
        ...
        let entries: Vec<wvc_config_types::LaunchHotkeyEntry> = plan
            .into_iter()
            .map(|p| wvc_config_types::LaunchHotkeyEntry {
                chord: p.chord,
