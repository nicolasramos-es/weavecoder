...
fn compile_static_regex(pattern: &str) -> Option<Regex> {
    match Regex::new(pattern) {
        ...
        Err(err) => {
            ...
            eprintln!("wvc: failed to compile static regex: {err}");
            None
        }
    }
}
...
pub fn generated_image_visual_context_blocks(
    ...
    let mut reminder = format!(
        "<system-reminder>\nA provider-native image generation call created `{}`. Weavecoder attached the image pixels as visual context for future turns because the active provider supports image input and the file is under the safe {} MB limit.\nFormat: {}",
        path,
...
pub fn generated_image_rendered_image(
    ...
    path: &str,
    output_format: &str,
) -> Option<wvc_session_types::RenderedImage> {
    let (media_type, data) = generated_image_payload(path, output_format)?;
    Some(wvc_session_types::RenderedImage {
        media_type,
        data,
        ...
        anchor: Some(wvc_session_types::RenderedImageAnchor::ToolCall { id: id.to_string() }),
    })
}
