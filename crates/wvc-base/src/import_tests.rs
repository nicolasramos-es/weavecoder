...
fn repeated_external_resume_reuses_the_imported_snapshot() {
    ...
    .unwrap();
    let target = wvc_session_types::ResumeTarget::ClaudeCodeSession {
...
#[cfg(target_os = "linux")]
#[test]
fn takeover_timeout_preserves_the_staged_wvc_session_after_sigterm() {
    use std::process::{Command, Stdio};
...
fn test_resolve_resume_target_to_wvc_imports_claude_code_session() {
    ...
    assert_eq!(
        imported_session_id_for_target(&wvc_session_types::ResumeTarget::ClaudeCodeSession {
            session_id: "claude-resolve-test".to_string(),
            session_path: transcript_path.to_string_lossy().to_string(),
        }),
...
fn test_reimporting_claude_session_preserves_wvc_continuation() {
    ...
    session.append_stored_message(StoredMessage {
        ...
        role: Role::User,
        content: vec![ContentBlock::Text {
            text: "wvc-only follow up".to_string(),
            cache_control: None,
        }],
        display_role: None,
        timestamp: None,
    ...
    assert_eq!(
        after.messages.len(),
        3,
        "wvc-side continuation must not be clobbered by re-import"
    );
    let preserved = after.messages.iter().flat_map(|m| m.content.iter()).any(
        |block| matches!(block, ContentBlock::Text { text, .. } if text == "wvc-only follow up"),
    );
    assert!(
        preserved,
