...
fn truncate_import_text(text: String, max_bytes: usize) -> String {
    ...
    format!(
        "{}{}",
        wvc_core::util::truncate_str(&text, prefix_budget),
        suffix
    )
}
...
pub fn imported_session_id_for_target(
    ...
    match target {
        ...
        wvc_session_types::ResumeTarget::CodexSession { session_id, .. } => {
            Some(imported_codex_session_id(session_id))
        }
        wvc_session_types::ResumeTarget::PiSession { session_path } => {
            Some(imported_pi_session_id(session_path))
        }
        wvc_session_types::ResumeTarget::OpenCodeSession { session_id, .. } => {
            Some(imported_opencode_session_id(session_id))
        }
        wvc_session_types::ResumeTarget::CursorSession { session_id, .. } => {
...
fn import_session_from_file_with_target(
    ...
    let mut session = Session::create_with_id(wvc_session_id, None, title);
...
fn take_over_live_claude_session_with_timeout(
    ...
    crate::logging::info(&format!(
        "Claude takeover complete: source_session={session_id} source_pid={} wvc_session={takeover_id}",
        live.pid
    ));
    crate::session_list_cache::invalidate();
