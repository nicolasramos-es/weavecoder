...
impl MultiProvider {
    ...
    pub(super) fn new_with_auth_status(auth_status: auth::AuthStatus) -> Self {
        ...
        let use_claude_cli = std::env::var("WVC_USE_CLAUDE_CLI")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        if use_claude_cli {
            crate::logging::warn(
                "WVC_USE_CLAUDE_CLI is deprecated and will be removed. Direct Anthropic API transport is the default.",
            );
        }

        let claude = if has_claude_creds && use_claude_cli {
            crate::logging::info(
                "Using deprecated Claude CLI provider (forced by WVC_USE_CLAUDE_CLI=1)",
            );
        ...
        let openrouter = if has_openrouter_creds {
            let named_profile = std::env::var("WVC_NAMED_PROVIDER_PROFILE")
                .ok()
