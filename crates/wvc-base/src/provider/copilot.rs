...
//! the downstream `wvc-provider-copilot-runtime` crate so provider edits do
...
//! pure model-catalog data (from `wvc-provider-copilot`) that its routing
//! logic needs, plus a credentials probe that delegates to auth.

pub use wvc_provider_copilot::{DEFAULT_MODEL, FALLBACK_MODELS, is_known_display_model};
