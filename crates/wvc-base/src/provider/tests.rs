...
fn save_test_openrouter_model_cache(namespace: &str, source_api_base: &str, model_ids: &[&str]) {
    ...
    let cache = wvc_provider_openrouter::DiskCache {
        ...
        models: model_ids
            .iter()
            .map(|id| wvc_provider_openrouter::ModelInfo {
                id: (*id).to_string(),
                name: String::new(),
                context_length: None,
                pricing: wvc_provider_openrouter::ModelPricing::default(),
                created: None,
            })
            .collect(),
    };
...
fn active_explicit_credential_reflects_openai_switch_immediately_and_none_for_auto() {
    use wvc_provider_core::{Provider, ResolvedCredential};
    with_clean_provider_test_env(|| {
        let rt = enter_test_runtime();
        let _runtime_guard = rt.enter();
        let provider = test_multi_provider_with_openai();
...
fn openai_model_routes_cover_oauth_api_and_no_auth_state_space() {
    with_clean_provider_test_env(|| {
        ...
        crate::env::set_var("OPENAI_API_KEY", "sk-test-openai-api-key");
        std::fs::remove_file(crate::storage::wvc_dir().unwrap().join("openai-auth.json"))
...
fn standard_openrouter_catalog_refresh_fires_when_named_profile_owns_slot() {
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            wvc_provider_openrouter_runtime::reset_profile_catalog_refresh_tracker_for_tests();
...
struct StubExternalRuntime {
    ...
    credential_mode: std::sync::RwLock<wvc_provider_core::CredentialMode>,
}
...
impl StubExternalRuntime {
    fn new(
        ...
        Self {
            ...
            credential_mode: std::sync::RwLock::new(wvc_provider_core::CredentialMode::Auto),
        }
    }
    ...
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(
                ...
                Some(wvc_provider_core::ResolvedCredential::ApiKey)
            );
