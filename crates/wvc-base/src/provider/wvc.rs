...
impl JcodeProvider {
    ...
    fn apply_runtime_profile() {
        let _ = crate::provider::activation::ProviderActivation::wvc_subscription(
...
impl Provider for JcodeProvider {
    ...
    fn uses_wvc_compaction(&self) -> bool {
        self.inner.uses_wvc_compaction()
    }
...
mod tests {
    ...
    #[test]
    fn wvc_provider_enables_subscription_runtime_mode() {
    ...
    #[test]
    fn wvc_provider_name_and_default_model_are_curated() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
    ...
    fn wvc_provider_exposes_only_explicit_subscription_routes() {
        ...
        assert!(plus_routes.iter().all(|route| {
            route.provider == "Weavecoder Subscription"
                && route.api_method == "wvc-subscription"
                && route.available
        }));
        assert_eq!(
