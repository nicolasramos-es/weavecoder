...
/// Memoization is skipped in test builds: test sandboxes swap `WVC_HOME`
...
pub(crate) fn copilot_pricing(model: &str) -> RouteCheapnessEstimate {
    let zero_premium_mode = matches!(
        std::env::var("WVC_COPILOT_PREMIUM").ok().as_deref(),
        Some("0")
    );
...
pub(crate) fn cheapness_for_route(
    ...
    use wvc_provider_core::{AuthMode, AuthRoute, DualAuthProvider};
...
mod tests {
    use super::*;
    use crate::env;
    use wvc_provider_core::{RouteBillingKind, RouteCostConfidence, RouteCostSource};
    ...
    fn with_clean_provider_test_env<T>(f: impl FnOnce() -> T) -> T {
        ...
        let prev_copilot_premium = std::env::var_os("WVC_COPILOT_PREMIUM");
        crate::auth::claude::set_active_account_override(None);
        crate::auth::codex::set_active_account_override(None);
        env::set_var("WVC_HOME", temp.path());
        env::remove_var("OPENAI_API_KEY");
        env::remove_var("WVC_COPILOT_PREMIUM");
        ...
        if let Some(prev_home) = prev_home {
            env::set_var("WVC_HOME", prev_home);
        } else {
            env::remove_var("WVC_HOME");
        }
        ...
        if let Some(prev_copilot_premium) = prev_copilot_premium {
            env::set_var("WVC_COPILOT_PREMIUM", prev_copilot_premium);
        } else {
            env::remove_var("WVC_COPILOT_PREMIUM");
        }
        result
    ...
    fn copilot_zero_mode_marks_estimate_high_confidence_and_zero_reference_cost() {
        with_clean_provider_test_env(|| {
            env::set_var("WVC_COPILOT_PREMIUM", "0");
