use super::*;
use wvc_provider_core::{FailoverDecision, ProviderFailoverPrompt};
...
impl MultiProvider {
    ...
    pub(super) fn fallback_sequence(active: ActiveProvider) -> Vec<ActiveProvider> {
        wvc_provider_core::fallback_sequence(active)
    }
    ...
    pub(super) fn classify_failover_error(err: &anyhow::Error) -> FailoverDecision {
        wvc_provider_core::classify_failover_error_message(&err.to_string())
    }
