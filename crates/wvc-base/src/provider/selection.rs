...
pub(super) use wvc_provider_core::{ActiveProvider, ProviderAvailability};
...
impl MultiProvider {
    pub(super) fn auto_default_provider(availability: ProviderAvailability) -> ActiveProvider {
        wvc_provider_core::auto_default_provider(availability)
    }

    pub(super) fn parse_provider_hint(value: &str) -> Option<ActiveProvider> {
        wvc_provider_core::parse_provider_hint(value)
    }
    ...
    pub(super) fn provider_label(provider: ActiveProvider) -> &'static str {
        wvc_provider_core::provider_label(provider)
    }

    pub(super) fn provider_key(provider: ActiveProvider) -> &'static str {
        wvc_provider_core::provider_key(provider)
    }
    ...
    pub(crate) fn canonical_session_provider_key(provider_key: &str) -> &str {
        ...
        if let Some(route) = wvc_provider_core::AuthRoute::parse(provider_key) {
    ...
    fn explicit_session_provider_key_for_model_request(model_request: &str) -> Option<String> {
        ...
        if let Some((prefix, rest)) = model_request.split_once(':') {
            ...
            if !prefix.is_empty() && !rest.trim().is_empty() {
                ...
                if let Some(route) = wvc_provider_core::AuthRoute::parse(prefix) {
    ...
    pub fn model_switch_request_for_session_model(
        ...
        if let Some(route) =
            wvc_provider_core::AuthRoute::parse_explicit_credential_prefix(provider_key)
        {
    ...
    pub(super) fn resolve_config_provider_selection(
        ...
        Self::parse_provider_hint(trimmed)
            .or_else(|| {
                wvc_provider_core::AuthRoute::parse(trimmed).map(|route| route.active_provider())
            })
...
mod tests {
    ...
    fn bare_session_provider_key_restores_auto_credential_not_oauth_pin() {
        ...
        for (model, provider_key, expected_request) in [
            ...
            assert!(
                wvc_provider_core::AuthRoute::parse_explicit_credential_prefix(prefix).is_none(),
