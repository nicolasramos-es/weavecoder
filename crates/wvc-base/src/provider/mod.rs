...
pub(crate) fn cached_live_models_for_openai_compatible_profile(
    ...
    let cache_is_stale = wvc_provider_openrouter::current_unix_secs()
...
fn configured_standard_openrouter_profile_routes() -> Vec<ModelRoute> {
    let Some(cache) = wvc_provider_openrouter::load_disk_cache_entry_for_namespace("openrouter")
    else {
        return Vec::new();
    };
...
impl MultiProvider {
    ...
    pub(super) fn set_config_default_model(
            ...
            Self::resolve_config_provider_selection(pref, crate::config::config())
        {
            ...
            let pinned = wvc_provider_core::AuthRoute::parse_explicit_credential_prefix(pref);
        ...
        let prefix = match active {
            ActiveProvider::Claude => {
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anthropic
                        .credential_mode()
                        .auth_route(wvc_provider_core::DualAuthProvider::Anthropic)
            ...
            ActiveProvider::OpenAI => {
                if let Some(openai) = self.openai_provider() {
                    openai
                        .credential_mode()
                        .auth_route(wvc_provider_core::DualAuthProvider::OpenAI)
                        .map(|route| route.model_prefix())
                        .unwrap_or("openai")
                } else {
                    "openai"
...
impl Provider for MultiProvider {
        ...
        match self.active_provider() {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    wvc_provider_core::DEFAULT_CLAUDE_MODEL.to_string()
                }
            }
            ActiveProvider::OpenAI => self
                .openai_provider()
                .map(|o| o.model())
                .unwrap_or_else(|| wvc_provider_core::DEFAULT_OPENAI_MODEL.to_string()),
    ...
    fn active_resolved_credential(&self) -> Option<wvc_provider_core::ResolvedCredential> {
        use wvc_provider_core::ResolvedCredential;
        match self.active_provider() {
            ActiveProvider::Claude => {
    ...
    fn active_explicit_credential(&self) -> Option<wvc_provider_core::ResolvedCredential> {
        use wvc_provider_core::ResolvedCredential;
