        ...
        match provider {
            ...
            ActiveProvider::OpenAI => {
                if let Some(openai) = self.openai_provider() {
                    ...
                    Err(anyhow::anyhow!(
                        "OpenAI credentials not available. Run `wvc login --provider openai` to log in."
                    ))
                }
            }
            ActiveProvider::Copilot => {
                ...
                if let Some(copilot) = copilot {
                    ...
                    Err(anyhow::anyhow!(
                        "GitHub Copilot is not available. Run `wvc login --provider copilot`."
                    ))
                }
            }
            ActiveProvider::Antigravity => {
                ...
                if let Some(antigravity) = antigravity {
                    ...
                    Err(anyhow::anyhow!(
                        "Antigravity is not available. Run `wvc login --provider antigravity`."
                    ))
                }
            }
            ActiveProvider::Gemini => {
                ...
                if let Some(gemini) = gemini {
                    ...
                    Err(anyhow::anyhow!(
                        "Gemini is not available. Run `wvc login --provider gemini`."
                    ))
                }
            }
            ActiveProvider::Cursor => {
                ...
                if let Some(cursor) = cursor {
                    ...
                    Err(anyhow::anyhow!(
                        "Cursor is not available. Run `wvc login --provider cursor`."
                    ))
                }
            }
