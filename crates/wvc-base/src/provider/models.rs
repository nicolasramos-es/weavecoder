...
fn model_exposes_1m_alias(normalized_model: &str) -> bool {
    if normalized_model.starts_with("claude-") {
        let mode = wvc_provider_core::anthropic_context_mode(normalized_model);
        ...
        if mode != wvc_provider_core::AnthropicContextMode::Standard {
...
pub(crate) fn config_context_limit_cache_keys(profile_id: &str, model_id: &str) -> Vec<String> {
    ...
    let slash_base = wvc_provider_core::model_id::slash_base(&id).to_string();
...
pub fn provider_for_model_with_hint(
    ...
    if model.contains('@') {
        Some("openrouter")
    } else if wvc_provider_core::model_id::matches_known_model(model, ALL_CLAUDE_MODELS) {
        Some("claude")
    } else if wvc_provider_core::model_id::matches_known_model(model, ALL_OPENAI_MODELS) {
        Some("openai")
