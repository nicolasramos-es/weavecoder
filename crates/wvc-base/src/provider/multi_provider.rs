    ...
    pub(super) async fn try_same_provider_account_failover(
        ...
        for alternative_label in &alternatives {
            ...
            match attempt {
                ...
                .push(format!(
                        "⚡ Auto-switched {} account: {} → {}. To turn this off, set `[provider].same_provider_account_failover = false` in `~/.wvc/config.toml` or export `WVC_SAME_PROVIDER_ACCOUNT_FAILOVER=false`.",
