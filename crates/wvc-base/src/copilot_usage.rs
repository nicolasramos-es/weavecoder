...
//! doesn't expose a usage API. Data persists to ~/.wvc/copilot_usage.json.
...
pub use wvc_usage_types::{AllTimeUsage, CopilotUsageTracker, DayUsage, MonthUsage};
...
mod tests {
    ...
    #[test]
    fn usage_path_respects_wvc_home() {
    ...
    #[test]
    fn save_and_load_roundtrip_under_wvc_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("WVC_HOME", temp.path().as_os_str());
