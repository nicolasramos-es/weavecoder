...
impl SkillRegistry {
    ...
    fn import_from_external() {
        ...
        {
            let count = Self::copy_skills_dir(&claude_skills, &wvc_skills);
            if count > 0 {
                sources.push(format!("{} from Claude Code", count));
                copied.extend(Self::list_skill_names(&wvc_skills));
            }
        }
        ...
        if let Ok(codex_skills) = crate::storage::user_home_path(".codex/skills")
            && codex_skills.is_dir()
        {
            let count = Self::copy_skills_dir(&codex_skills, &wvc_skills);
            if count > 0 {
    ...
    pub fn load_global() -> Result<Self> {
        ...
        if let Ok(wvc_dir) = crate::storage::wvc_dir() {
            ...
            if wvc_skills.exists() {
                registry.load_from_dir(&wvc_skills)?;
            }
        }
    ...
    pub fn reload_global(&mut self) -> Result<usize> {
        ...
        if let Ok(wvc_dir) = crate::storage::wvc_dir() {
            let wvc_skills = wvc_dir.join("skills");
            if wvc_skills.exists() {
                count += self.load_from_dir_count(&wvc_skills)?;
            }
        }
...
pub struct EndorsedSkill {
    ...
    /// Grouping label used to organize the endorsed list (e.g. "wvc",
    /// "NVIDIA CUDA-X").
...
mod tests {
    ...
    #[test]
    fn explicit_wvc_skill_wins_over_plugin_skill_with_same_name() {
        let temp = tempfile::tempdir().expect("tempdir");
