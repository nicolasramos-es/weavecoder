...
//!
//! These drive the real `wvc_base::import` pipeline against synthetic
//! `.jsonl` fixtures written into a sandboxed `WVC_HOME`, so they never touch
...
/// Serializes tests that mutate `WVC_HOME`. This test binary has its own
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = self.prev.take() {
            wvc_base::env::set_var(self.key, prev);
        } else {
...
/// Create the sandboxed Claude project dir under `$WVC_HOME/external/...`.
...
fn import_array_tool_result_with_image_uses_placeholder_not_base64() {
    ...
    let saved = std::fs::read_to_string(wvc_session_file(temp.path(), "img-session")).unwrap();
    assert!(
        !saved.contains(&big),
...
/// Path to where wvc persists imported sessions under the sandbox.
/// With `WVC_HOME` set, `storage::wvc_dir()` returns it directly, and
/// sessions live at `<WVC_HOME>/sessions/<id>.json`.
fn wvc_session_file(home: &Path, claude_id: &str) -> PathBuf {
