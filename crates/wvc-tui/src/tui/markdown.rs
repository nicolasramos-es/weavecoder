...
) -> wvc_tui_markdown::DiagramDisplayMode {
    match mode {
        crate::config::DiagramDisplayMode::None => wvc_tui_markdown::DiagramDisplayMode::None,
        crate::config::DiagramDisplayMode::Margin => wvc_tui_markdown::DiagramDisplayMode::Margin,
        crate::config::DiagramDisplayMode::Pinned => wvc_tui_markdown::DiagramDisplayMode::Pinned,
    }
}

fn from_markdown_diagram_mode(
    mode: wvc_tui_markdown::DiagramDisplayMode,
) -> crate::config::DiagramDisplayMode {
    match mode {
        wvc_tui_markdown::DiagramDisplayMode::None => crate::config::DiagramDisplayMode::None,
        wvc_tui_markdown::DiagramDisplayMode::Margin => crate::config::DiagramDisplayMode::Margin,
        wvc_tui_markdown::DiagramDisplayMode::Pinned => crate::config::DiagramDisplayMode::Pinned,
    }
}

fn to_markdown_spacing_mode(
    mode: crate::config::MarkdownSpacingMode,
) -> wvc_tui_markdown::MarkdownSpacingMode {
    match mode {
        crate::config::MarkdownSpacingMode::Compact => {
            wvc_tui_markdown::MarkdownSpacingMode::Compact
        }
        crate::config::MarkdownSpacingMode::Document => {
            wvc_tui_markdown::MarkdownSpacingMode::Document
        }
    }
}
...
pub fn install_wvc_markdown_hooks() {
    wvc_tui_markdown::set_latex_log_hook(|error| {
        crate::logging::warn(&format!(
            "LaTeX image rendering fell back to Unicode: {error}"
        ));
    });
    wvc_tui_markdown::set_config_snapshot_hook(|| {
        let cfg = crate::config::config();
        wvc_tui_markdown::MarkdownConfigSnapshot {
    ...
    wvc_tui_markdown::set_memory_snapshot_hook(|| {
        let snapshot = crate::process_memory::snapshot_with_source("client:markdown:memory");
        wvc_tui_markdown::ProcessMemorySnapshot {
...
pub fn set_diagram_mode_override(mode: Option<crate::config::DiagramDisplayMode>) {
    wvc_tui_markdown::set_diagram_mode_override(mode.map(to_markdown_diagram_mode));
}

pub fn get_diagram_mode_override() -> Option<crate::config::DiagramDisplayMode> {
    wvc_tui_markdown::get_diagram_mode_override().map(from_markdown_diagram_mode)
}
...
pub fn with_diagram_mode_scope<T>(
    ...
    wvc_tui_markdown::with_diagram_mode_scope(to_markdown_diagram_mode(mode), f)
}

pub fn with_deferred_mermaid_render_context<T>(f: impl FnOnce() -> T) -> T {
    wvc_tui_markdown::with_deferred_mermaid_render_context(f)
}
