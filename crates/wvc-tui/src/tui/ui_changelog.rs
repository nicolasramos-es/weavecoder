...
fn parse_changelog() -> Vec<ChangelogEntry<'static>> {
    let changelog: &'static str = wvc_build_meta::CHANGELOG;
    parse_changelog_from_impl(changelog)
}
...
/// Reads the last-seen commit hash from ~/.wvc/last_seen_changelog,
...
pub(super) fn get_unseen_changelog_entries() -> &'static Vec<String> {
    ...
    ENTRIES.get_or_init(|| {
        ...
        let state_file = dirs::home_dir()
            .map(|h| h.join(".wvc").join("last_seen_changelog"))
            .unwrap_or_else(|| std::path::PathBuf::from(".wvc/last_seen_changelog"));
