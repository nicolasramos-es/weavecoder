...
//! [`wvc_tui_render::swarm_gallery`] module so the live TUI and the
    ...
    members
        ...
        .map(|member| GalleryMember {
            ...
            todo_items: member
                .todo_items
                .iter()
                .map(|t| wvc_tui_render::swarm_gallery::GalleryTodo {
                    content: t.content.clone(),
                    status: t.status.clone(),
                    tool_intents: t
                        .tool_intents
                        .iter()
                        .map(|tool| wvc_tui_render::swarm_gallery::GalleryToolIntent {
...
pub(crate) fn render_swarm_chat_card_lines(
    ...
    wvc_tui_render::swarm_gallery::render_swarm_chat_cards(&gallery_members, width)
}
...
mod tests {
    use super::*;
    use wvc_tui_render::swarm_gallery::members_to_tiles;
