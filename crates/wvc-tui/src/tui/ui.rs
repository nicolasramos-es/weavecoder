...
pub(crate) use wvc_tui_markdown::{CopyTargetKind, RawCopyTarget};
pub(crate) use wvc_tui_messages::{
...
impl CopyViewportSnapshot {
    ...
    fn inline_image_id_for_label_line(&self, abs_line: usize) -> Option<u64> {
        ...
        prepared
            ...
            .iter()
            .find(|region| {
                region.render == wvc_tui_messages::ImageRegionRender::Fit
...
pub fn draw(frame: &mut Frame, app: &dyn TuiState) {
    ...
    wvc_tui_style::adapt_buffer_for_theme(frame.buffer_mut());
    wvc_tui_style::palette::adapt_buffer_for_palette(frame.buffer_mut());
...
fn draw_inner(frame: &mut Frame, app: &dyn TuiState) {
    ...
    let margins = if onboarding_welcome {
        ...
        let members = app.inline_swarm_members();
        let spinner_frame =
            (app.animation_elapsed() * wvc_tui_render::swarm_gallery::STRIP_SPINNER_FPS) as usize;
