...
fn test_render_file_activity_as_collapsible_compact_row() {
    crate::tui::markdown::set_center_code_blocks(false);
    let content = wvc_tui_messages::encode_collapsible_swarm_content(
...
fn test_light_theme_adapted_frame_has_readable_contrast() {
    ...
    terminal
        .draw(|frame| {
            ...
            wvc_tui_style::theme_mode::adapt_buffer(
                frame.buffer_mut(),
                wvc_tui_style::ThemeMode::Light,
            );
        })
        .expect("draw");
...
fn test_configured_palette_recolors_a_real_rendered_frame() {
    ...
    let mut palette = wvc_tui_style::Palette::default();
    palette.set(wvc_tui_style::Role::User, (250, 40, 200));
