    ...
    let state = TestState {
        ...
        batch_progress: Some(crate::bus::BatchProgress {
            ...
            running: vec![ToolCall {
                ...
                name: "bash".to_string(),
                input: serde_json::json!({
                    "command": "cargo test --package wvc --lib tui::ui::tests::render_tool_message_batch_rows_do_not_soft_wrap_on_narrow_width -- --nocapture --exact with-extra-flags-and-output-to-stretch-the-line"
                }),
                intent: None,
                thought_signature: None,
            }],
...
fn test_prepare_messages_renders_anchored_reasoning_message_in_flow() {
    ...
    trace.push_str(&wvc_tui_markdown::reasoning_line_markup(
        "anchored thinking",
    ));
    ...
    assert!(
        !joined[reasoning_idx].contains(wvc_tui_markdown::REASONING_SENTINEL),
