...
pub(super) fn status_spinner_only_symbol(app: &App) -> Option<&'static str> {
    ...
    if status_uses_primary_spinner(&app.status) {
        Some(wvc_tui_style::theme::activity_indicator(
            status_spinner_elapsed(app),
            STATUS_SPINNER_FPS,
...
fn render_status_spinner_into_buffer(buffer: &Buffer, area: Rect, symbol: &str) -> bool {
    area.width > 0
        ...
        && buffer
            .cell((area.x, area.y))
            .is_some_and(|cell| wvc_tui_style::theme::is_activity_indicator_frame(cell.symbol()))
        && !symbol.is_empty()
}
...
fn render_status_spinner_into_buffer_mut(buffer: &mut Buffer, area: Rect, symbol: &str) {
    buffer.set_stringn(
        ...
        Style::default().fg(wvc_tui_style::adapt_color_for_theme(
            wvc_tui_style::theme::ai_color(),
        )),
    );
}
...
mod tests {
    ...
    fn status_spinner_fast_path_uses_status_elapsed_clock() {
        ...
        let fast_path_symbol = wvc_tui_style::theme::activity_indicator(
