...
//!
//! 1. **Rare-hotkey feedback.** When the user presses a chord wvc recognizes
...
/// A chord wvc responds to, with a human description of what it does.
#[derive(Debug, Clone)]
...
/// Resolve what a chord does, or `None` when wvc has no binding for it.
pub(super) fn lookup(
...
impl App {
    ...
    pub(super) fn handle_hotkeys_command(&mut self, trimmed: &str) -> bool {
        ...
        self.push_display_message(wvc_tui_messages::DisplayMessage::system(listing));
        true
    }
    ...
    pub(crate) fn observe_known_hotkey(
        ...
        let macos_option_char = cfg!(target_os = "macos")
            && modifiers.is_empty()
            && wvc_tui_core::keybind::macos_option_char_to_ascii_key(code).is_some();
