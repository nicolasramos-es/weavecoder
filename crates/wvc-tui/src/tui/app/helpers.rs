...
pub(super) fn launch_client_executable() -> PathBuf {
    crate::build::client_update_candidate(wvc_selfdev_types::client_selfdev_requested())
...
pub(super) fn inferred_reasoning_efforts(
    ...
    wvc_provider_core::inferred_reasoning_efforts(provider_name, model_name)
}
...
pub(super) fn build_resume_command(
    ...
    match target {
        ...
        ResumeTarget::CodexSession { session_id, .. } => {
            ...
            let args = resume_invocation_args(&imported_id, socket);
            let title = format!("🧠 Codex {}", wvc_core::util::truncate_str(session_id, 8));
            (exe, args, title)
        }
        ...
        ResumeTarget::OpenCodeSession { session_id, .. } => {
            ...
            let args = resume_invocation_args(&imported_id, socket);
            let title = format!("◌ OpenCode {}", wvc_core::util::truncate_str(session_id, 8));
            (exe, args, title)
        }
        ResumeTarget::CursorSession { session_id, .. } => {
            ...
            let args = resume_invocation_args(&imported_id, socket);
            let title = format!("▮ Cursor {}", wvc_core::util::truncate_str(session_id, 8));
            (exe, args, title)
        }
    }
}
...
fn build_fresh_session_command(socket: Option<&str>) -> crate::terminal_launch::TerminalCommand {
    ...
    crate::terminal_launch::TerminalCommand::new(&exe, args)
        .title("wvc · new session".to_string())
        .kind("new-terminal")
        .fresh_spawn()
}
...
pub(super) fn clipboard_image() -> Option<(String, String)> {
            ...
            && !html_output.stdout.is_empty()
        {
            ...
            if let Some(url) = extract_image_url(&html) {
                crate::logging::info(&format!(
                    ...
                    wvc_core::util::truncate_str(&url, 80)
                ));
    ...
    #[cfg(target_os = "macos")]
    {
        let temp_path = std::env::temp_dir().join("wvc_clipboard.png");
        let script = format!(
