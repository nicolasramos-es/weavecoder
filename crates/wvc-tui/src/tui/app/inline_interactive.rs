...
struct RemoteModelCatalogCache {
    ...
    origin: String,
    #[serde(flatten)]
    snapshot: wvc_provider_core::ModelCatalogSnapshot,
...
fn model_picker_route_is_current(
    ...
    wvc_provider_core::model_route_provider_labels_match(&route.provider, current_provider)
}
...
fn model_picker_route_is_recommended(model_name: &str, route: &PickerOption) -> bool {
    RECOMMENDED_MODELS.contains(&model_name)
        && wvc_provider_core::model_route_metadata_is_recommended(
            model_name,
            &route.provider,
            &route.api_method,
            route.available,
...
fn model_picker_route_provider_matches_key(
    ...
    wvc_provider_core::model_route_provider_matches_key(
        route_provider_key,
...
fn model_picker_route_is_default(
    ...
    if let Some((bare_model, provider_label)) = default_model.rsplit_once('@') {
        return bare_model == model_name
            && wvc_provider_core::model_route_provider_labels_match(
                &route.provider,
                provider_label,
            );
    }
...
impl App {
    ...
    fn open_model_picker_with_routes(
        ...
        fn route_matches_recent_auth(route_provider: &str, login_provider: &str) -> bool {
            wvc_provider_core::model_route_provider_labels_related(route_provider, login_provider)
        }
        ...
        for target in targets {
            ...
            let name = match target {
                ...
                ResumeTarget::CodexSession { session_id, .. } => {
                    format!("Codex {}", wvc_core::util::truncate_str(session_id, 8))
                }
                ...
                ResumeTarget::OpenCodeSession { session_id, .. } => {
                    format!("OpenCode {}", wvc_core::util::truncate_str(session_id, 8))
                }
    ...
    fn handle_live_claude_takeover(&mut self, target: &ResumeTarget) -> bool {
        ...
        let display_id = wvc_core::util::truncate_str(session_id, 8);
    ...
    pub(super) fn picker_fuzzy_score(pattern: &str, text: &str) -> Option<i32> {
        wvc_fuzzy::fuzzy_score_tokens(pattern, text)
    }
    ...
    pub(super) fn apply_inline_interactive_filter(picker: &mut InlineInteractiveState) {
        if picker.filter.is_empty() {
            ...
            let prepared = wvc_fuzzy::PreparedTokenQuery::new(&picker.filter);
...
mod tests {
    ...
    fn model_picker_current_route_allows_provider_aliases() {
        assert!(wvc_provider_core::model_route_provider_labels_match(
            "Anthropic",
            "Claude"
        ));
        assert!(wvc_provider_core::model_route_provider_labels_match(
    ...
    fn remote_model_catalog_cache_rejects_stale_and_future_timestamps() {
        let snapshot = wvc_provider_core::ModelCatalogSnapshot::new(
    ...
    fn remote_model_catalog_cache_rejects_forged_or_oversized_routes() {
        let safe_snapshot = wvc_provider_core::ModelCatalogSnapshot::new(
