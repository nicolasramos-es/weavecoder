...
impl App {
    ...
    fn failover_config_hint() -> &'static str {
        "To turn this off, set [provider].cross_provider_failover = \"manual\" in ~/.wvc/config.toml or export WVC_CROSS_PROVIDER_FAILOVER=manual."
    }
    ...
    fn handle_provider_failover_prompt(&mut self, prompt: crate::provider::ProviderFailoverPrompt) {
            ...
            .cross_provider_failover
        {
            ...
            crate::config::CrossProviderFailoverMode::Countdown if !self.is_remote => {
                ...
                self.push_display_message(DisplayMessage::system(format!(
                    "⚠ {} became unavailable - wvc will switch to {} in 3 seconds unless you cancel.\n\nReason: {}\n\nRetrying would send {}. Press Esc to cancel.\n\n{}",
                    prompt.from_label,
                    prompt.to_label,
                    prompt.reason,
                    input_summary,
    ...
    fn current_route_api_method(&self) -> Option<String> {
        ...
        match (provider_name.as_str(), credential) {
            ("claude", Some(wvc_provider_core::ResolvedCredential::Oauth)) => {
                Some("claude-oauth".to_string())
            }
            ("claude", Some(wvc_provider_core::ResolvedCredential::ApiKey)) => {
                Some("claude-api".to_string())
            }
            ("openai", Some(wvc_provider_core::ResolvedCredential::Oauth)) => {
                Some("openai-oauth".to_string())
            }
            ("openai", Some(wvc_provider_core::ResolvedCredential::ApiKey)) => {
    ...
    fn is_guardrail_reroute_model(model: &str) -> bool {
        let canonical = wvc_provider_core::model_id::canonical(model);
        wvc_provider_core::model_id::strip_date_suffix(&canonical) == GUARDRAIL_REROUTE_MODEL
    }
    ...
    pub(super) fn effective_context_tokens_from_usage(
        ...
        // Shared heuristic (wvc-compaction-core): keeps the sidebar figure
    ...
    pub(super) fn update_compaction_usage_from_stream(&mut self) {
        if self.is_remote || !self.provider.uses_wvc_compaction() {
            return;
        }
