...
fn swarm_prompt_edit_path_prefers_nonblank_project_override() {
    ...
    std::fs::write(wvc_home.path().join("swarm-prompt.md"), "global routing")
        .expect("write global prompt");

    let path = ensure_swarm_prompt_edit_path(project.path().to_str(), wvc_home.path())
...
fn swarm_prompt_edit_path_falls_back_to_nonblank_global_override() {
    ...
    let global_prompt = wvc_home.path().join("swarm-prompt.md");
...
fn swarm_prompt_edit_path_materializes_builtin_default_globally() {
    ...
    assert_eq!(path, wvc_home.path().join("swarm-prompt.md"));
...
mod colors {
    ...
    fn with_clean_config(body: impl FnOnce()) {
        ...
        impl Drop for Restore {
            fn drop(&mut self) {
                ...
                let _ = config.save();
                wvc_tui_style::set_palette(wvc_tui_style::Palette::default());
            }
        }
    ...
    fn colors_lists_every_role_with_its_hex_value() {
        ...
        for role in wvc_tui_style::ALL_ROLES.iter().copied() {
            assert!(
    ...
    fn setting_a_role_persists_it_and_reports_the_new_score() {
        with_clean_config(|| {
            ...
            assert!(
                wvc_tui_style::palette().is_overridden(wvc_tui_style::Role::Error),
    ...
    fn generate_writes_a_complete_palette_and_scores_it() {
        with_clean_config(|| {
            ...
            for role in wvc_tui_style::ALL_ROLES.iter().copied() {
                ...
                assert!(
                    wvc_tui_style::palette::parse_hex(value).is_some(),
