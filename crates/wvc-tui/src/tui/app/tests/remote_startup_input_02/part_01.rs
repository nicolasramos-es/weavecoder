...
#[test]
fn test_remote_wvc_subscription_catalog_is_not_augmented_with_local_auth_routes() {
    with_temp_wvc_home(|| {
...
#[test]
fn test_remote_mixed_catalog_keeps_wvc_subscription_separate_from_other_providers() {
    ...
    // `with_temp_wvc_home` takes the shared env lock and points auth at an
...
fn test_remote_hydrated_catalog_adds_entitled_wvc_subscription_routes() {
    with_temp_wvc_home(|| {
        ...
        let expected = crate::subscription_catalog::curated_models()
            ...
            .map(|model| model.id)
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(wvc_routes.len(), expected.len());
        assert_eq!(
            wvc_routes
                .iter()
                .map(|route| route.model.as_str())
