...
fn cleanup_background_task_files(task_id: &str) {
    let task_dir = std::env::temp_dir().join("wvc-bg-tasks");
...
/// `with_temp_wvc_home` hold this same non-reentrant mutex for their whole
...
fn clear_persisted_test_ui_state() {
    if let Ok(home) = crate::storage::wvc_dir() {
...
fn create_wvc_repo_fixture() -> tempfile::TempDir {
