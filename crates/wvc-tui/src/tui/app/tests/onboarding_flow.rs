    ...
    fn model_routes(&self) -> Vec<crate::provider::ModelRoute> {
        vec![
            crate::provider::ModelRoute {
                model: wvc_provider_core::DEFAULT_CLAUDE_MODEL.to_string(),
            ...
            crate::provider::ModelRoute {
                model: wvc_provider_core::DEFAULT_OPENAI_MODEL.to_string(),
                provider: "OpenAI".to_string(),
                api_method: "openai-api-key".to_string(),
                available: true,
                detail: String::new(),
...
fn import_failure_h_key_prepares_agent_repair_brief() {
    ...
    with_temp_wvc_home(|| {
        ...
        assert!(brief.contains("wvc auth-test --provider openai --json"), "{brief}");
        ...
        assert!(on_disk.contains("wvc auth-test --provider openai --json"), "{on_disk}");
...
fn import_continue_reaches_ready_quality_first_openai_model() {
    ...
    with_temp_wvc_home(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(model, wvc_provider_core::DEFAULT_OPENAI_MODEL);
