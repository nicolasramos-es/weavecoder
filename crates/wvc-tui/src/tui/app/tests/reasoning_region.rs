...
fn reasoning_region_emits_dim_italic_lines_no_gutter_header_or_footer() {
    ...
    with_reasoning_current_home(|| {
        ...
        let sentinel = wvc_tui_markdown::REASONING_SENTINEL;
        assert!(
...
fn reasoning_region_closes_before_normal_output() {
    ...
    with_reasoning_current_home(|| {
        ...
        assert!(
            !answer_line.contains(wvc_tui_markdown::REASONING_SENTINEL),
        ...
        assert!(
            !text.contains(wvc_tui_markdown::REASONING_SENTINEL),
...
fn reasoning_region_renders_dim_italic_text_without_gutter() {
    ...
    assert!(
        !rendered.contains(wvc_tui_markdown::REASONING_SENTINEL),
...
fn strip_reasoning_lines_removes_reasoning_keeps_answer() {
    ...
    content.push_str(&wvc_tui_markdown::reasoning_line_markup("thinking one"));
    content.push_str(&wvc_tui_markdown::reasoning_line_markup("thinking two"));
    content.push('\n');
    content.push_str("Here is the answer.\n");

    let stripped = strip_reasoning_lines(&content);
    assert_eq!(stripped, "Here is the answer.");
    assert!(!stripped.contains(wvc_tui_markdown::REASONING_SENTINEL));
}
...
fn strip_reasoning_lines_reasoning_only_becomes_empty() {
    ...
    let mut content = String::new();
    content.push_str(&wvc_tui_markdown::reasoning_line_markup("only thinking"));
    let stripped = strip_reasoning_lines(&content);
...
fn multiple_reasoning_blocks_anchor_in_order_and_clear_next_prompt() {
    // Hermetic WVC_HOME: these assertions depend on the default
    ...
    with_reasoning_current_home(|| {
        ...
        assert!(
            !app.streaming_text()
                .contains(wvc_tui_markdown::REASONING_SENTINEL),
...
fn anchored_trace_never_moves_and_clears_on_next_prompt() {
    ...
    // not leaking in from the developer's real ~/.wvc (other tests
