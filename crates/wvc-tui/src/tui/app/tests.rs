...
fn kv_cache_baseline_from_other_session_is_ignored() {
    ...
    // ~/.wvc/logs KV_CACHE_USAGE telemetry (common_prefix=0, current
...
fn version_command_shows_remote_server_identity_and_update_status() {
    ...
    assert!(content.contains("wvc client:"), "{content}");
...
fn skills_command_lists_loaded_and_endorsed_skills() {
    ...
    assert!(
        content.contains("Endorsed skills (recommended by wvc)"),
        "{content}"
    );
...
fn skills_command_refreshes_registry_from_disk_before_listing() {
    ...
    let skill_dir = temp.path().join(".wvc").join("skills").join("late-skill");
...
fn skill_invocation_with_prompt_activates_and_submits_in_one_turn() {
    let mut app = create_test_app();
    let temp = tempfile::tempdir().expect("tempdir");
    let skill_dir = temp.path().join(".wvc/skills/prompt-skill");
    std::fs::create_dir_all(&skill_dir).expect("create skill dir");
    std::fs::write(
        skill_dir.join("SKILL.md"),
...
fn skill_invocation_with_prompt_attaches_pending_image_to_user_message() {
    let mut app = create_test_app();
    let temp = tempfile::tempdir().expect("tempdir");
    let skill_dir = temp.path().join(".wvc/skills/image-skill");
    std::fs::create_dir_all(&skill_dir).expect("create skill dir");
    std::fs::write(
        skill_dir.join("SKILL.md"),
...
fn stale_server_history_is_deferred_before_remote_state_is_applied() {
    crate::env::remove_var("WVC_ALLOW_SERVER_VERSION_MISMATCH");
    let mut app = create_test_app();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let _guard = rt.enter();
    let mut remote = crate::tui::backend::RemoteConnection::dummy();
...
fn deferred_stale_server_history_captures_session_id_for_reload_handoff() {
    ...
    crate::env::remove_var("WVC_ALLOW_SERVER_VERSION_MISMATCH");
    crate::env::set_var("WVC_TEST_CLIENT_VERSION_OVERRIDE", "v0.21.0 (deadbeef)");
    ...
    crate::env::remove_var("WVC_TEST_CLIENT_VERSION_OVERRIDE");
}
...
fn ancient_server_history_is_deferred_via_client_side_release_check() {
    ...
    crate::env::set_var("WVC_TEST_CLIENT_VERSION_OVERRIDE", "v0.17.0 (d741696f)");
    ...
    let content = app.display_messages().last().unwrap().content.clone();
    assert!(
        content.contains("older release") && content.contains("wvc server stop"),
        "{content}"
    );
}
...
fn older_server_reporting_no_update_is_still_deferred_via_client_check() {
    ...
    let _env_guard = crate::storage::lock_test_env();
    crate::env::remove_var("WVC_ALLOW_SERVER_VERSION_MISMATCH");
    crate::env::set_var("WVC_TEST_CLIENT_VERSION_OVERRIDE", "v0.22.0 (abcd1234)");
...
fn older_server_history_repairs_stale_shared_server_channel_end_to_end() {
    // Full-path sandbox: a real temp WVC_HOME set up in the exact field state
