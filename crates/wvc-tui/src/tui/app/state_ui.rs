...
impl App {
    ...
    pub(super) fn save_input_for_reload(&self, session_id: &str) {
            ...
            && !self.todo_confidence_spike_challenged
        {
            ...
            if let Ok(wvc_dir) = crate::storage::wvc_dir() {
                let path = wvc_dir.join(format!("client-input-{}", session_id));
    ...
    pub(super) fn restore_input_for_reload(session_id: &str) -> Option<RestoredReloadInput> {
        let wvc_dir = crate::storage::wvc_dir().ok()?;
        let path = wvc_dir.join(format!("client-input-{}", session_id));
        if !path.exists() {
            return None;
        }
    ...
    pub fn debug_socket_path() -> std::path::PathBuf {
        crate::storage::runtime_dir().join("wvc-debug.sock")
    }
}
...
pub(super) fn handle_info_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/version" {
        let version = wvc_build_meta::version();
        let is_canary = if app.session.is_canary {
            " (canary/self-dev)"
        } else {
            ""
        };
        let mut content = format!("wvc client: {}{}", version, is_canary);
        if app.is_remote {
    ...
    if trimmed == "/info" {
        ...
        if let Some(ref provider_id) = app.provider_session_id {
            info.push_str(&format!(
                "Provider Session: {}...\n",
                wvc_core::util::truncate_str(provider_id, 16)
            ));
        }
