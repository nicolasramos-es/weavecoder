...
pub(crate) fn build_repair_brief(
    ...
    brief.push_str(
        "wvc (a terminal coding agent) could not import or validate a login \
...
mod tests {
    ...
    fn brief_lists_the_non_interactive_commands_and_log_path() {
        ...
        assert!(
            brief.contains("wvc auth-test --provider openai --json"),
            "{brief}"
        );
        assert!(brief.contains("wvc login --provider openai"), "{brief}");
        assert!(brief.contains("--api-key-stdin"), "{brief}");
        assert!(brief.contains("wvc provider add"), "{brief}");
        // Points at the logs.
    ...
    fn brief_without_provider_or_agent_is_still_actionable() {
        ...
        assert!(
            brief.contains("wvc auth-test --all-configured --json"),
            "{brief}"
        );
