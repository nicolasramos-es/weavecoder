...
impl App {
    ...
    pub(super) fn update_cost_impl(&mut self) {
        ...
        let pinned_anthropic = wvc_provider_core::pinned_mode_for(
            wvc_provider_core::DualAuthProvider::Anthropic,
            runtime_provider.as_deref(),
        );
        let pinned_openai = wvc_provider_core::pinned_mode_for(
        ...
        let is_explicit_openai_api =
            matches!(pinned_openai, Some(wvc_provider_core::AuthMode::ApiKey));
        let is_explicit_openai_oauth =
            matches!(pinned_openai, Some(wvc_provider_core::AuthMode::Oauth));
