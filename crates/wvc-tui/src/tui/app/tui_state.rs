    ...
    fn dual_credential_active(
        ...
        if route.is_remote {
            ...
            return self
                ...
                .route_api_method
                .as_deref()
                .and_then(wvc_provider_core::AuthRoute::parse)
...
impl crate::tui::TuiState for App {
    ...
    fn status_notice(&self) -> Option<String> {
        if !self.is_remote
            && self.provider.uses_wvc_compaction()
    ...
    fn context_snapshot(&self) -> crate::tui::ContextSnapshot {
        ...
        let (compaction_count, compaction_summary_chars, is_compacting, compaction_fresh) =
            if self.is_remote {
                (0, 0, false, true)
            } else if self.provider.uses_wvc_compaction() {
        ...
        if self.is_remote {
            ...
            let skip = if self.provider.uses_wvc_compaction() {
    ...
    fn info_widget_data(&self) -> crate::tui::info_widget::InfoWidgetData {
                ...
                || !managed_members.is_empty()
            {
                ...
                Some(crate::tui::info_widget::SwarmInfo {
                    ...
                    spinner_frame: (self.animation_elapsed()
                        * wvc_tui_render::swarm_gallery::STRIP_SPINNER_FPS)
                        as usize,
                    managed_members,
                })
            } else {
        ...
        let compaction_info = if !self.is_remote && self.provider.uses_wvc_compaction() {
            let compaction = self.registry.compaction();
        ...
        crate::tui::info_widget::InfoWidgetData {
            ...
            cache_hit_info,
            compaction_info,
            is_compacting: if !self.is_remote && self.provider.uses_wvc_compaction() {
                let compaction = self.registry.compaction();
                compaction
                    .try_read()
