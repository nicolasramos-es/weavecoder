...
impl App {
    ...
    pub(in crate::tui::app) fn append_reasoning_text(&mut self, text: &str) {
        ...
        for ch in text.chars() {
            if ch == '\n' {
                ...
                committed.push_str(&wvc_tui_markdown::reasoning_line_markup(&line));
            } else {
        ...
        let partial = wvc_tui_markdown::reasoning_partial_markup(&self.reasoning_pending_line);
    ...
    pub(in crate::tui::app) fn close_reasoning_region(&mut self, _footer: Option<String>) {
        ...
        if !pending.is_empty() {
            self.streaming
                .streaming_text
                .push_str(&wvc_tui_markdown::reasoning_line_markup(&pending));
        }
