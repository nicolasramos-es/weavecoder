...
//! `~/.wvc/config.toml`. This command is the interactive front end for that:
...
//! [`wvc_tui_style::harmony`] so a user gets specific, actionable feedback
...
fn active_background() -> (u8, u8, u8) {
    if wvc_tui_style::is_light_theme() {
        (255, 255, 255)
    } else {
        (18, 18, 18)
    }
...
fn show_harmony(app: &mut App) {
    ...
    let report = wvc_tui_style::analyze_harmony(&configured_palette(), background);
...
fn generate_palette(app: &mut App, seed: Option<&str>) {
    ...
    let background = active_background();
    let generated = wvc_tui_style::harmony::generate_from_seed(seed_rgb, background);
    let report = wvc_tui_style::analyze_harmony(&generated, background);
...
/// concurrent config edit by another wvc session is not clobbered.
fn persist(
