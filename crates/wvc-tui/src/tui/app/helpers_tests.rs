...
#[test]
fn build_resume_command_uses_imported_wvc_session_for_claude_code() {
...
#[test]
fn build_resume_command_uses_imported_wvc_session_for_codex() {
    let _pinned = pinned_resume_test_home();
    let (program, args, title) = build_resume_command(
...
fn fresh_session_command_includes_fresh_spawn_and_socket() {
    ...
    assert_eq!(command.title.as_deref(), Some("wvc · new session"));
    assert_eq!(
        command.args,
        vec![
            "--fresh-spawn".to_string(),
