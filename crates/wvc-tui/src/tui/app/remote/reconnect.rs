        ...
        Err(e) => {
            if state.reconnect_attempts == 0 && !app.server_spawning {
                return Err(anyhow::anyhow!(
                    "Failed to connect to server. Is `wvc serve` running? Error: {}",
                    e
                ));
            }
    ...
    if state.reconnect_attempts > 0 {
        ...
        if must_reload_client {
            ...
            if (has_reload_ctx_for_session || !app.reload_info.is_empty())
                && let Ok(wvc_dir) = crate::storage::wvc_dir()
            {
                let marker = wvc_dir.join(format!("client-reload-pending-{}", session_id));
...
pub(super) fn load_reload_reconnect_hints(
    ...
    let has_client_reload_marker = session_to_resume
        .and_then(|sid| {
            ...
            let marker = wvc_dir.join(format!("client-reload-pending-{}", sid));
            if marker.exists() {
