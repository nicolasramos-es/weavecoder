...
impl App {
    ...
    pub(super) fn update_terminal_title(&self) {
        ...
        let server_name = self.remote_server_short_name.as_deref().unwrap_or("wvc");
        ...
        let fallback_label = if server_name.eq_ignore_ascii_case("wvc") {
            format!("wvc {session_label}")
        } else {
            format!("wvc/{} {session_label}", server_name.to_lowercase())
        };
        if server_name.eq_ignore_ascii_case("wvc") {
    ...
    /// would `wvc --resume <bogus-id>` and crash with "No session found
    ...
    pub fn restore_session(&mut self, session_id: &str) {
        ...
        if let Ok(session) = Session::load(session_id) {
            ...
            for item in wvc_tui_messages::display_messages_from_rendered_messages(
...
pub(super) fn handle_dev_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/restart" {
        app.push_display_message(DisplayMessage {
            role: "system".to_string(),
            content: "Restarting wvc (same binary, session preserved)...".to_string(),
            tool_calls: vec![],
            duration_secs: None,
            title: None,
            tool_data: None,
    ...
    if trimmed == "/z" || trimmed == "/zz" || trimmed == "/zzz" || trimmed == "/zstatus" {
        ...
        if trimmed == "/zstatus" {
            ...
            let env = std::env::var("WVC_COPILOT_PREMIUM").ok();
