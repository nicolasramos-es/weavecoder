...
impl ActiveCredentialOverrides {
    fn from_app(app: &dyn TuiState) -> Self {
        Self {
            anthropic: app.active_dual_credential(wvc_provider_core::ActiveProvider::Claude),
            openai: app.active_dual_credential(wvc_provider_core::ActiveProvider::OpenAI),
        }
    }

    fn get(
        &self,
        provider: wvc_provider_core::ActiveProvider,
    ) -> Option<crate::auth::ActiveCredential> {
        match provider {
            wvc_provider_core::ActiveProvider::Claude => self.anthropic,
            wvc_provider_core::ActiveProvider::OpenAI => self.openai,
            _ => None,
        }
    }
}
...
fn auth_full_specs(
    ...
    let anthropic_label = provider_label(
        ...
        dual_method_label(wvc_provider_core::ActiveProvider::Claude, auth, active),
    );

    let openai_label = provider_label(
        "openai",
        auth.openai,
        dual_method_label(wvc_provider_core::ActiveProvider::OpenAI, auth, active),
    );
...
fn header_provider_auth_tag(
    ...
    if let Some(provider) = wvc_provider_core::parse_provider_hint(name) {
        use crate::auth::{ActiveCredential, resolve_dual_credential_auth};
        match resolve_dual_credential_auth(provider, auth, runtime_provider.as_deref()) {
            ...
            None if matches!(
                provider,
                wvc_provider_core::ActiveProvider::Claude
                    | wvc_provider_core::ActiveProvider::OpenAI
            ) =>
            {
                return "";
            }
...
fn version_display_candidates() -> Vec<String> {
    let full = format!("wvc {}", semver());
    let core = format!("wvc {}", semver_core());
    let minor = format!("wvc {}", semver_minor());
...
fn build_persistent_header_with_auth(
    ...
    let client_version_full = server_name
        .as_ref()
        .map(|_| wvc_build_meta::version().to_string());
...
mod tests {
    ...
    fn ensure_test_wvc_home_if_unset() {
        ...
        let path = TEST_HOME.get_or_init(|| {
            let path = std::env::temp_dir().join(format!("wvc-test-home-{}", std::process::id()));
    ...
    fn persistent_header_labels_server_and_client_versions() {
        ...
        let client_version = compact_version_label(wvc_build_meta::version());
        assert!(
    ...
    fn persistent_header_keeps_git_hash_when_semvers_match_but_builds_differ() {
        let mut app = create_test_app();
        let client_semver = compact_version_label(wvc_build_meta::version());
        ...
        assert!(
            client_line.contains(&format!("· {}", wvc_build_meta::version())),
