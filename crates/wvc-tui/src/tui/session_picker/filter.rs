...
impl SessionPicker {
    ...
    pub(super) fn session_is_claude_code(session: &SessionInfo) -> bool {
        wvc_tui_session_picker::session_is_claude_code(session.source, &session.id)
    }

    pub(super) fn session_is_codex(session: &SessionInfo) -> bool {
        wvc_tui_session_picker::session_is_codex(session.source, session.model.as_deref())
    }

    pub(super) fn session_is_pi(session: &SessionInfo) -> bool {
        wvc_tui_session_picker::session_is_pi(
            session.source,
    ...
    pub(super) fn session_is_open_code(session: &SessionInfo) -> bool {
        wvc_tui_session_picker::session_is_open_code(
            session.source,
            session.provider_key.as_deref(),
        )
    }
    ...
    pub(super) fn session_is_cursor(session: &SessionInfo) -> bool {
        wvc_tui_session_picker::session_is_cursor(session.source, session.provider_key.as_deref())
    }
