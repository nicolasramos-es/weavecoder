...
impl SessionPicker {
    fn running_spinner_frame() -> usize {
        std::time::SystemTime::now()
            ...
            .saturating_div(u128::from(
                wvc_tui_render::swarm_gallery::STRIP_SPINNER_FRAME_MS,
            )) as usize
    }
    ...
    pub(super) fn render_session_list(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let spinner = wvc_tui_render::swarm_gallery::STRIP_SPINNER_FRAMES
            [spinner_frame % wvc_tui_render::swarm_gallery::STRIP_SPINNER_FRAMES.len()];
        ...
        let items: Vec<ListItem> = if let Some(message) = self.loading_message.as_deref() {
            ...
            && self.filter_mode == wvc_tui_session_picker::SessionFilterMode::Active
        {
            vec![
                ListItem::new(Line::from(vec![Span::styled(
        ...
        } else if self.filter_mode == wvc_tui_session_picker::SessionFilterMode::Active {
