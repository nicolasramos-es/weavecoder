...
fn benchmark_real_resume_loading_phases() {
    ...
    let sessions_dir = storage::wvc_dir().expect("wvc dir").join("sessions");
    ...
    let wvc_summary_elapsed = phase_start.elapsed();
    ...
    eprintln!(
        concat!(
            ...
            "wvc_summary={}ms wvc_loaded={} skipped_empty={} skipped_imported={} summary_errors={} ",
...
mod external_session_opt_out {
    ...
    fn external_sessions_default_is_on() {
        assert!(wvc_config_types::DisplayConfig::default().external_sessions);
    }
}
