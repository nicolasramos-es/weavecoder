...
fn session_list_disk_cache_path() -> Result<PathBuf> {
    Ok(storage::wvc_dir()?.join("cache/session-picker-list-v1.json"))
}
...
pub fn load_cached_sessions_grouped() -> Option<(Vec<ServerGroup>, Vec<SessionInfo>)> {
    let sessions_dir = storage::wvc_dir().ok()?.join("sessions");
    ...
    let (mut sessions, external_sessions) = std::thread::scope(|scope| {
        ...
        'fill: while start < candidates.len() {
            ...
            let parsed = parallel_map(batch, move |stem| {
                parse_wvc_session_info(sessions_dir_ref, &stem, catchup_ref)
            });
    ...
    sessions
        ...
        .map(|session| {
            ...
            let short_name = working_dir
                ...
                .unwrap_or_else(|| {
                    format!("claude {}", wvc_core::util::truncate_str(&session_id, 8))
                });
...
fn load_codex_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let working_dir = meta
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let short_name = format!("codex {}", wvc_core::util::truncate_str(&session_id, 8));
    let title = format!(
        "Codex session {}",
...
fn load_pi_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let working_dir = header
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let short_name = format!("pi {}", wvc_core::util::truncate_str(&session_id, 8));
    let title = format!(
        "Pi session {}",
        wvc_core::util::truncate_str(&session_id, 8)
    );
...
fn load_cursor_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let short_name = working_dir
        ...
        .and_then(|name| name.to_str())
        .map(|name| name.to_string())
        .unwrap_or_else(|| format!("cursor {}", wvc_core::util::truncate_str(&session_id, 8)));
