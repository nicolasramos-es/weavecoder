...
pub use wvc_tui_workspace::workspace_map;
pub use wvc_tui_workspace::workspace_map_widget;
...
pub trait TuiState {
    ...
    fn active_dual_credential(
        &self,
        _provider: wvc_provider_core::ActiveProvider,
...
pub(crate) fn subscribe_metadata(
    ...
    resolve_subscribe_metadata(
        ...
        remote_working_dir,
        wvc_selfdev_types::client_selfdev_requested(),
    )
}
...
pub(crate) fn resolve_subscribe_metadata(
    ...
    if !selfdev && let Some(dir) = client_working_dir {
        ...
        while let Some(path) = current {
            if crate::build::is_wvc_repo(path) {
                selfdev = true;
                break;
            }
            current = path.parent();
...
pub fn display_messages_from_session(session: &crate::session::Session) -> Vec<DisplayMessage> {
    let mut messages = wvc_tui_messages::display_messages_from_rendered_messages(
