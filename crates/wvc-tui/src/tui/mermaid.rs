...
pub use wvc_tui_mermaid::{ImageScrollBenchmark, cache_stat_syscalls};

#[cfg(feature = "mmdr-size-api")]
pub use wvc_tui_mermaid::terminal_theme;

pub fn install_wvc_mermaid_hooks() {
    wvc_tui_mermaid::set_log_hooks(crate::logging::info, crate::logging::warn);
    wvc_tui_mermaid::set_render_completed_hook(|| {
        crate::bus::Bus::global().publish(crate::bus::BusEvent::MermaidRenderCompleted);
    });
    wvc_tui_mermaid::set_memory_snapshot_hook(|| {
        let snapshot = crate::process_memory::snapshot_with_source("client:mermaid:memory");
        wvc_tui_mermaid::ProcessMemorySnapshot {
