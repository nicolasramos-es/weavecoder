...
pub(super) fn semver() -> &'static str {
    ...
    SEMVER.get_or_init(|| format!("v{}", wvc_build_meta::semver()))
}

/// True when this process is running from the stable release binary path.
/// Only matches the explicit ~/.wvc/builds/stable/wvc path, NOT
/// ~/.local/bin/wvc launcher path (which now points to current).
