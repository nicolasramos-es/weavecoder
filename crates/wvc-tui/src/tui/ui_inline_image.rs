...
fn prewarm_sender() -> &'static mpsc::SyncSender<PrewarmRequest> {
    PREWARM_TX.get_or_init(|| {
        ...
        if let Err(err) = std::thread::Builder::new()
            .name("wvc-inline-image-prewarm".to_string())
...
impl AnchoredInlineImages {
    ...
    pub(crate) fn unplaced_items(
        &self,
        messages: &[wvc_tui_messages::DisplayMessage],
    ) -> Vec<InlineImageItem> {
...
pub(crate) fn image_label_line(
    ...
    wvc_tui_render::truncate_line_preserving_suffix_to_width(
        &prefix,
        &Line::from(suffix),
        width as usize,
    )
...
fn image_fallback_note_line(width: u16) -> Line<'static> {
    wvc_tui_render::truncate_line_with_ellipsis_to_width(
...
mod tests {
    ...
    fn build_section_emits_one_fit_region_per_image_with_label() {
        ...
        for region in &section.image_regions {
            ...
            assert!(
                wvc_tui_render::line_plain_text(first).trim().is_empty(),
        ...
        let label_line = wvc_tui_render::line_plain_text(&section.wrapped_lines[1]);
        assert!(
    ...
    fn anchored_image_lines_round_trip_through_region_scan() {
        ...
        for offset in 1..rows as usize {
            ...
            assert!(
                wvc_tui_render::line_plain_text(line).trim().is_empty(),
