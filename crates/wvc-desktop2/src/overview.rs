...
mod tests {
    ...
    fn rows_are_by_working_dir_and_stable() {
        ...
        assert_eq!(field.rows[0].label, "wvc");
    ...
    fn left_and_right_stop_at_the_rows_ends() {
        ...
        for id in ["a1", "a2"] {
            ...
            assert_eq!(right.label, "wvc", "{id} wrapped into another row");
        }
    }
    ...
    fn up_and_down_change_rows() {
        ...
        assert_eq!(field.neighbor("b1", Dir::Up).unwrap().label, "wvc");
    ...
    fn dead_axes_are_reported_but_edges_are_not() {
        let field = field();
        // Two rows and three cards in "wvc": nothing is dead here, even at
        // the edges.
