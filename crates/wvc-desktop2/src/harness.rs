...
pub use wvc_sdk::{SocketState, Stage, describe_disconnect, explain};
...
fn run(
    ...
    wvc_sdk::ensure_runtime(&LaunchOptions::default(), &|status| {
...
fn to_entry(session: wvc_sdk::SessionInfo) -> crate::strip::Entry {
    crate::strip::Entry {
        session_id: session.session_id,
        title: session.title,
        working_dir: session.working_dir,
...
fn to_transcript(messages: Vec<wvc_sdk::HistoryMessage>) -> crate::transcript::Transcript {
