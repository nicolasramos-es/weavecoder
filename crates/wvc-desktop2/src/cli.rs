...
pub fn dispatch(args: &[String]) -> Option<Result<()>> {
    if args.iter().any(|arg| arg == "--version" || arg == "-V") {
        println!("wvc-desktop2 {}", wvc_build_meta::version());
        return Some(Ok(()));
    }
...
fn check_primary_selection() -> Result<()> {
    ...
    let sentinel = format!("wvc-primary-check-{}", std::process::id());
    ...
    while std::time::Instant::now() < deadline {
        ...
        match update {
            ...
            harness::HarnessUpdate::TurnDone if sent => {
                println!("\n[e2e] turn done");
                let out = std::env::temp_dir().join("wvc-desktop2-e2e.png");
...
fn check_reconnect() -> Result<()> {
    ...
    let runtime = std::env::temp_dir().join(format!("wvc-reconnect-{}", std::process::id()));
