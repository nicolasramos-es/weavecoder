...
//! - Markdown and LaTeX come from [`wvc_render_core`], the backend-neutral
...
fn checkbox_spans(spans: &[wvc_render_core::StyledSpan]) -> Vec<wvc_render_core::StyledSpan> {
...
fn table_lines(rows: &[Vec<String>], alignments: &[Alignment], columns: usize) -> Vec<StyledLine> {
    use wvc_render_core::{StyledSpan, TextAttrs};
...
fn strip_quote_bar(spans: &[wvc_render_core::StyledSpan]) -> Vec<wvc_render_core::StyledSpan> {
    let mut spans = spans.to_vec();
    if spans
        .first()
