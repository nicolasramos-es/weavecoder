...
use crate::{SCHEMA_VERSION, fts::*, relations::*, symbols::*};
...
impl CodeGraph {
    ...
    fn ensure_schema(&mut self) -> Result<()> {
        ...
        if current_version == 0 {
            ...
            // Additive migration: create_schema uses IF NOT EXISTS, so older
            // databases gain new tables (e.g. file_snapshots in v2).
            Self::create_schema(conn)?;
            conn.execute_batch(&format!("PRAGMA user_version = {};", SCHEMA_VERSION))?;
        } else if current_version > SCHEMA_VERSION {
            return Err(anyhow::anyhow!(
                "Schema version mismatch: database is v{}, newer than supported v{}",
                current_version,
                SCHEMA_VERSION
            ));
        }
    ...
    pub fn list_symbols(&self, query: SymbolQuery) -> Result<Vec<Symbol>> {
        ...
        let mut sql = String::from(
            "SELECT id, name, kind, file_path, line, col, language, doc, embedding FROM symbols",
        );
    ...
    /// List all relations in the database (used to rebuild the in-memory graph).
    ...
    /// Update the embedding BLOB for a symbol by ID.
    pub fn update_symbol_embedding(
        ...
        let conn = self.conn.as_mut().unwrap();
        conn.execute(
            "UPDATE symbols SET embedding = ?2 WHERE id = ?1",
            params![symbol_id, embedding],
        )?;
        Ok(())
    }

    /// Get the stored snapshot (hash/mtime/size) for a file, if any.
    pub fn get_snapshot(&self, file_path: &str) -> Result<Option<FileSnapshot>> {
        let conn = self.conn.as_ref().unwrap();
        let mut stmt = conn.prepare(
            "SELECT file_path, hash, mtime, size FROM file_snapshots WHERE file_path = ?1",
        )?;
    ...
    /// Insert or replace the snapshot for a file.
    pub fn upsert_snapshot(&mut self, snapshot: &FileSnapshot) -> Result<()> {
        let conn = self.conn.as_mut().unwrap();
        conn.execute(
            "INSERT OR REPLACE INTO file_snapshots (file_path, hash, mtime, size)
             VALUES (?1, ?2, ?3, ?4)",
            params![
                snapshot.file_path,
                snapshot.hash,
    ...
    /// Remove the snapshot for a file (called when the file is deleted).
    pub fn delete_snapshot(&mut self, file_path: &str) -> Result<()> {
        let conn = self.conn.as_mut().unwrap();
        conn.execute(
            "DELETE FROM file_snapshots WHERE file_path = ?1",
            params![file_path],
        )?;
        Ok(())
    }

    /// List all stored file snapshots.
    pub fn list_snapshots(&self) -> Result<Vec<FileSnapshot>> {
        let conn = self.conn.as_ref().unwrap();
        let mut stmt = conn.prepare("SELECT file_path, hash, mtime, size FROM file_snapshots")?;
        let rows = stmt.query_map([], |row| {
            Ok(FileSnapshot {
                file_path: row.get(0)?,
                hash: row.get(1)?,
    ...
    /// Delete all symbols (and their relations via CASCADE) belonging to a file.
    pub fn delete_symbols_for_file(&mut self, file_path: &str) -> Result<usize> {
        let conn = self.conn.as_mut().unwrap();
    ...
    pub fn symbol_count_for_file(&self, file_path: &str) -> Result<usize> {
        let conn = self.conn.as_ref().unwrap();
