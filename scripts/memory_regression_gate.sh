...
#
# Env overrides:
#   WVC_MEMGATE_SESSION               probe session id
#   WVC_MEMGATE_MAX_RSS_ANON_KB       idle rss_anon bound (default 56320 = 55 MiB)
#   WVC_MEMGATE_MAX_LIVE_HEAP_BYTES   idle allocated_bytes bound (default 47185920 = 45 MiB)
#
# Exit codes: 0 pass, 1 threshold exceeded, 2 could not measure, 3 skipped
# (probe session missing on this machine).
#
# Emits one machine-parseable line: `WVC_MEMGATE_RESULT {json}`.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SESSION_ID="${WVC_MEMGATE_SESSION:-session_hog_1783086065415_4ad4ae66cd43dd5b}"
IDLE_SECS=60
MAX_RSS_ANON_KB="${WVC_MEMGATE_MAX_RSS_ANON_KB:-56320}"
MAX_LIVE_HEAP_BYTES="${WVC_MEMGATE_MAX_LIVE_HEAP_BYTES:-47185920}"
...
if ! ls "$HOME/.wvc/sessions/${SESSION_ID}"* >/dev/null 2>&1; then
    echo "WVC_MEMGATE_RESULT {\"status\":\"skipped\",\"reason\":\"probe session ${SESSION_ID} not found\"}"
    exit 3
fi
...
PROBE_OUT="$("$SCRIPT_DIR/memory_probe.sh" --session "$SESSION_ID" --idle-secs "$IDLE_SECS" --skip-trim)" || {
    echo "WVC_MEMGATE_RESULT {\"status\":\"error\",\"reason\":\"memory_probe failed\"}"
    exit 2
}

IDLE_LINE="$(printf '%s\n' "$PROBE_OUT" | jq -c 'select(.phase=="idle")' | head -1)"
[[ -n "$IDLE_LINE" ]] || {
    echo "WVC_MEMGATE_RESULT {\"status\":\"error\",\"reason\":\"no idle phase in probe output\"}"
    exit 2
}
...
SERVER_INFO="$(jq -cn \
    --argjson rss "$(awk '/VmRSS/{print $2*1024}' "/proc/$(pgrep -f 'shared-server/wvc serve' | head -1)/status" 2>/dev/null || echo 0)" \
