...
bin=${WVC_AUTH_MATRIX_BIN:-}
out_dir=${WVC_AUTH_MATRIX_OUT:-"$repo_root/target/auth-test-reports"}
prompt=${WVC_AUTH_MATRIX_PROMPT:-"Reply with exactly AUTH_TEST_OK and nothing else. Do not call tools."}
providers=${WVC_AUTH_MATRIX_PROVIDERS:-"claude copilot openrouter deepseek zai alibaba-coding-plan openai-compatible"}
mode=${WVC_AUTH_MATRIX_MODE:-configured}
keep_going=${WVC_AUTH_MATRIX_KEEP_GOING:-1}
per_command_timeout=${WVC_AUTH_MATRIX_TIMEOUT:-90}

usage() {
  cat <<'EOF'
Usage: scripts/auth_regression_matrix.sh [options]

Runs wvc auth-test across the auth/provider matrix and writes one JSON report per provider.
...
Options:
  ...
  --bin PATH            wvc binary to run (default: cargo run --bin wvc --)
...
Environment equivalents:
  WVC_AUTH_MATRIX_BIN=/path/to/wvc
  WVC_AUTH_MATRIX_OUT=target/auth-test-reports
  WVC_AUTH_MATRIX_PROVIDERS="claude deepseek zai"
  WVC_AUTH_MATRIX_MODE=configured|all
  WVC_AUTH_MATRIX_LOGIN=1
  WVC_AUTH_MATRIX_NO_SMOKE=1
  WVC_AUTH_MATRIX_NO_TOOL_SMOKE=1
  WVC_AUTH_MATRIX_KEEP_GOING=0
  WVC_AUTH_MATRIX_TIMEOUT=90

Examples:
  scripts/auth_regression_matrix.sh --configured --no-smoke
  scripts/auth_regression_matrix.sh --provider deepseek --provider zai
  WVC_AUTH_MATRIX_BIN=target/selfdev/wvc scripts/auth_regression_matrix.sh --all
EOF
}
...
if [[ "${WVC_AUTH_MATRIX_LOGIN:-0}" == "1" ]]; then
  extra_args+=(--login)
fi
if [[ "${WVC_AUTH_MATRIX_NO_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-smoke)
fi
if [[ "${WVC_AUTH_MATRIX_NO_TOOL_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-tool-smoke)
fi
...
run_wvc() {
  if [[ -n "$bin" ]]; then
    ...
    timeout "$per_command_timeout" cargo run --quiet --bin wvc -- "$@"
  fi
}
...
if [[ "$mode" == "configured" ]]; then
  ...
  if ! run_wvc auth-test --all-configured --no-smoke --no-tool-smoke --json --output "$configured_json" >/tmp/wvc-auth-matrix-discovery.out 2>/tmp/wvc-auth-matrix-discovery.err; then
    if [[ -s "$configured_json" ]]; then
      ...
      cat /tmp/wvc-auth-matrix-discovery.err >&2 || true
