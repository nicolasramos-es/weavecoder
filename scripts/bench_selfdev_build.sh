...
#   2. leaf touch        - 1-line change in the root `wvc` bin crate
#   3. tui touch         - 1-line change in wvc-tui (largest UI crate)
#   4. core touch        - 1-line change in wvc-app-core (mid-stack crate)
#   5. commit blast      - simulate HEAD moving (WVC_BUILD_GIT_HASH change),
#                          which reruns wvc-build-meta's build script and
...
# `scripts/dev_cargo.sh build --profile selfdev -p wvc --bin wvc` succeeds.
#
...
build_cmd=(scripts/dev_cargo.sh build --profile selfdev -p wvc --bin wvc)
...
tui_file="crates/wvc-tui/src/lib.rs"
echo "3. tui touch (${tui_file}):"
touch_file "$tui_file"
run_build "tui" || true
revert_file "$tui_file"

core_file="crates/wvc-app-core/src/lib.rs"
echo "4. core touch (${core_file}):"
touch_file "$core_file"
run_build "core" || true
revert_file "$core_file"

echo "5. commit blast radius (simulated HEAD move via WVC_BUILD_GIT_HASH):"
fake_hash="bench$(date +%s | tail -c 4)"
start=$(date +%s.%N)
if WVC_BUILD_GIT_HASH="$fake_hash" "${build_cmd[@]}" >/tmp/bench_selfdev_build_commit-blast.log 2>&1; then
    end=$(date +%s.%N)
...
echo "    wvc-build-meta embeds GIT_HASH via env! and wvc-base/app-core/"
