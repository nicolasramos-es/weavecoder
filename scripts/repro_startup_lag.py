...
The hypothesis
--------------
A freshly spawned wvc feels laggy for the first few seconds and then becomes
...
def main() -> int:
    ...
    root = Path(tempfile.mkdtemp(prefix="wvc-startup-lag-"))
    run = root / "run"
    ...
    runtime = Path(env.get("WVC_RUNTIME_DIR") or f"/run/user/{os.getuid()}")
