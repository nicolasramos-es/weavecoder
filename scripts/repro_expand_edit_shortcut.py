...
closes that gap by driving the **real wvc binary** under a pseudo-terminal and
...
Everything runs in a throwaway WVC_HOME / runtime dir / socket. The user's
...
# protocol wvc turns on at startup):
...
# ──────────────────────────────────────────────────────────────────────────────
# A PTY-driven wvc client. This is the real live-render input path; bytes
...
def launch_client(binary: str, env: dict, session_id: str, name: str,
    ...
    cenv["WVC_DEBUG_CMD_PATH"] = str(debug_cmd)
    cenv["WVC_DEBUG_RESPONSE_PATH"] = str(debug_resp)
    proc = subprocess.Popen(
        [
            binary,
            "--no-update",
            "--no-selfdev",
            "--socket", env["WVC_SOCKET"],
            "--resume", session_id,
        ],
...
def main() -> int:
    ...
    root = Path(tempfile.mkdtemp(prefix="wvc-expand-edit-"))
    home = root / "home"
    run = root / "run"
