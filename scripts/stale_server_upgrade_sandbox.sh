...
#   Client: the freshly built current binary (target/debug/wvc, has the fix).
#   Field state: shared-server channel pinned to OLD (v0.14.6); stable -> NEW.
#
# It starts the real old daemon, then runs the NEW client's `wvc server reload`
# (which repairs the stale shared-server channel, then forces a reload). PASS iff
# the resulting daemon is running v0.22.x.
#
# Usage:
#   cargo build -p wvc --bin wvc
#   scripts/stale_server_upgrade_sandbox.sh
#
# Linux x86_64 only (uses the published wvc-linux-x86_64 release asset).
set -uo pipefail

REPO_ROOT="$(cd -- "$(dirname -- "$0")/.." && pwd)"
NEW_BIN="${NEW_BIN:-$REPO_ROOT/target/debug/wvc}"
OLD_VERSION="${OLD_VERSION:-v0.14.6}"
OLD_DIR="${OLD_DIR:-/tmp/wvc-sandbox}"
OLD_WRAP="$OLD_DIR/wvc-linux-x86_64"

[ -x "$NEW_BIN" ] || { echo "missing new client binary: $NEW_BIN (run: cargo build -p wvc --bin wvc)"; exit 2; }

# Fetch + extract the real old release binary if it is not already present.
if [ ! -x "$OLD_WRAP" ]; then
  mkdir -p "$OLD_DIR"
  url="$(curl -fsSL "https://api.github.com/repos/nicolasramos-es/weavecoder/releases/tags/$OLD_VERSION" \
        | grep -o 'https://[^"]*wvc-linux-x86_64.tar.gz' | head -1)"
...
SANDBOX="$(mktemp -d /tmp/wvc-stale-sandbox.XXXXXX)"
export WVC_HOME="$SANDBOX/home"
export WVC_RUNTIME_DIR="$SANDBOX/runtime"
# Hard isolation: pin the socket explicitly so we can NEVER touch the real
# global daemon at /run/user/<uid>/wvc.sock.
export WVC_SOCKET="$SANDBOX/runtime/wvc.sock"
# Make the new client's clean release version comparable (debug build is dirty).
export WVC_TEST_CLIENT_VERSION_OVERRIDE="v0.22.0 (sandbox)"
mkdir -p "$WVC_HOME" "$WVC_RUNTIME_DIR"

BUILDS="$WVC_HOME/builds"
...
cp "$OLD_DIR/wvc-linux-x86_64.bin" "$OLD_DIR/libssl.so.10" \
   "$OLD_DIR/libcrypto.so.10" "$BUILDS/versions/0.14.6/"
cat > "$BUILDS/versions/0.14.6/wvc" <<'WRAP'
#!/usr/bin/env sh
set -eu
real=$0
...
WRAP
chmod +x "$BUILDS/versions/0.14.6/wvc"

# --- Install the NEW binary as version 0.22.0 (newer mtime) ----------------
cp "$NEW_BIN" "$BUILDS/versions/0.22.0/wvc"
touch -d "+1 minute" "$BUILDS/versions/0.22.0/wvc"

# --- Field state: shared-server -> OLD, stable/current -> NEW --------------
ln -sf "../versions/0.14.6/wvc" "$BUILDS/shared-server/wvc"
echo "0.14.6" > "$BUILDS/shared-server-version"
ln -sf "../versions/0.22.0/wvc" "$BUILDS/stable/wvc"
echo "0.22.0" > "$BUILDS/stable-version"
ln -sf "../versions/0.22.0/wvc" "$BUILDS/current/wvc"
...
cleanup() {
  ...
  pkill -f "$BUILDS/versions/0.14.6/wvc-linux-x86_64.bin" 2>/dev/null || true
  pkill -f "$BUILDS/versions/0.22.0/wvc" 2>/dev/null || true
  rm -rf "$SANDBOX"
}
trap cleanup EXIT
...
"$BUILDS/shared-server/wvc" --no-update --provider antigravity serve \
...
# --- 2) New client: wvc server reload (repairs channel, then reloads) ----
log "Running NEW client: wvc server reload"
