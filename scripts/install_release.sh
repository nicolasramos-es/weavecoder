...
#
# Paths after install:
# - ~/.wvc/builds/versions/<hash>/wvc (immutable)
# - ~/.wvc/builds/stable/wvc -> .../versions/<hash>/wvc
# - ~/.wvc/builds/current/wvc -> .../versions/<hash>/wvc
# - ~/.local/bin/wvc -> ~/.wvc/builds/current/wvc (launcher)
set -euo pipefail

repo_root="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"

profile="${WVC_RELEASE_PROFILE:-release-lto}"
if [[ "${1:-}" == "--fast" ]]; then
  profile="release"
  shift
fi
...
bin="$repo_root/target/$profile/wvc"
...
# Install versioned binary into ~/.wvc/builds/versions/<hash>/
builds_dir="$HOME/.wvc/builds"
version_dir="$builds_dir/versions/$hash"
mkdir -p "$version_dir"
install -m 755 "$bin" "$version_dir/wvc"

# Update stable symlink
stable_dir="$builds_dir/stable"
mkdir -p "$stable_dir"
ln -sfn "$version_dir/wvc" "$stable_dir/wvc"
...
mkdir -p "$current_dir"
ln -sfn "$version_dir/wvc" "$current_dir/wvc"
printf '%s\n' "$hash" > "$builds_dir/current-version"

# Update launcher path to current channel
install_dir="${WVC_INSTALL_DIR:-$HOME/.local/bin}"
mkdir -p "$install_dir"
ln -sfn "$current_dir/wvc" "$install_dir/wvc"

echo "Installed: $version_dir/wvc"
echo "Updated stable symlink: $stable_dir/wvc -> $version_dir/wvc"
echo "Updated current symlink: $current_dir/wvc -> $version_dir/wvc"
echo "Updated launcher symlink: $install_dir/wvc -> $current_dir/wvc"
...
case "$(uname -s)" in
  Darwin|Linux)
    if "$install_dir/wvc" setup-hotkey </dev/null >/dev/null 2>&1; then
      echo "Configured system-wide wvc launch hotkeys (when supported)."
    fi
    ;;
esac
...
if [ "${WVC_SKIP_SERVER_RELOAD:-}" != "1" ]; then
  if "$install_dir/wvc" server reload </dev/null >/dev/null 2>&1; then
    echo "Reloaded the running wvc server onto $hash (if one was active)."
  fi
fi
...
wvc_configure_path "$install_dir"
