...
Checkpoints:
  ...
  cold_selfdev_build   selfdev wvc build after cargo clean
  warm_selfdev_edit    touched-file selfdev wvc build loop
USAGE
}
...
if [[ $skip_cold -eq 0 ]]; then
  ...
  cold_selfdev_json=$(run_bench cold_selfdev_build selfdev-wvc --cold)
fi

warm_check_json=$(run_bench warm_check_edit check --runs "$runs" --touch "$touch_path")
warm_selfdev_json=$(run_bench warm_selfdev_edit selfdev-wvc --runs "$runs" --touch "$touch_path")
