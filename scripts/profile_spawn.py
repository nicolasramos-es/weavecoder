#!/usr/bin/env python3
"""Benchmark + profile the time and resource utilization of a wvc spawn.

A wvc "spawn" is the full cold-launch path of the interactive client:
...
Everything runs under an isolated WVC_HOME / WVC_RUNTIME_DIR / WVC_SOCKET
so it never touches the user's real shared server, logs, sessions, or creds.

Two spawn shapes are profiled:

  server : the background daemon (`wvc serve`) on a private socket. This is
...
def isolated_env(root: str) -> dict[str, str]:
    env = os.environ.copy()
    env["WVC_HOME"] = os.path.join(root, "home")
    env["WVC_RUNTIME_DIR"] = os.path.join(root, "run")
    env["WVC_SOCKET"] = os.path.join(env["WVC_RUNTIME_DIR"], "wvc.sock")
    env["WVC_NO_TELEMETRY"] = "1"
    os.makedirs(env["WVC_HOME"], exist_ok=True)
    os.makedirs(env["WVC_RUNTIME_DIR"], exist_ok=True)
    return env
...
def find_child_server(env: dict[str, str], exclude: set[int]) -> int | None:
    ...
    for pid in pids:
        ...
        if not cmd:
            continue
        argv = cmd.split("\0")
        if "serve" in argv and any(env["WVC_SOCKET"] in a or "serve" == a for a in argv):
            # confirm it is our isolated daemon by socket env or socket arg
            envblob = _read(f"/proc/{pid}/environ") or ""
            if env["WVC_SOCKET"] in envblob or env["WVC_SOCKET"] in cmd:
                return pid
    return None
...
def profile_server_spawn(binary: str, sample_interval_s: float) -> dict | None:
    root = tempfile.mkdtemp(prefix="wvc-spawn-server-")
...
def profile_client_spawn(binary: str, sample_interval_s: float, hold_s: float) -> dict | None:
    if not _HAVE_PTY:
        return None
    root = tempfile.mkdtemp(prefix="wvc-spawn-client-")
    env = isolated_env(root)
    log_path = Path(env["WVC_HOME"]) / "logs" / f"wvc-{time.strftime('%Y-%m-%d')}.log"
    pid = None
    master_fd = None
    try:
        ...
        if pid == 0:  # child
            ...
            os.execve(
                binary,
                [binary, "--no-update", "--socket", env["WVC_SOCKET"]],
                env,
            )
            os._exit(127)
        ...
        while time.perf_counter() < deadline:
            ...
            if server_mon is not None:
                server_mon.poll()
                if server_ready_ms is None and socket_connectable(env["WVC_SOCKET"]):
...
def main() -> int:
    ...
    default_bin = os.environ.get("WVC_BENCH_BIN") or shutil.which("wvc") or "./target/release/wvc"
    ...
    print("=" * 64)
    print("wvc spawn profile: time + resource utilization")
    print("=" * 64)
    print(f"Binary : {binary}")
    print(f"Size   : {size/1048576:.1f} MB")
    print(f"Runs   : {args.runs}   sample interval: {args.sample_interval_ms} ms")
    print(f"Host   : {os.cpu_count()} CPUs, isolated WVC_HOME per run")
