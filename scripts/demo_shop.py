...
def state_path() -> Path:
    configured = os.environ.get("WVC_DEMO_SHOP_STATE")
    if configured:
        return Path(configured).expanduser()
    return Path(f"/tmp/wvc-demo-shop-{os.getuid()}.json")
...
def command_search(args: argparse.Namespace) -> int:
    ...
    print("Add one with: wvc-demo-shop add <sku>")
    return 0
...
def command_add(args: argparse.Namespace) -> int:
    ...
    print("Review it with: wvc-demo-shop cart")
    print("Continue with: wvc-demo-shop checkout")
    print(
