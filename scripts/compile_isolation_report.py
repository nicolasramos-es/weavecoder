#!/usr/bin/env python3
"""Report compile-time isolation risks in the Weavecoder crate graph.
...
WATCHED_CRATES = ["wvc-base", "wvc-app-core", "wvc-tui", "wvc"]
...
def target_state_violations(stats_by_name: dict[str, CrateStats]) -> list[str]:
    ...
    tui = stats_by_name.get("wvc-tui")
    if tui and "wvc-app-core" in tui.normal_workspace_deps:
        violations.append("target-state: wvc-tui still directly depends on wvc-app-core")

    app_core = stats_by_name.get("wvc-app-core")
    if app_core and "wvc-base" in app_core.normal_workspace_deps:
        violations.append("target-state: wvc-app-core still directly depends on wvc-base")

    base = stats_by_name.get("wvc-base")
    if base:
        for dep in base.normal_workspace_deps:
            if dep in {
                ...
                violations.append(f"target-state: wvc-base still depends on leaf/runtime crate {dep}")
        for dep in base.normal_external_deps:
            if dep.startswith("aws-") or dep in {"aws-types"}:
                violations.append(f"target-state: wvc-base still depends directly on AWS crate {dep}")

    for crate in stats_by_name.values():
        for glob in crate.glob_reexports:
            if crate.name in {"wvc", "wvc-app-core", "wvc-tui"}:
