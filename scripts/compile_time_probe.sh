...
Runs a full-feature selfdev wvc build with Cargo timings enabled and summarizes
...
Options:
  ...
  --package <name>          Cargo package to build (default: wvc)
  --bin <name>              Cargo binary to build (default: wvc)
  --feature-profile <name>  WVC_DEV_FEATURE_PROFILE for dev_cargo.sh (default: default)
...
Examples:
  ...
  scripts/compile_time_probe.sh --touch crates/wvc-tui/src/tui/app/input.rs
...
Notes:
  ...
  - The "wvc serial stack" summary is not a formal Cargo critical path. It is a
    focused view of the known long-pole crates: wvc-base, wvc-app-core,
    wvc-tui, root wvc lib, and wvc bin.
USAGE
}
...
PY
  ...
  WVC_DEV_FEATURE_PROFILE="$feature_profile" \
...
def is_wvc_stack_unit(unit: dict[str, Any]) -> bool:
    name = unit.get("name")
    target = unit.get("target") or ""
    if name in {"wvc-base", "wvc-app-core", "wvc-tui"} and "build script" not in target:
        return True
    if name == "wvc" and (target == "" or f'bin "{bin_name}"' in target):
        return True
    return False

wvc_stack = sorted([unit for unit in units if is_wvc_stack_unit(unit)], key=lambda unit: float(unit.get("start", 0.0)))
stack_span = None
if wvc_stack:
    stack_span = max(float(unit["end"]) for unit in wvc_stack) - min(float(unit.get("start", 0.0)) for unit in wvc_stack)
stack_sum = sum(float(unit.get("duration", 0.0)) for unit in wvc_stack)
stack_frontend_sum = sum(float(unit.get("frontend_duration") or 0.0) for unit in wvc_stack)
stack_codegen_sum = sum(float(unit.get("codegen_duration") or 0.0) for unit in wvc_stack)
