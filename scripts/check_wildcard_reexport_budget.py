...
Counts production Rust occurrences of `pub use wvc_<crate>::*;` (cross-crate
...
`wvc_*` cross-crate wildcards are budgeted.
...
PATTERN = re.compile(r"^\s*pub\s+use\s+(?:::)?wvc_[a-z0-9_]+(?:::[A-Za-z0-9_]+)*::\*\s*;")
