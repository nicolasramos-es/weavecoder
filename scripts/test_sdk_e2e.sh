...
socket="${TMPDIR:-/tmp}/wvc-sdk-e2e-$$.sock"
log="${TMPDIR:-/tmp}/wvc-sdk-e2e-$$.log"
...
# `wvc api-bridge`, which only the shipped binary provides.
echo "== building wvc =="
cargo build --profile selfdev -p wvc --bin wvc
wvc_bin="$repo_root/target/selfdev/wvc"

echo "== building bridge =="
# The standalone bridge binary, not `wvc api-bridge`, because the full wvc
...
cargo build --profile selfdev -p wvc-harness-api-server --bin wvc-harness-api-bridge
...
# WVC_ACTIVE_PROVIDER therefore makes this suite pass or fail based on the
...
WVC_API_SOCKET="$socket" WVC_ACTIVE_PROVIDER= ./target/selfdev/wvc-harness-api-bridge >"$log" 2>&1 &
bridge_pid=$!
cleanup() {
...
WVC_API_SOCKET="$socket" node "$sdk_dir/test/live-turn.mjs"

echo "== exercising the control surface =="
WVC_API_SOCKET="$socket" node "$sdk_dir/test/live-control.mjs"

echo "== exercising models, effort, compaction, rename, and undo =="
WVC_API_SOCKET="$socket" node "$sdk_dir/test/live-capabilities.mjs"
...
node "$sdk_dir/test/live-launch.mjs" "$wvc_bin"

echo "== checking instance isolation and path safety =="
node "$sdk_dir/test/live-isolation.mjs" "$wvc_bin"

echo "== checking every documented launch option =="
node "$sdk_dir/test/live-options.mjs" "$wvc_bin"
