...
DEFAULT_BINARY_PATH = "/tmp/wvc-compat-dist/wvc-linux-x86_64"
DEFAULT_OPENAI_AUTH_PATH = "~/.wvc/openai-auth.json"
...
OPENAI_AUTH = _resolve_existing_file(
    env_name="WVC_HARBOR_OPENAI_AUTH",
...
def _benchmark_instruction_preamble() -> str:
    # Keep Harbor runs aligned with normal TUI/wvc-run prompting by default.
    # The legacy preamble can still be enabled explicitly for reproducing older
    # runs, but new benchmark runs should rely on wvc's normal system prompt
    # and the official Terminal-Bench task instruction.
    if os.environ.get("WVC_HARBOR_LEGACY_PREAMBLE"):
        return LEGACY_BENCHMARK_INSTRUCTION_PREAMBLE
    return os.environ.get("WVC_HARBOR_EXTRA_PREAMBLE", "")


def _load_task_hint() -> str:
    if not os.environ.get("WVC_HARBOR_ENABLE_HINTS"):
        return ""
    task_name = os.environ.get("WVC_HARBOR_CURRENT_TASK", "").strip()
    hints_path = os.environ.get("WVC_HARBOR_TASK_HINTS_FILE", "").strip()
    extra = os.environ.get("WVC_HARBOR_EXTRA_PREAMBLE", "").strip()
    parts: list[str] = []
    if extra:
        parts.append(extra)
...
class WeavecoderHarborAgent(BaseAgent):
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            ...
            self._provider_arg, self._wvc_model = "openai", self._model_arg
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "WVC_OPENAI_REASONING_EFFORT": os.environ.get("WVC_OPENAI_REASONING_EFFORT", "high"),
            "WVC_OPENAI_SERVICE_TIER": os.environ.get("WVC_OPENAI_SERVICE_TIER", "priority"),
