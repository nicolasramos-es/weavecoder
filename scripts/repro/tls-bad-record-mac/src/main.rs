...
//
// We then wrap that error exactly like wvc's anthropic provider does
...
fn classify(error_str: &str) -> bool {
    // VERBATIM copy of wvc's shared is_transient_transport_error
    // (crates/wvc-base/src/provider/routing.rs @ v0.24.0).
...
async fn main() -> anyhow::Result<()> {
    ...
    // ---- 4. Client (mirrors wvc's reqwest+rustls config) ----
