#!/usr/bin/env python3
"""Find tests that mutate the process-global WVC_HOME without the shared lock.

Rust runs tests in parallel threads of one process, so any test that changes
WVC_HOME changes it for every concurrently running test. `storage::lock_test_env`
...
test mid-assertion, which is the shape of wvc-tui's intermittent failures.
...
CRATE = sys.argv[1] if len(sys.argv) > 1 else "crates/wvc-tui/src"

files = subprocess.run(
    ["grep", "-rl", "WVC_HOME", "--include=*.rs", CRATE],
    capture_output=True, text=True,
).stdout.split()

MUTATES = re.compile(r'(set_var\s*\(\s*"WVC_HOME"|set_path\s*\(\s*"WVC_HOME"|remove_var\s*\(\s*"WVC_HOME")')
...
for path, name in offenders:
    print(f"{path}: {name}")
print(f"\n{len(offenders)} function(s) mutate WVC_HOME without the shared test-env lock")
