#!/usr/bin/env bash
#
# Stress test: spawn 40 wvc TUI client instances rapidly
# Measures startup time, memory usage, CPU, fd count, socket health
#
set -euo pipefail

NUM_INSTANCES=${1:-40}
WVC_BIN="${WVC_BIN:-$(which wvc)}"
LOG_DIR="/tmp/wvc-stress-test-$(date +%s)"
mkdir -p "$LOG_DIR"

MAIN_SOCK="/run/user/$(id -u)/wvc.sock"
DEBUG_SOCK="/run/user/$(id -u)/wvc-debug.sock"

echo "========================================="
echo " wvc Stress Test: $NUM_INSTANCES instances"
echo "========================================="
echo "Binary: $WVC_BIN"
...
snapshot() {
    ...
    # wvc process count and total RSS
    local wvc_procs=$(pgrep -c wvc 2>/dev/null || echo 0)
    local total_rss=0
    local total_vms=0
    for pid in $(pgrep wvc 2>/dev/null); do
    ...
    echo "wvc_processes: $wvc_procs" >> "$LOG_DIR/snapshots.log"
    ...
    echo "[$label] procs=$wvc_procs rss=${total_rss}KB($(( total_rss / 1024 ))MB) server_rss=$(awk '/^VmRSS:/{print $2}' /proc/${server_pid:-0}/status 2>/dev/null || echo '?')KB fds=$(ls /proc/${server_pid:-0}/fd 2>/dev/null | wc -l) threads=$(ls /proc/${server_pid:-0}/task 2>/dev/null | wc -l)"
}
...
if ! [ -S "$MAIN_SOCK" ]; then
    echo "ERROR: No wvc server running at $MAIN_SOCK"
    echo "Start one with: wvc serve &"
    exit 1
fi
...
BASELINE_RSS=$(pgrep wvc 2>/dev/null | while read pid; do awk '/^VmRSS:/{print $2}' /proc/$pid/status 2>/dev/null; done | paste -sd+ | bc 2>/dev/null || echo 0)
BASELINE_PROCS=$(pgrep -c wvc 2>/dev/null || echo 0)
...
(
    while true; do
        ts=$(date +%s)
        wvc_procs=$(pgrep -c wvc 2>/dev/null || echo 0)
        total_rss=0
        for pid in $(pgrep wvc 2>/dev/null); do
        ...
        echo "$ts,$wvc_procs,$total_rss,$server_rss,$server_fds,$server_threads,$cpu_load"
        sleep 1
    done
...
# --- Spawn instances ---
echo "=== Spawning $NUM_INSTANCES wvc instances ==="
PIDS=()
SPAWN_TIMES=()

# Use script to give each instance a pty (wvc requires tty)
for i in $(seq 1 $NUM_INSTANCES); do
    ...
    script -q -c "$WVC_BIN --no-update --no-selfdev" /dev/null \
...
POST_PROCS=$(pgrep -c wvc 2>/dev/null || echo 0)
POST_RSS=0
for pid in $(pgrep wvc 2>/dev/null); do
    rss=$(awk '/^VmRSS:/{print $2}' /proc/$pid/status 2>/dev/null || echo 0)
