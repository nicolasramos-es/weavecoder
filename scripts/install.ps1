...
function Assert-WeavecoderBinaryCandidate {
    ...
    # Tolerate build/pre-release suffixes (-dev, -rc1...) in the requested tag:
    # the binary reports the bare semver while the release tag may carry a
    # suffix. Compare only the numeric core.
    $expectedCore = $expectedNumber.Split('-')[0]
    $reportedCore = $reportedVersion.TrimStart('v').Split('-')[0]
    if ($reportedCore -ne $expectedCore) {
