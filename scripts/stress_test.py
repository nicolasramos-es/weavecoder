#!/usr/bin/env python3
"""
wvc Stress Test: Spawn 40 sessions via debug socket, measure performance.
...
MAIN_SOCK = f"/run/user/{os.getuid()}/wvc.sock"
DEBUG_SOCK = f"/run/user/{os.getuid()}/wvc-debug.sock"
...
def get_server_pid():
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            if "wvc.sock" in line and "LISTEN" in line:
                parts = line.split()
                return int(parts[1])
    except:
        pass
    
    # Fallback: find the oldest wvc process (likely the server)
    try:
        result = subprocess.run(
            ["pgrep", "-o", "wvc"], capture_output=True, text=True, timeout=5
        )
...
print_header(f"wvc Stress Test: {NUM_INSTANCES} sessions")
...
if not os.path.exists(MAIN_SOCK):
    ...
    print("  Start a server with: wvc serve &")
    sys.exit(1)

if not os.path.exists(DEBUG_SOCK):
    print_err(f"No debug socket at {DEBUG_SOCK}")
    print("  Enable with: touch ~/.wvc/debug_control")
    sys.exit(1)
...
baseline_procs = int(subprocess.run(["pgrep", "-c", "wvc"], capture_output=True, text=True).stdout.strip() or "0")
...
print_stat("Total wvc procs", baseline_procs)
...
for i in range(1, NUM_INSTANCES + 1):
    t0 = time.monotonic()
    resp = debug_cmd(f"create_session:/tmp/wvc-stress-{i}", timeout=30)
    t1 = time.monotonic()
...
peak_procs = int(subprocess.run(["pgrep", "-c", "wvc"], capture_output=True, text=True).stdout.strip() or "0")
...
print_stat("Total wvc procs", peak_procs)
...
final_procs = int(subprocess.run(["pgrep", "-c", "wvc"], capture_output=True, text=True).stdout.strip() or "0")
