...
def build_specs() -> dict[str, ToolSpec]:
    wvc = shutil.which("wvc") or str(Path.home() / ".local/bin/wvc")
    ...
    specs = {
        "wvc_memory_off": ToolSpec(
            name="wvc_memory_off",
            argv=[wvc, "--no-update", "--no-selfdev"],
            version_argv=[wvc, "version"],
            env={"WVC_NO_TELEMETRY": "1", "WVC_MEMORY_ENABLED": "0"},
            wvc=True,
        ),
        "wvc_memory_on": ToolSpec(
            name="wvc_memory_on",
            argv=[wvc, "--no-update", "--no-selfdev"],
            version_argv=[wvc, "version"],
            env={"WVC_NO_TELEMETRY": "1", "WVC_MEMORY_ENABLED": "1"},
            wvc=True,
        ),
        "pi": ToolSpec(
            name="pi",
...
def run_tool(spec: ToolSpec, sessions: int, cwd: Path, timeout_s: float, settle_s: float) -> ToolRunResult:
    ...
    try:
        if spec.wvc:
            temp_root = tempfile.mkdtemp(prefix="wvc-memory-bench-")
            env = os.environ.copy()
            if spec.env:
                env.update(spec.env)
            env["WVC_HOME"] = os.path.join(temp_root, "home")
            env["WVC_RUNTIME_DIR"] = os.path.join(temp_root, "run")
            ...
            for auth_name in (
                ...
                real_auth = Path.home() / ".wvc" / auth_name
                bench_auth = Path(env["WVC_HOME"]) / auth_name
                if real_auth.exists() and not bench_auth.exists():
                    bench_auth.symlink_to(real_auth)
            if spec.name == "wvc_memory_on":
                real_models = Path.home() / ".wvc" / "models"
                bench_models = Path(env["WVC_HOME"]) / "models"
                if real_models.exists() and not bench_models.exists():
                    bench_models.symlink_to(real_models)
            socket_path = os.path.join(env["WVC_RUNTIME_DIR"], "bench.sock")
            ...
            if not wait_for_socket(socket_path, timeout_s):
                raise RuntimeError("wvc server did not become ready")
            if spec.name == "wvc_memory_on":
                time.sleep(max(settle_s, 5.0))
            per_session_settle = max(settle_s, 2.0) if spec.name == "wvc_memory_on" else settle_s
