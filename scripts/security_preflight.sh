...
if [[ "${#tracked_files[@]}" -gt 0 ]]; then
  if command -v rg >/dev/null 2>&1; then
    rg -n --color=never -e "$secret_regex" \
      ...
      "${tracked_files[@]}" > /tmp/wvc-secret-scan.txt
    scan_status=$?
  else
    scan_files=()
    ...
    if [[ "${#scan_files[@]}" -gt 0 ]]; then
      grep -I -n -E "$secret_regex" "${scan_files[@]}" > /tmp/wvc-secret-scan.txt
      scan_status=$?
    fi
  fi
fi
...
if [[ "$scan_status" -gt 1 ]]; then
  rm -f /tmp/wvc-secret-scan.txt
  die "secret scan failed to execute"
fi

if [[ -s /tmp/wvc-secret-scan.txt ]]; then
  cat /tmp/wvc-secret-scan.txt
  rm -f /tmp/wvc-secret-scan.txt
...
audit_ignores=(
  ...
  --ignore RUSTSEC-2026-0141 # lettre via notify-email, Boring TLS backend not used by wvc
