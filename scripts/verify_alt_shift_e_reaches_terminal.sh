...
# proves wvc's handler reacts to the real Alt+Shift+E bytes; this script
...
#         => niri forwarded the key; wvc would see it.
...
if echo "$HEX" | grep -qiE "1b5b313031|1b65|1b45"; then
  ...
  echo "   (terminal received an 'e' key event; wvc will see it.)"
  exit 0
else
