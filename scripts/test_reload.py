...
def wvc_debug_socket():
    """Find the active wvc debug socket path."""
    runtime_dir = os.environ.get("XDG_RUNTIME_DIR") or f"/run/user/{os.getuid()}"
    return os.path.join(runtime_dir, "wvc-debug.sock")
...
def dbg(command, session_id=None, timeout=TIMEOUT_SECS):
    ...
    return _send_recv(wvc_debug_socket(), req, timeout=timeout)
...
def test_reload_context_roundtrip():
    wvc_dir = pathlib.Path.home() / ".wvc"
    ctx_path = wvc_dir / "reload-context.json"
...
def test_canary_binary_path():
    ...
    manifest_path = home / ".wvc" / "build-manifest.json"
    ...
    if canary_hash:
        canary_binary = home / ".wvc" / "builds" / "canary" / "wvc"
...
def test_multiple_sessions_responsive():
    ...
    try:
        for i in range(N):
            s = create_session(f"/tmp/wvc-test-{i}")
            sessions.append(s)
...
def check_server_up():
    dbg_sock = wvc_debug_socket()
    if not os.path.exists(dbg_sock):
        raise RuntimeError(
            f"Debug socket not found at {dbg_sock}.\n"
            "  Start wvc server first:\n"
            "    wvc serve   (or just launch wvc in the repo)"
        )
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test wvc selfdev reload mechanism",
    ...
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║      wvc selfdev reload test suite                    ║")
    print("╚══════════════════════════════════════════════════════════╝\n")
