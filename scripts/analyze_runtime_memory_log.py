...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze wvc runtime memory JSONL logs for growth, spikes, attribution, and optimization hints"
    )
    parser.add_argument("paths", nargs="*", help="Specific JSONL files or directories to analyze")
    parser.add_argument(
        "--log-dir",
        help="Directory containing runtime memory JSONL logs (default: ~/.wvc/logs/memory or $WVC_HOME/logs/memory)",
    )
...
def default_log_dir() -> Path:
    wvc_home = os.environ.get("WVC_HOME")
    if wvc_home:
        return Path(wvc_home).expanduser() / "logs" / "memory"
    return Path.home() / ".wvc" / "logs" / "memory"
    ...
    if runaway_population:
        ...
        actions = [
            {
                ...
                "commands": ["wvc debug 'server:memory-incident'", "wvc debug 'swarm:list'"],
            },
            {
                "priority": 2,
        ...
        actions = [
            {
                ...
                "commands": ["wvc debug 'server:memory'"],
            }
        ]
        ...
        actions = [
            ...
            },
            {
                ...
                "commands": [
                    "wvc debug 'allocator:profile:on'",
                    "wvc debug 'allocator:profile:dump /tmp/wvc-server.heap'",
                ],
            },
        ]
