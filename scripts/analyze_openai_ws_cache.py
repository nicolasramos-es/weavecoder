#!/usr/bin/env python3
"""Analyze OpenAI persistent-websocket reuse and KV-cache effectiveness from wvc logs.
...
Use it before/after changing ``WVC_OPENAI_WS_IDLE_RECONNECT_SECS`` (or the
...
With no arguments it scans ~/.wvc/logs/wvc-*.log.
"""
...
def _log_files(argv: list[str]) -> list[str]:
    ...
    return sorted(glob.glob(os.path.join(home, ".wvc", "logs", "wvc-*.log")))
