...
# Run from the wvc repo root

set -euo pipefail

ITERATIONS=${1:-5}
RESULTS_FILE="/tmp/wvc_tool_benchmark_$(date +%Y%m%d_%H%M%S).csv"

echo "=== wvc Tool Call Benchmark ==="
...
# Find debug socket
DEBUG_SOCK="${WVC_DEBUG_SOCK:-/run/user/$(id -u)/wvc-debug.sock}"

if [ ! -S "$DEBUG_SOCK" ]; then
    echo "ERROR: Debug socket not found at $DEBUG_SOCK"
    echo "Make sure wvc is running with debug control enabled."
    exit 1
fi
