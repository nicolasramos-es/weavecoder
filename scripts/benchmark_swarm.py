...
Compares wvc's swarm (multi-agent coordination) with single-agent performance
...
Environment:
    Requires wvc server running with debug_control enabled:
        touch ~/.wvc/debug_control
        wvc serve
"""
...
def main():
    ...
    if not os.path.exists(DEBUG_SOCKET):
        ...
        print("Make sure wvc server is running with debug_control enabled:")
        print("  touch ~/.wvc/debug_control")
        print("  wvc serve")
        sys.exit(1)
