...
Targets:
  ...
  release-wvc    Run scripts/dev_cargo.sh build --release -p wvc --bin wvc --quiet
  selfdev-wvc    Run scripts/dev_cargo.sh build --profile selfdev -p wvc --bin wvc --quiet
...
Examples:
  ...
  scripts/bench_compile.sh build -- --package wvc --bin test_api
  scripts/bench_compile.sh release-wvc --json
  scripts/bench_compile.sh selfdev-wvc --json
USAGE
}
...
case "$target" in
  ...
  build)
    ...
    ;;
  release-wvc)
    cmd=(scripts/dev_cargo.sh build --release -p wvc --bin wvc --quiet)
    ;;
  selfdev-wvc)
    cmd=(scripts/dev_cargo.sh build --profile selfdev -p wvc --bin wvc --quiet)
    ;;
  *)
