...
IN_CONTAINER_HOME = "/tmp/wvc-home"
IN_CONTAINER_RUNTIME = "/tmp/wvc-runtime"
IN_CONTAINER_INPUT = "/tmp/wvc-input"
IN_CONTAINER_OUTPUT = "/tmp/wvc-output"
IN_CONTAINER_BINARY = "/usr/local/bin/wvc"
IN_CONTAINER_LIB_DIR = f"{IN_CONTAINER_RUNTIME}/lib"
IN_CONTAINER_CA_BUNDLE = f"{IN_CONTAINER_HOME}/ca-certificates.crt"
DEFAULT_BINARY_PATH = "/tmp/wvc-compat-dist/wvc-linux-x86_64.bin"
DEFAULT_CLAUDE_AUTH_PATH = "~/.wvc/auth.json"
DEFAULT_OPENROUTER_ENV_PATH = "~/.config/wvc/openrouter.env"
DEFAULT_ANTHROPIC_ENV_PATH = "~/.config/wvc/anthropic.env"
CA_BUNDLE_CANDIDATES = (
    os.environ.get("WVC_HARBOR_CA_BUNDLE"),
...
def _load_anthropic_key() -> str | None:
    return _load_key_from_env_file(
        os.environ.get("WVC_HARBOR_ANTHROPIC_ENV", DEFAULT_ANTHROPIC_ENV_PATH),
        "ANTHROPIC_API_KEY",
        "ANTHROPIC_API_KEY",
        "WVC_HARBOR_ANTHROPIC_KEY",
    )


def _load_openrouter_key() -> str | None:
    # Priority: explicit env, then the wvc openrouter.env file (raw key per line).
    direct = os.environ.get("OPENROUTER_API_KEY") or os.environ.get("WVC_HARBOR_OPENROUTER_KEY")
    if direct and direct.strip():
        return direct.strip()
    path = Path(os.environ.get("WVC_HARBOR_OPENROUTER_ENV", DEFAULT_OPENROUTER_ENV_PATH)).expanduser()
    if path.exists() and path.is_file():
        for line in path.read_text().splitlines():
            line = line.strip()
            if not line or line.startswith("#"):
...
WVC_BINARY = _resolve_existing_file(
    env_name="WVC_HARBOR_BINARY",
    default_path=DEFAULT_BINARY_PATH,
)
CA_BUNDLE = _resolve_existing_file(
    env_name="WVC_HARBOR_CA_BUNDLE",
...
OPENSSL_RUNTIME_LIBS = tuple(
    ...
    for lib in (
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("WVC_HARBOR_LIBSSL"),
                *_sibling_runtime_lib_candidates(WVC_BINARY, "libssl"),
                "/usr/lib/libssl.so.3",
        ...
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("WVC_HARBOR_LIBCRYPTO"),
                *_sibling_runtime_lib_candidates(WVC_BINARY, "libcrypto"),
...
def _benchmark_instruction_preamble() -> str:
    return os.environ.get("WVC_HARBOR_EXTRA_PREAMBLE", "")
...
class WeavecoderClaudeHarborAgent(BaseAgent):
    """Harbor adapter that runs wvc with Opus 4.8.
    ...
    OAuth (provider=claude via ~/.wvc/auth.json) are also supported.
    """
    ...
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            self._provider_arg, self._wvc_model = self._model_arg.split("/", 1)
        else:
            self._provider_arg, self._wvc_model = "anthropic-api", self._model_arg
        ...
        if self._provider_arg == "claude":
            self._claude_auth = _resolve_existing_file(
                env_name="WVC_HARBOR_CLAUDE_AUTH",
                default_path=DEFAULT_CLAUDE_AUTH_PATH,
            )

    @staticmethod
    def name() -> str:
        return "wvc-harbor-claude"
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        # "error setting certificate file") so tests never run. wvc itself
        ...
        await environment.upload_file(WVC_BINARY, IN_CONTAINER_BINARY)
        ...
        version_result = await environment.exec(
            ...
            env={
                ...
                "WVC_HOME": IN_CONTAINER_HOME,
                "WVC_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
                "WVC_NO_TELEMETRY": "1",
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "WVC_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
            "WVC_NO_TELEMETRY": "1",
            "WVC_PROVIDER": self._provider_arg,
            "WVC_MODEL": self._wvc_model,
            "WVC_ANTHROPIC_REASONING_EFFORT": os.environ.get("WVC_ANTHROPIC_REASONING_EFFORT", "high"),
        ...
        result = await environment.exec(
            command=(
                ...
                '--provider "$WVC_PROVIDER" --model "$WVC_MODEL" '
        ...
        try:
            await environment.download_dir(IN_CONTAINER_OUTPUT, self.logs_dir / "wvc-output")
        ...
        metadata: dict[str, Any] = {
            ...
            "model": self._wvc_model,
            "wvc_binary": str(WVC_BINARY),
        }

        output_dir = self.logs_dir / "wvc-output"
