#!/bin/bash
# Capture wvc screenshots with your actual terminal theme
# Usage: ./capture_screenshot.sh [output_name]

set -e

OUTPUT_DIR="$(dirname "$0")/../docs/screenshots"
OUTPUT_NAME="${1:-wvc-screenshot}"
OUTPUT_PATH="$OUTPUT_DIR/${OUTPUT_NAME}.png"

mkdir -p "$OUTPUT_DIR"

echo "📸 wvc Screenshot Capture"
echo ""
echo "Instructions:"
echo "  1. Make sure wvc is running in a visible terminal"
echo "  2. Set up the UI state you want to capture"
echo "  3. Press Enter here, then click on the wvc window"
echo ""
