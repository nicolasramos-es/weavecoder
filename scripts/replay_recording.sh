#!/bin/bash
# Replay a wvc recording as video
#
# This script:
# 1. Starts a fresh wvc instance in a new terminal
...
# Start wvc in a new kitty window
echo "🚀 Starting wvc..."
kitty --title "wvc-replay" -e bash -c "cd $(pwd) && ~/.cargo/bin/wvc; read -p 'Press Enter to close...'" &
KITTY_PID=$!
sleep 2  # Wait for wvc to start

# Focus the new window
sleep 0.5
# Find and focus the wvc-replay window
WINDOW_ID=$(niri msg windows 2>/dev/null | grep -B5 "wvc-replay" | grep -oP 'Window ID \K\d+' | head -1)
if [ -n "$WINDOW_ID" ]; then
