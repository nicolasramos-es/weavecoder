#!/usr/bin/env python3
"""
Comprehensive memory system test for wvc.

Tests all memory features with both Claude and OpenAI providers via the debug socket.

Usage:
    # With existing server (uses /run/user/1000/wvc-debug.sock)
...
def main():
    parser = argparse.ArgumentParser(description='Test wvc memory system')
    ...
    if args.fresh:
        ...
        test_socket = '/tmp/wvc-memory-test.sock'
        ...
        socket_path = args.socket or '/run/user/1000/wvc-debug.sock'
        if not os.path.exists(socket_path):
            log_fail(f"Debug socket not found: {socket_path}")
            log("Use --fresh to start a test server, or ensure wvc is running")
            sys.exit(1)
