...
fixture_root=${WVC_AUTH_FIXTURE_DIR:-$fixture_root_default}
...
Commands:
  ...
  save <name>                  Save the current sandbox WVC_HOME as a fixture
  load <name>                  Replace the sandbox WVC_HOME with a saved fixture
  reset-sandbox                Remove only the current sandbox WVC_HOME
  delete <name>                Delete a saved fixture
  env <name>                   Print exports for running against a loaded fixture
  run <name> -- <args...>      Load fixture, then run wvc with args in sandbox
  help                         Show this help

Environment overrides:
  WVC_ONBOARDING_SANDBOX     Sandbox name to load into/from (default: default)
  WVC_ONBOARDING_DIR         Explicit onboarding sandbox directory
...
run_wvc() {
  ...
  (
    ...
    if [[ -x "$binary_path" ]]; then
      env WVC_HOME="$wvc_home" WVC_RUNTIME_DIR="$runtime_dir" "$binary_path" "$@"
    else
      env WVC_HOME="$wvc_home" WVC_RUNTIME_DIR="$runtime_dir" cargo run --bin wvc -- "$@"
    fi
  )
}
...
save_fixture() {
  ...
  ensure_parent_dirs
  if [[ ! -d "$wvc_home" ]]; then
    echo "sandbox WVC_HOME does not exist: $wvc_home" >&2
    exit 1
  fi
  mkdir -p "$(dirname "$dst")"
  copy_dir_contents "$wvc_home" "$dst"
  chmod -R go-rwx "$(dirname "$dst")" 2>/dev/null || true
  cat > "$meta" <<EOF
name=$name
saved_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)
sandbox_name=$sandbox_name
source_wvc_home=$wvc_home
warning=May contain real local auth tokens. Do not commit or share.
EOF
  echo "Saved auth fixture '$name' from $wvc_home"
...
load_fixture() {
  ...
  copy_dir_contents "$src" "$wvc_home"
  chmod -R go-rwx "$wvc_home" 2>/dev/null || true
...
case "$command" in
  ...
  reset-sandbox)
    ...
    echo "Reset sandbox WVC_HOME: $wvc_home"
    ;;
  delete)
    name=$(require_name "${1:-}")
    delete_fixture "$name"
