#!/bin/bash
# wvc demo recording orchestrator
# Usage: ./scripts/record_demo.sh <demo_name> <prompt>
#
# This script:
# 1. Opens a fresh wvc in a new kitty window
# 2. Starts wf-recorder on that window
# 3. Sends the prompt to wvc
...
DEMO_DIR="/tmp/wvc-demo/$DEMO_NAME"
OUTPUT_DIR="$repo_root/assets/demos"
SOCK=$(ls /tmp/kitty.sock* 2>/dev/null | head -1)

mkdir -p "$DEMO_DIR" "$OUTPUT_DIR"

echo "=== wvc Demo Recorder ==="
echo "Demo: $DEMO_NAME"
echo "Prompt: $PROMPT"
echo "Working dir: $DEMO_DIR"
echo ""

# Step 1: Launch wvc in a new kitty OS window
echo "[1/5] Launching wvc..."
kitten @ --to unix:$SOCK launch --type=os-window \
    --cwd "$DEMO_DIR" \
    --title "wvc-demo-$DEMO_NAME" \
    "$repo_root/target/release/wvc"

sleep 3  # Let wvc fully start

# Step 2: Find the window
DEMO_WIN_ID=$(niri msg windows 2>/dev/null | grep -B5 "wvc-demo-$DEMO_NAME" | grep "Window ID" | awk '{print $3}' | tr -d ':')
...
# Step 4: Type the prompt into wvc
...
for os_win in data:
    for tab in os_win.get('tabs', []):
        for win in tab.get('windows', []):
            if 'wvc-demo-$DEMO_NAME' in win.get('title', ''):
                print(win['id'])
                sys.exit(0)
")
...
if [ -n "$KITTY_WIN_ID" ]; then
    ...
    kitten @ --to unix:$SOCK send-text --match "title:wvc-demo-$DEMO_NAME" "$PROMPT"
    sleep 0.5
    kitten @ --to unix:$SOCK send-text --match "title:wvc-demo-$DEMO_NAME" $'\r'
fi
...
# Poll for completion - check if wvc is still processing
