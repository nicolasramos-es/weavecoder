...
def main() -> int:
    ...
    ap.add_argument("--binary", default=str(REPO_ROOT / "target" / "selfdev" / "wvc"))
    ...
    runtime = Path(os.environ.get("WVC_RUNTIME_DIR") or f"/run/user/{os.getuid()}")
    env = os.environ.copy()
    ...
    root = Path(tempfile.mkdtemp(prefix="wvc-profile-real-", dir=str(scratch)))
