...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Profile a single resumed wvc spawn")
    parser.add_argument("--binary", default="./target/selfdev/wvc")
...
def latest_log_file(log_dir: Path) -> Path:
    logs = sorted(log_dir.glob("wvc-*.log"), key=lambda p: p.stat().st_mtime)
    if not logs:
...
def profile_single_spawn(binary: str, cwd: str, timeout_s: float) -> dict:
    root = Path(tempfile.mkdtemp(prefix="wvc-single-profile-"))
    home = root / "home"
    runtime_dir = root / "run"
    socket_path = runtime_dir / "wvc.sock"
    debug_socket_path = runtime_dir / "wvc-debug.sock"
    ...
    env.update(
        {
            "WVC_HOME": str(home),
            "WVC_RUNTIME_DIR": str(runtime_dir),
            "WVC_SOCKET": str(socket_path),
            "WVC_DEBUG_SOCKET": str(debug_socket_path),
            "WVC_SWARM_ENABLED": "0",
            "WVC_NO_TELEMETRY": "1",
            "WVC_TRACE": "1",
            "WVC_TEMP_SERVER": "1",
            "WVC_SERVER_OWNER_PID": str(os.getpid()),
        }
    )
    ...
    try:
        ...
        while time.perf_counter() < deadline:
            ...
            if rlist:
                ...
                lower = buffer.lower()
                if b"loading session" in lower or b"wvc" in lower or len(buffer) > 4096:
