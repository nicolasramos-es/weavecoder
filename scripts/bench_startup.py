#!/usr/bin/env python3
"""Benchmark and optionally regression-check wvc startup time.

This script runs isolated startup measurements under a temporary WVC_HOME and
WVC_RUNTIME_DIR so it does not interfere with the user's real server, logs, or
credentials.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("binary", nargs="?", default="./target/release/wvc")
...
def measure_server_startup(binary: str, runs: int) -> list[float]:
    times: list[float] = []
    for _ in range(runs):
        root = tempfile.mkdtemp(prefix="wvc-server-bench-")
        env = isolated_env(root)
        socket_path = env["WVC_SOCKET"]
        proc = None
        try:
            start = time.perf_counter()
