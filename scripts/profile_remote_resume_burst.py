...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Profile resumed wvc PTY burst startup")
    p.add_argument("--binary", default="./target/release/wvc")
    p.add_argument("--burst", type=int, default=20)
    p.add_argument("--timeout", type=float, default=15.0)
    p.add_argument("--stagger-ms", type=float, default=0.0)
    p.add_argument("--json-out", default="/tmp/wvc_remote_burst_profile.json")
    return p.parse_args()
...
def run_burst(
    ...
    while time.perf_counter() < deadline and (
        ...
        for fd in rlist:
            ...
            if b"loading session" in lower or b"wvc" in lower or len(client.buffer) > 2048:
                client.done = True
...
def main() -> None:
    args = parse_args()
    root = Path(tempfile.mkdtemp(prefix="wvc-remote-burst-"))
    home = root / "home"
    run = root / "run"
    ...
    debug_socket = run / "wvc-debug.sock"

    server = subprocess.Popen(
        [args.binary, "serve", "--socket", env["WVC_SOCKET"], "--debug-socket"],
        env=env,
