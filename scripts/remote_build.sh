...
#
# Defaults:
# - Config file: ~/.config/wvc/remote-build.env (override with WVC_REMOTE_CONFIG)
# - Host: WVC_REMOTE_HOST from env/config, or --host
# - Remote dir: .cache/remote-builds/wvc/<repo-name> (override with WVC_REMOTE_DIR or --remote-dir)
#
# Examples:
#   scripts/remote_build.sh --release
#   scripts/remote_build.sh test
#   scripts/remote_build.sh check --all-targets
#   scripts/remote_build.sh --host mybox --remote-dir ~/src/wvc test -- --nocapture
...
Options:
  ...
  --host HOST          Remote SSH host (default: $WVC_REMOTE_HOST from env/config; required if unset)
  --remote-dir DIR     Remote project directory (default: $WVC_REMOTE_DIR or .cache/remote-builds/wvc/<repo-name>)
...
Behavior:
  ...
  - For build sync-back, copies target/{debug|release}/<artifact> from remote to local
    (artifact defaults to 'wvc', or '--bin <name>' when provided)
  - Default config file is ~/.config/wvc/remote-build.env
EOF
}
...
REMOTE="${WVC_REMOTE_HOST:-}"
REMOTE_DIR="${WVC_REMOTE_DIR:-.cache/remote-builds/wvc/${REPO_NAME}}"
SSH_BIN="${WVC_REMOTE_SSH_BIN:-ssh}"
RSYNC_BIN="${WVC_REMOTE_RSYNC_BIN:-rsync}"
...
if [[ -z "$REMOTE" ]]; then
    echo "error: remote host not configured; set WVC_REMOTE_HOST or pass --host HOST" >&2
    exit 2
fi
...
SSH_SERVER_ALIVE_INTERVAL="${WVC_REMOTE_SERVER_ALIVE_INTERVAL:-10}"
SSH_SERVER_ALIVE_COUNT_MAX="${WVC_REMOTE_SERVER_ALIVE_COUNT_MAX:-1}"
SSH_OPTS=(
    -o BatchMode=yes
...
RSYNC_SSH_COMMAND="${WVC_REMOTE_RSYNC_SSH:-$SSH_BIN -o BatchMode=yes -o ConnectTimeout=$SSH_CONNECT_TIMEOUT -o ServerAliveInterval=$SSH_SERVER_ALIVE_INTERVAL -o ServerAliveCountMax=$SSH_SERVER_ALIVE_COUNT_MAX}"
...
if ! preflight_output="$(remote_ssh "printf 'wvc-remote-ok\\n'" 2>&1)"; then
    echo "error: remote host '$REMOTE' is not reachable within ${SSH_CONNECT_TIMEOUT}s" >&2
    echo "$preflight_output" >&2
    echo "hint: set WVC_REMOTE_CARGO=0 to force local cargo, or fix WVC_REMOTE_HOST/WVC_REMOTE_CONNECT_TIMEOUT." >&2
    exit 75
fi
...
if [[ "$SYNC_SOURCE" -eq 1 ]]; then
    ...
    "$RSYNC_BIN" -avz -e "$RSYNC_SSH_COMMAND" "$metadata_file" "$REMOTE:$REMOTE_DIR/.wvc-build-meta"
else
    echo ""
...
printf -v REMOTE_INNER_CMD 'cd %q && env WVC_BUILD_METADATA_FILE=.wvc-build-meta %s' "$REMOTE_DIR" "$REMOTE_CARGO_CMD"
