#!/usr/bin/env python3
"""Token-protected HTTP service that generates wvc pairing codes.

GET /pair-code?t=<token> -> {"code": "123456", "host": "100.109.78.41", "port": 7643, "uri": "wvc://pair?..."}
"""
import http.server, json, re, subprocess, os
from urllib.parse import urlparse, parse_qs

TOKEN = open('/etc/wvc-pair-token').read().strip()
HOST = '100.109.78.41'
PORT = 7643
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        env['WVC_GATEWAY_HOST'] = HOST
        try:
            out = subprocess.run(
                ['sudo', '-u', 'ec2-user', '-i', 'wvc', 'pair'],
