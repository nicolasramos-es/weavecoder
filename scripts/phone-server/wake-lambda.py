...
HOST = os.environ.get("WVC_GATEWAY_HOST", "100.109.78.41")
PORT = int(os.environ.get("WVC_GATEWAY_PORT", "7643"))
...
def fetch_pair_code(ssm):
    ...
    command = (
        ...
        f"WVC_GATEWAY_HOST={host} "
        "/home/ec2-user/.local/bin/wvc pair"
    )
    ...
    return {
        ...
        "host": HOST,
        "port": PORT,
        "uri": f"wvc://pair?host={HOST}&port={PORT}&code={code}",
        "expires_in": 300,
    }
...
<h1>wvc server</h1><p id="s"><span class="dot"></span>Authenticating…</p>
<p class="spin" id="hint">checking every 5s…</p><button id="pairbtn">Pair this phone</button><div id="pairout"></div>
<script nonce="__NONCE__">
const fragment = new URLSearchParams(location.hash.slice(1));
if (fragment.get('t')) sessionStorage.setItem('wvc-wake-token', fragment.get('t'));
history.replaceState(null, '', location.pathname);
const token = sessionStorage.getItem('wvc-wake-token');
...
async function poll(){
  try{
    ...
    if(j.healthy){c.classList.add('ok');s.innerHTML='<span class="dot"></span><b>Ready.</b> Open the wvc app now.';document.getElementById('hint').textContent='server is up';document.getElementById('pairbtn').style.display='inline-block';return;}
...
async function pair(){
  ...
  try{const j=await api('pair');if(j.code){o.innerHTML='<div class="code">'+j.code.slice(0,3)+' '+j.code.slice(3)+'</div><div>host '+j.host+':'+j.port+' · expires in 5 min</div><div style="margin-top:10px"><a href="'+j.uri+'">Open in wvc app</a></div>';}else{o.textContent='error: '+(j.error||'unknown');}}catch(e){o.textContent='error: '+e.message;}
}
