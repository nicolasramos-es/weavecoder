...
1. **`WeavecoderPhoneOperator`** for normal deployments and maintenance of the existing stack.
2. **`WeavecoderPhoneProvisioner`** for infrequent rebuilds, MFA-gated, time-limited, restricted to `us-east-1`, `wvc-phone-*` resources, and bounded runtime roles.
...
- `breaker-lambda.py` calls `ec2:DescribeInstances`, `ec2:StopInstances`, and `sns:Publish` to `arn:aws:sns:us-east-1:302154194530:wvc-guard-warn`.
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:iam::302154194530:role/wvc-phone/WeavecoderPhoneOperator",
        "arn:aws:iam::302154194530:role/wvc-phone/WeavecoderPhoneProvisioner"
      ]
    }
  ]
}
...
Set `WeavecoderPhoneProvisioner` maximum session duration to one hour. Do not allow either role to change its own trust or permissions.
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/wvc-phone-wake:*"
    }
  ]
}
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "sns:Publish",
      "Resource": "arn:aws:sns:us-east-1:302154194530:wvc-guard-warn"
    },
    {
      "Sid": "WriteOwnLogs",
      "Effect": "Allow",
      ...
      "Resource": "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/wvc-guard-breaker:*"
    }
  ]
}
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:lambda:us-east-1:302154194530:function:wvc-phone-wake",
        "arn:aws:lambda:us-east-1:302154194530:function:wvc-phone-wake:*",
        "arn:aws:lambda:us-east-1:302154194530:function:wvc-guard-breaker",
        "arn:aws:lambda:us-east-1:302154194530:function:wvc-guard-breaker:*"
      ]
    },
    {
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "iam:PassRole",
      "Resource": "arn:aws:iam::302154194530:role/wvc-phone/runtime/WeavecoderPhoneInstance",
      "Condition": {
        "StringEquals": {
    ...
    },
    {
      ...
      "Action": "iam:PassRole",
      "Resource": [
        "arn:aws:iam::302154194530:role/wvc-phone/runtime/WeavecoderPhoneWakeLambda",
        "arn:aws:iam::302154194530:role/wvc-phone/runtime/WeavecoderPhoneBreakerLambda"
      ],
      "Condition": {
        "StringEquals": {
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/wvc-phone-wake:*",
        "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/wvc-guard-breaker:*"
      ]
    },
    {
      ...
      "Resource": [
        "arn:aws:sns:us-east-1:302154194530:wvc-guard-stop",
        "arn:aws:sns:us-east-1:302154194530:wvc-guard-warn"
      ]
    },
    {
      "Sid": "ReadNamedBudget",
      ...
      "Resource": "arn:aws:budgets::302154194530:budget/wvc-dev-monthly-cost"
    }
  ]
}
```
...
- Human `WeavecoderPhoneProvisioner`: CloudFormation stack operations on `wvc-phone-server*`, read-only diagnostics, and `iam:PassRole` only for `WeavecoderPhoneCloudFormationExecution` with `iam:PassedToService = cloudformation.amazonaws.com`.
- `WeavecoderPhoneCloudFormationExecution`: service permissions below, usable only by CloudFormation.
- Every created resource is tagged `Project=wvc-phone-server` and `ManagedBy=cloudformation`.
- Every created runtime role is under path `/wvc-phone/runtime/` and must carry the `WeavecoderPhoneRuntimeBoundary` permissions boundary.
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:cloudformation:us-east-1:302154194530:stack/wvc-phone-server*/*",
        "arn:aws:cloudformation:us-east-1:302154194530:changeSet/wvc-phone-server*/*"
      ]
    },
    {
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "iam:PassRole",
      "Resource": "arn:aws:iam::302154194530:role/wvc-phone/WeavecoderPhoneCloudFormationExecution",
      "Condition": {
        "StringEquals": {
...
|---|---|---|
| EC2 | Run/terminate the one server; create/tag volume and ENI; create/manage one SG; allocate/associate/release one EIP; modify shutdown behavior; describe AMIs/subnets/VPCs | `us-east-1`; request/resource tag `Project=wvc-phone-server`; approved instance types only; IMDSv2 required; approved VPC/subnet |
| IAM | Create/update/delete the three runtime roles and one instance profile; put/delete inline policies; pass roles | Path `/wvc-phone/runtime/` only; require boundary ARN on `CreateRole`; never allow changing deploy/provisioner/emergency roles |
| Lambda | Create/update/delete the two named functions; versions/aliases; permissions | Function ARN prefix `wvc-phone-*` and `wvc-guard-breaker*`; project tag |
| API Gateway v2 | Create/update/delete one HTTP API, integration, route, and stage | Project tag and stack ownership |
| SNS | Create/manage/delete `wvc-guard-stop` and `wvc-guard-warn`; subscriptions | Exact topic-name ARNs |
...
| Budgets | Create/update/delete `wvc-dev-monthly-cost` | Exact budget ARN |
...
aws lambda get-function --function-name wvc-phone-wake --region us-east-1
aws lambda get-function --function-name wvc-guard-breaker --region us-east-1
aws apigatewayv2 get-api --api-id 8c3wp4cbag --region us-east-1
aws sns list-topics --region us-east-1
aws cloudwatch describe-alarms --alarm-name-prefix wvc- --region us-east-1
aws budgets describe-budget --account-id 302154194530 --budget-name wvc-dev-monthly-cost
...
8. **Canary mutation paths without touching production.** Through the provisioner, deploy and remove a tiny tagged canary stack using the same resource classes where practical. For the operator, update a disposable Lambda alias or canary function rather than invoking `wvc-guard-breaker`, which stops production. Do not use the breaker invocation as an IAM test.
...
- Daily deployment and maintenance succeed through `WeavecoderPhoneOperator`.
