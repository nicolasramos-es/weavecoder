...
DEFAULT_SOCKET = f"/run/user/{os.getuid()}/wvc.sock"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Summarize wvc server/client process memory using smaps_rollup")
    p.add_argument("--json", action="store_true", help="Print JSON instead of a human summary")
    p.add_argument(
        "--include-aux",
        action="store_true",
        help="Include non-default-socket helper/test wvc processes in the output",
    )
    p.add_argument(
        "--socket",
        default=DEFAULT_SOCKET,
        help=f"Main wvc socket to treat as the primary instance (default: {DEFAULT_SOCKET})",
    )
    return p.parse_args()
...
def classify_process(argv: list[str], cmd: str, main_socket: str) -> tuple[str | None, bool]:
    ...
    if "cargo build" in cmd or "rustc --crate-name wvc" in cmd or "handle_resume_session" in cmd:
        return None, False
...
def iter_wvc_processes(main_socket: str, include_aux: bool) -> Iterable[ProcMem]:
...
def main() -> int:
    args = parse_args()
    procs = list(iter_wvc_processes(args.socket, args.include_aux))
