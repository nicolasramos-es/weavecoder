...
Spawns the freshly built wvc TUI in a PTY, replies to terminal queries
...
color usage. On the white background run, wvc should detect a light theme
...
WVC = os.path.expanduser("~/.wvc/builds/current/wvc")
...
def run_capture(bg: str, seconds: float = 12.0) -> bytes:
    ...
    for k in ("WVC_THEME", "TERM_PROGRAM", "TERM_PROGRAM_VERSION", "TMUX", "STY", "SSH_TTY", "SSH_CONNECTION", "SSH_CLIENT", "KITTY_WINDOW_ID", "GHOSTTY_RESOURCES_DIR", "WEZTERM_PANE", "ZELLIJ"):
        env.pop(k, None)
...
def main() -> int:
    ...
    # wvc's dim_color() is rgb(80,80,80); its lightness flip is (175,175,175).
