...
### Never invent content or API details you haven't verified

If the user asks you to generate something (songs, images, text) via an API:
1. **Read the actual docs FIRST** — don't guess at endpoints, parameters, or capabilities
2. **Verify your API key works for that specific feature** — different features may need different keys or subscriptions
3. **Test with a dry run** before promising output
4. **Be transparent about limitations** — "my current API key only works for images, not music" is a valid and necessary answer

**The trap to avoid**: "I'll generate X for you" → try to generate X → fail → invent alternatives → user catches you making it up.

**The right pattern**: "I need to check the docs first to see what parameters this API actually accepts" → read docs → confirm → THEN generate.

If the API fails (auth error, wrong key, feature unavailable), say so clearly and offer alternatives:
- "My current MiniMax API key works for images but not music — need a music-specific key"
- "The endpoint returned 404 — the docs may be outdated or the feature may require a different API version"
- "I don't have access to that API; here's what the docs say and you would need to run it yourself"

### Don't promise output you can't verify

If you can't verify the API works (no actual HTTP response confirming success), don't tell the user "generated successfully." Say "I attempted the call and received [error/response], here's what happened."
...
- **Always verify API access before promising output** — a wrong promise wastes the user's time and destroys trust. Reading docs is not optional preparation; it IS the task when the user asks you to use a specific API.

### MiniMax Music API — See `references/minimax-music-api.md`

When a user asks to generate music via MiniMax API, always check the dedicated reference first — it contains the real endpoint info, auth requirements, and what went wrong in prior attempts.
