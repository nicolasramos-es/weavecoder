...
The Hermes dashboard runs on `http://127.0.0.1:9119` by default. Key configuration:

### Network binding (LAN access)

To access the dashboard from other machines on your local network:

```bash
hermes dashboard --host 0.0.0.0 --port 9119 --insecure
```

The `--insecure` flag is required for non-loopback binds (the dashboard exposes API keys). Store the host setting persistently:

```bash
hermes config set API_SERVER_HOST 0.0.0.0
```

This writes `API_SERVER_HOST: 0.0.0.0` to `config.yaml`. The dashboard process itself still needs `--host 0.0.0.0 --insecure` at launch time.

### In-browser Chat tab (embedded TUI)

The dashboard includes an in-browser terminal that lets you chat with Hermes from the web UI. Enable it with:

```bash
hermes dashboard --host 0.0.0.0 --port 9119 --insecure --tui
```

Or set the env var for persistence:

```bash
echo "HERMES_DASHBOARD_TUI=1" >> ~/.hermes/.env
```

Background-start the dashboard permanently:

```bash
nohup hermes dashboard --host 0.0.0.0 --port 9119 --insecure --tui > /tmp/hermes-dashboard.log 2>&1 &
```
...
```yaml
dashboard:
  theme: mono             # default, midnight, ember, mono, cyberpunk, rose
  show_token_analytics: false
```

---

## Server Migration (Moving Hermes Between Machines)

When moving Hermes to a new server (e.g., from an Intel N100 mini PC to a Mac Mini M1, or between cloud VMs), you need to transfer the full `~/.hermes/` directory. This is not just a config file — skills, memory, session history, OAuth tokens, and credential pools all live there.

### What to transfer

| What | Path | Why it matters |
|------|------|----------------|
| Config | `~/.hermes/config.yaml` | Model, provider, terminal backend, display settings, all custom configs |
| Secrets | `~/.hermes/.env` | API keys for all providers — **critical, do not skip** |
| Skills | `~/.hermes/skills/` | All agent-created and hub-installed skills with their reference templates |
| Sessions | `~/.hermes/state.db` | SQLite session store — preserves history and FTS5 search |
| Auth | `~/.hermes/auth.json` | OAuth tokens (Nous Portal, OpenAI Codex, GitHub Copilot) |
| Profiles | `~/.hermes/profiles/` | Isolated configs if you use profiles |
| MCP config | in `config.yaml` | MCP server definitions (e.g., `cua-driver`) |
| Cron jobs | in `config.yaml` + in `state.db` | Scheduled jobs reference both |
| Curator state | `~/.hermes/skills/.usage.json` | Skill usage telemetry (optional but nice to keep) |

### Migration procedure

```bash
# 1. On old machine, create a tarball of everything Hermes
tar czf hermes-backup.tar.gz ~/.hermes/

# 2. Copy to new machine (scp, rsync, USB drive, etc.)
scp hermes-backup.tar.gz user@new-machine:~

# 3. On new machine, extract
tar xzf hermes-backup.tar.gz -C ~/

# 4. Verify critical components
ls ~/.hermes/config.yaml ~/.hermes/.env ~/.hermes/state.db

# 5. Verify Hermes can read everything
hermes doctor
hermes config check
hermes skills list
...
**macOS:** Hermes on macOS uses `cua-driver` for `computer_use` tools. After migration, install it:
```bash
pip install cua-driver
# Or via the MCP config that's already in config.yaml if you transferred it
```

The MCP server config for `cua-driver` is in `config.yaml` under `mcp_servers.cua-driver` — it references the binary path from the old machine. If the binary path differs on the new machine, update it:
```bash
hermes config set mcp_servers.cua-driver.command /new/path/to/cua-driver
```

**Linux:** SSH key and terminal backend setup may differ. Verify with `hermes doctor`.

**After migration:**
- If you changed hostname or IP, update any cron job delivery targets
- Restart the gateway: `hermes gateway restart`
- Verify skills loaded correctly: run `/skill <name>` for critical ones
- Verify MCP servers: `hermes mcp list && hermes mcp test cua-driver`

### What NOT to transfer
- `~/.hermes/logs/` — gateway logs are ephemeral, start fresh
- `~/.hermes/sessions/*.jsonl` — old raw transcripts; `state.db` has the canonical session store
- `.venv/` or `venv/` inside `~/.hermes/` — reinstall with `pip install hermes-agent` on new machine

### Verification checklist

After migration, run these before declaring success:
- [ ] `hermes doctor` — all green
- [ ] `hermes config check` — no missing/outdated fields
- [ ] `hermes chat -q "hello"` — can chat
- [ ] `hermes skills list` — all skills present
- [ ] `hermes sessions list` — old sessions visible
- [ ] `hermes mcp list` — MCP servers connected
- [ ] `hermes cron list` — cron jobs intact
- [ ] Gateway starts if you use it: `hermes gateway status`
